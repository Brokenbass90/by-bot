...
## Security incident: MT5 token

После тематического commit параллельная правка вернула реальный MT5-токен и включённое по умолчанию execution в `signal_copy/config.py`. Небезопасный commit `5754ced` успел попасть в remote. Локальная ветка пересобрана без секрета (`4e55586`), с `EXECUTION_ENABLE=0`, пустым account allowlist и `ALLOW_LIVE=False`.

Обязательные действия владельца: отозвать этот MT5-токен, создать новый только в локальном `.env`, затем отдельно разрешить `git push --force-with-lease` очищенной ветки. До этого MT5-контур не запускать.
