...
## ДОБАВЛЕНО 2026-07-06 late afternoon (Codex — Alpaca fills, web deploy, maker-fill scan)
- Git/deploy: локально были Claude-коммиты до `7830297`; `git pull --ff-only` clean, `git push` обновил origin до `7830297`. VPS `/root/by-bot` fast-forwarded `ff3ee50 -> 7830297` без reset и без трогания runtime-generated `configs/intraday_config.json` / `configs/portfolio_allocator_latest.env`. Server py_compile OK.
- Web deploy: перезапущен только `uvicorn web.main` на `127.0.0.1:8765`; live `bybot.service` не трогался. `/position.html` и `/api/position` отвечают `200`; `/api/trades/chart` без auth отдаёт `401` (ожидаемо); `runtime/live_positions.json` содержит `entry_ts` для ADA.
- Alpaca LIVE: API/digest на VPS подтвердили реальные позиции `SNOW`, `GE`, `ABBV`, `BAC`; account equity около `$496`, cash/BP около `$149`. По каждой позиции стоит broker-side sell stop: `SNOW stop 234.23`, `GE 356.47`, `ABBV 247.77`, `BAC 56.31`. Это не paper: endpoint `https://api.alpaca.markets`. Native trailing в конфиге есть, но текущие fractional-позиции защищены обычными simple stop orders.
- Bybit ADA: `ADAUSDT Sell` по ATT1 всё ещё открыта и защищена. Runtime at check: entry `0.189137`, current около `0.1857`, qty `138`, exchange SL `0.1893`, TP на бирже `null` by design, runner ladder enabled; TP1 `0.183584` уже `hit`, TP2 `0.177717` pending, breakeven/trailing armed. Текущий notional около `25.6 USDT`, uPnL около `+0.47 USDT`. `MANUAL TPSL LOCK TP=None SL=0.1893` нормален: TP управляется runner-лестницей, не биржевым TP.
- WS transport: read-only guard check: `active=false`, `critical_streak=0`, last status `NO_CONNECT_TRANSIENT`, last window `bybit_msgs=31095`, `disconnect=1`, handshake `0`. Риск не аварийный, но нужен общий system health checker по disconnect-rate/CPU/RAM.
- VPS hygiene: найден старый server research `att1_density_v3_more_pivots` consuming ~55% CPU / 45% RAM; остановлен, чтобы не мешать live/WebSocket. На VPS оставлены live bot, web, orderbook/liquidation collectors, restart_when_flat watcher. Новые тяжёлые исследования запускаются локально, не на live server.
- Inplay maker-fill gate: добавлен research-only `scripts/run_inplay_maker_fill_gate_20260706.py`. Он генерирует r061 сигналы через `InPlayBreakoutWrapper`, затем прогоняет `simulate_maker_trade` по offset `{0.1,0.25,0.4}*ATR` x validity `{6,12,24}`. Pre-reg thresholds: stress PF >= 1.2, 3/4 folds, unfilled < 50%, concentration < 0.35. Smoke 30d OK. Full scan запущен локально: `screen=inplay_maker_fill_gate_20260706`, log `logs/inplay_maker_fill_gate_20260706/run.log`.
- First maker-fill combo printed later: `offset=0.1`, `validity=6`, `stress_trades=84`, `stress_netR=+1.39`, `stress_pf=1.037`, `unfilled=9.68%`, gate `FAIL stress_weak;time_folds_weak_2/4`. Full grid still running; no promotion from this first row.
- FX/CFD backfill: локальный `screen=fx_cfd_backfill_gate_20260706` жив. CSV status at check: total ~1.09M M5 rows; `EURUSD/GBPUSD/USDJPY/EURJPY` около `145k-148k` rows, `GBPJPY/USDCAD/AUDJPY/BTCUSDT` около `100k`, `XAUUSD` пока `13.6k`. FX verdict not ready; золото особенно ещё data-gated.
- Live money impact: новый crypto sleeve НЕ включён; новый Alpaca sleeve уже live с capped v38 и broker stops; Bybit risk не увеличивался; IVB1/inplay/FX остаются shadow/research до PASS.
- Next check: через 1-2 часа проверить `logs/inplay_maker_fill_gate_20260706/run.log`, `screen -ls`, Alpaca `runtime/alpaca_live_v38/account_state.json`, Bybit `runtime/live_positions.json`, и FX row counts.

## ДОБАВЛЕНО 2026-07-06 evening (Codex — ADA runner stop truth)
- Direct Bybit REST check beats runtime labels: `scripts/check_bybit_open_positions.py` showed open `ADAUSDT Sell size=138`, `takeProfit=""`, `stopLoss="0.1893"`, unrealized PnL about `+0.61 USDT`. So exchange protection exists; `live_positions.json exchange_sl=0.179861` was an internal runner/local-state value, not the actual broker stop.
- Root nuance: runner profit-lock/trailing wanted to move short stop below entry, but the old sanitizer validated SL vs entry, not current market; it could leave `tr.sl_price` with a non-protective value while exchange retained the older stop. Added local code fix: `protective_stop_is_live()` checks stop vs current price; runner now updates local SL only after a valid protective broker update. Tests: `py_compile smart_pump_reversal_bot.py bot/tpsl_policy.py`; `.venv/bin/python -m pytest -q tests/test_tpsl_policy.py tests/test_live_position_view.py` -> `14 passed`.
- Deploy note: live bot was NOT restarted with this fix while ADA is open. Deploy/restart only when flat or via existing restart-when-flat hygiene.

## ДОБАВЛЕНО 2026-07-06 night setup (Codex — inplay verdict + dynamic selector)
- Inplay maker-fill strict gate completed: `reports/research/inplay_maker_fill_gate_20260706_20260706_164001/`. Verdict `FAIL`, but close to useful: best `offset=0.4`, `validity=24`, stress `84 trades`, `+6.46R`, `PF=1.173`, `unfilled=25.66%`, concentration `0.255`; failed because prereg required stress PF >= 1.2 and 3/4 positive folds, actual folds `2/4`. Interpretation: fill-rate is acceptable; edge is regime/symbol-window dependent, not stable enough for shadow/live.
- Added `scripts/run_inplay_maker_dynamic_selector_20260706.py`: causal selector that trains symbols on a prior window, selects top-N, then tests only the future window with the same maker-fill execution. Smoke passed after CSV fix.
- Started overnight local screen `inplay_maker_dynsel_overnight_20260706`, log `logs/inplay_maker_dynsel_overnight_20260706/run.log`. Bounded grid: train `90,180`, test `45`, topN `2,3,5`, min trades `3,5`, offset `0.4,0.55`, validity `24,36`, folds `4`, symbols broad crypto basket. PASS here can justify shadow/risk=0.0; no automatic live money.
- FX/CFD backfill still running: `screen=fx_cfd_backfill_gate_20260706`; cache now ~1.14M M5 rows. `EURUSD/GBPUSD/USDJPY/EURJPY/GBPJPY` near `145k-148k`, `XAUUSD` still only `13.6k`, so FX/CFD verdict remains blocked by data.
- Latest read-only live checks: web `/position.html` and `/api/position` return `200`; Bybit direct REST `ADAUSDT Sell size=138`, stop `0.1893`, PnL about `+0.78 USDT`; `runtime/live_positions.json` current about `0.1829`, notional about `25.2 USDT`, TP1 hit, TP2 pending. Alpaca Monthly LIVE shows 4 positions, equity about `$496`, cash/BP `$149`.

## ДОБАВЛЕНО 2026-07-07 morning (Codex — live truth + compute status)
- Bybit direct REST remains the source of truth: open `ADAUSDT Sell`, size `138`, entry `0.18913665`, unrealized PnL about `+1.33 USDT`, broker-side `stopLoss=0.1893`, exchange `takeProfit=""`. This is protected near breakeven, but it is not a profit-lock stop. Runtime still showed an internal runner SL below market (`0.178939`), so live UI/AI must not treat local runner SL as exchange truth. The already-deployed-on-disk protective-stop fix will only become active after flat/restart.
- VPS hygiene: `restart_when_flat_20260706` is alive and waiting for Bybit flat (`open_positions=1`, `flat_confirmations=0/5`). Do not restart `bybot.service` manually while ADA runner is open unless owner explicitly chooses that risk. Same-value `MANUAL TPSL LOCK` spam is fixed in code, but current live process may still emit old alerts until restart-after-flat.
- Alpaca LIVE: account state from live endpoint shows equity about `$496`, cash/BP about `$149`, positions `SNOW`, `GE`, `ABBV`, `BAC`; all four have broker-side sell stop orders. The 30-minute monitor is read-only/dry-run outside send-orders windows. Fractional positions use simple stop protection; native trailing is configured but skipped for these fractional holdings.
- Crypto sleeve status: no second live crypto sleeve yet. `inplay_maker_fill_gate` final result remains FAIL (`best stress PF=1.173`, prereg PF needed `>=1.2`, time folds `2/4`). `inplay_maker_dynsel_overnight_20260706` is still running at high CPU; early rows are mostly `0-1` trades and FAIL, so no shadow/live promotion before final verdict.
- FX/CFD: 730d Dukascopy backfill completed for `EURUSD,GBPUSD,USDJPY,EURJPY,GBPJPY,XAUUSD`; all 6 passed M5 cache preflight with coverage `0.669-0.706`. `fx_cfd_multi_strategy_gate` is running at high CPU over 7 strategy families. No FX/CFD capital until strategy gate/OOS verdicts exist.
- Web UX: added a sidebar link `Systems -> Live Position` to `/position.html` so owner can reach the detailed open-position panel without remembering the URL. This is web-only; no live bot or order behavior changed.

## ДОБАВЛЕНО 2026-07-07 advisor processing (Codex — DeepSeek review triage + FX verdict)
- Processed three external-advisor pasted reviews. Accepted the strategic correction but not the unsafe parts: add a separate `Exploration` track with soft thresholds for idea discovery, while `Validation/Promotion` gates remain strict for canary/live money. This resolves the tension between "move faster" and "do not fund unproven sleeves".
- `reports/MASTER_MAP_AND_PLAN_2026_07_08.md` updated: FX/CFD is no longer data-blocked; it is research-data-ready. Added two-track gate policy, exploration queue, and explicit no-blind-delete rule for old strategies. Created fresh `reports/CODEX_HANDOFF_2026_07_07.md`.
- Rejected external advice to delete/archive 71 strategies immediately. Cleanup is allowed only after inventory/reference mapping; no destructive repo cleanup during live operations.
...
## ДОБАВЛЕНО 2026-07-07 web shell fix + strategy priority (Codex)
- Owner rejected blind deletion/archive of old strategies; Codex agrees. Policy stands: keep all potentially useful strategy families, build inventory/coverage/ownership first, archive only confirmed dead diagnostic clutter after references are mapped. Goal remains broad coverage: long/short, sloped/horizontal levels, trend/range/breakout/retest/sweep/funding across markets.
- Web fix after owner report that `/position.html` replaced the old UI: `web/main.py` now serves the SPA shell for `/position.html`, `web/static/index.html` selects `Live Position` from URL, and the detailed panel is embedded from `/static/position.html`. `X-Frame-Options` changed to `SAMEORIGIN` so the same-origin iframe works. `web/static/position.html` chart no longer uses spread over full candle arrays, removing the `RangeError: Maximum call stack size exceeded` failure.
- ADA truth unchanged: Bybit broker TP is empty by design; ATT1 runner manages TP2/holding in bot logic while exchange SL protects near breakeven. This is not ideal profit-lock behavior. Next research work for ATT1 is exit A/B: current runner vs profit-lock-after-TP1 vs tighter ATR trail vs retest-exit/re-entry. Do not change live ADA by hand unless owner explicitly approves.
- Top 3 next bets, in order: (1) ATT1 exit/re-entry A/B because live signal exists and current ADA exposed the exit weakness; (2) level_memory-powered range/sweep/reclaim family, long/short and horizontal/sloped variants, replacing naive broad MRB; (3) FX/XAU H1 range/sweep/session exploration on completed Dukascopy data. Inplay stays frozen until entry-quality/level_memory repair; broad MRB is a failed baseline, not a dead idea.
- Runtime `ASC1 hot-reloaded Added DOTUSDT / Removed LINKUSDT` is universe selection, not live allocator risk by itself. It affects scanners/shadow/candidates unless the corresponding sleeve has positive live risk.

## ДОБАВЛЕНО 2026-07-07 midday (Codex — exit telemetry truth + level-memory sweep research)
- Fresh VPS read-only check at `2026-07-07 12:00 UTC`: `ADAUSDT Sell` remains open, qty `138`, entry `0.189137`, current about `0.1775`, uPnL about `+1.61 USDT`, exchange SL `0.1893`, exchange TP `null`. Runtime row says `tp_model=none`, `runner.enabled=false`, `runner.targets=[]`, breakeven/trailing/time-stop disabled. Therefore the current live position is protected near entry by exchange SL but does NOT have a visible profit-lock/TP runner in runtime. Do not describe ADA as trailing/runner-protected until the live core is flat/restarted and verified.
- Web/AI observability patch: `bot/position_view.py` now emits `exit_state` and `runner_state`, including warnings such as `no_tp_plan_visible`, `profit_not_locked`, `runner_disabled`, `trailing_disabled`, and hides misleading headline `R` when it is computed from a tiny current-SL denominator instead of original risk. `web/static/position.html` displays these warnings and sends Alpaca positions/recent events/manage rails into the AI chat context.
- Tests: `tests/test_position_routes.py` now covers unprotected-profit runner telemetry and nested runner targets. Local validation: `9 passed` for `tests/test_position_routes.py tests/test_live_position_view.py`, plus `py_compile bot/position_view.py`.
- FX H1 exploration result: completed, no capital-ready row. Best diagnostic is `XAUUSD trend_retest_session_v2` with positive estimated return in the soft exploration harness, but the row is not promotion-grade; most EURUSD/GBPUSD/USDJPY range/sweep/trend rows are negative. FX/CFD is research-ready, not live/demo-capital-ready.
- Added research-only `scripts/run_crypto_level_memory_sweep_reclaim_20260707.py`: H1 horizontal support/resistance sweep+reclaim with `level_memory` filter, long/short, cached data only, no network/orders. Smoke on BTC/ETH/ADA/SUI/DOGE 180d: `24` trades, net `+4.93R`, PF `1.40`, 2/4 folds, but too few trades for PASS. Full local screen started: `crypto_lm_sweep_reclaim_20260707` over 24 symbols/360d/lookbacks/respect/RR grid. PASS here can only justify stricter validation or shadow/risk=0.0, never live money directly.

## ДОБАВЛЕНО 2026-07-07 early afternoon (Codex — runner restore fix, no live-core restart)
- Root cause of the ADA confusion is now explicit: ATT1/inplay-style entries can create a runner ladder (`tps`, `tp_fracs`, breakeven, ATR trailing, time-stop), but `_restore_trade_state_from_exchange_row()` rebuilt open exchange positions without runner fields. After a process restart, an open runner trade could become a local `TradeState` with exchange SL only and no durable TP/trailing plan.
- Fix on disk: `bot/runner_state.py` now serializes/deserializes runner snapshots. `smart_pump_reversal_bot.py` writes `runner` snapshots to `runtime/live_trade_events.jsonl`, logs runner TP/BE/trailing/time-stop state changes, and hydrates runner state during exchange restore. If exchange qty is already reduced, restore infers earlier TP slices as hit so they are not closed twice.
- Current ADA was not hot-hydrated and live core was not restarted. The fix protects future/restarted runner trades after flat/restart; it does not silently move ADA's live SL or close profit. Any current ADA manual close/SL move still requires explicit owner approval.
- Validation: `.venv/bin/python -m pytest tests/test_runner_state_restore.py tests/test_live_position_view.py tests/test_position_routes.py` -> `12 passed`. Research script smoke with progress logging completed on `ADAUSDT,BTCUSDT`.
- `scripts/run_crypto_level_memory_sweep_reclaim_20260707.py` now emits JSON progress per parameter combo. The previous full screen had an empty log and no new output; restart long scans only with progress logging.

## ДОБАВЛЕНО 2026-07-07 evening (Codex — ADA incident close + heartbeat runner guard)
- ADAUSDT ATT1 short was manually closed by owner after AI/web correctly flagged `runner empty / no visible TP plan / no profit lock`. Realized result was profitable (`+1.36 USDT` today on Bybit), but it is recorded as a live execution incident: the entry worked, autonomous exit management did not. This is not counted as a clean autonomous sleeve win.
- Fresh VPS check after close: Bybit `positions=[]`, `position_query_ok=true`, equity about `1020.64 USDT`, available about `1020.64 USDT`, `bybot.service=active`. This permits a flat-window restart/deploy.
- Added runner heartbeat fallback in `smart_pump_reversal_bot.py`: `_manage_all_open_runners()` is called from `pulse()` after TPSL ensure and advances TP ladder / breakeven / ATR trailing / time-stop for every open Bybit trade with `runner_enabled`, independent of detect/tape subscription. Flag: `RUNNER_HEARTBEAT_ENABLE=1` default.
- Added optional far exchange TP safety net `_maybe_arm_exchange_safety_tp()` behind `RUNNER_EXCHANGE_TP_ENABLE=0` default. It is not enabled until explicit owner approval because it places real exchange TP orders.
- Added portfolio-wide health monitor `bot/portfolio_health.py`, alert-only by default. `PORTFOLIO_HEALTH_ENABLE=1`, `PORTFOLIO_HEALTH_AUTOCUT=0`. It writes `runtime/portfolio_health.json`; future auto-cut wiring must be reviewed sleeve-by-sleeve. After first VPS deploy a false `att1 halt` was caught from legacy dirty trades; fixed immediately by applying `PORTFOLIO_HEALTH_START_TS` / `ATT1_EDGE_START_TS` / clean r001 start `1783162792` to the monitor window.
- Telegram truth improvement: startup/status now prints `money-sleeves` and `runner_guard`, separating enabled code flags from live-money sleeves and showing heartbeat/safetyTP/health/autocut state.
...
- Real liquidation stream pulled from VPS: `runtime/liquidations/bybit_liquidations.jsonl`, `85009` rows, ~12MB. Started separate `screen=cascade_real_liq_20260707`; log `logs/cascade_liq_pull_20260707/run_cascade_real_liq_20260707.log`; outdir `reports/research/cascade_real_liq_20260707`.
- Separate MRB repair runs completed before night: `z18` FAIL (`netR=-139.18`, PF `0.844`, `0/4` folds) and `z24_fast` FAIL (`netR=-67.67`, PF `0.835`, `0/4` folds). Broad z-score basket remains non-promotable; флетовая ветка должна идти through `level_memory`/quality-level sweep-reclaim, not naive basket/grid.

## ДОБАВЛЕНО 2026-07-08 morning (Codex — Alpaca truth + next-sleeve OOS starts)
- Fresh read-only Alpaca LIVE snapshot generated via `scripts/tg_daily_digest.py --alpaca-only --dry-run` at `2026-07-08 06:06 UTC`: account active, equity `$490.80`, cash/BP `$192.23`, portfolio value `$490.80`. This is about `-$4.10` vs the `$494.90` funded base (`~ -0.8%`), i.e. a real but modest canary drawdown, not a blow-up.
- Alpaca positions at that snapshot: `ABBV -$1.96`, `BAC +$0.65`, `GE -$2.84`, `PANW -$0.75`, `XYZ -$0.07`. Broker-side simple stop orders exist for `ABBV/BAC/GE/PANW`; `XYZ` has an accepted market-sell order, not a stop, so recheck after US open whether it filled/flattened.
- Added strict prereg runner `scripts/run_level_memory_oos_prereg_20260708.py` for the closest crypto candidate: frozen `respect_min=0.65`, `lookback=48`, `rr in {1.2,1.6}`, base/stress/concentration, 40/8 selector, holdout symbols, BTC-regime split. Started `screen=level_memory_oos_prereg_20260708`, log `logs/level_memory_oos_prereg_20260708/run.log`, current output dir `reports/research/level_memory_oos_prereg_20260708_20260708_062304/`. This is research-only; no live risk.
- Updated `scripts/run_cascade_real_gate.py` with `--trigger-mode window_v1` and `--diag`. Legacy mode remains default. New mode replaces the binding contiguous-liq-onset/perctile-of-zero conditions with rolling window intensity and rolling-mean/absolute spike gates, and writes per-condition rates. Tests: `.venv/bin/python -m pytest tests/test_run_cascade_real_gate.py tests/test_cascade_reversal.py` -> `15 passed`.
- Started `screen=cascade_window_v1_real_20260708`, log `logs/cascade_window_v1_real_20260708/run.log`, output `reports/research/cascade_window_v1_real_20260708/`. First live log lines on LINK/ADA/SOL still show `trades=0`; final `diag.csv` is the important artifact if zero persists.
- Refreshed local AI context via `python3 scripts/build_ai_full_context.py --quiet`.
- Live-money impact: none. Bybit remains flat with only ATT1 r001 as live crypto money. Alpaca remains `$500` canary. No new sleeve promoted.

## ДОБАВЛЕНО 2026-07-08 midday (Codex — pre-pause live truth + FX grid)
- VPS read-only check before pause: `/root/by-bot` commit `f7ed011`, `bybot.service=active`, `runtime/portfolio_health.json` is being written. This confirms the live server has the runner-heartbeat and portfolio-health fixes from the 2026-07-07 incident path. Newer local commits above `f7ed011` are research/docs orchestration and not required for immediate live safety.
- Bybit direct REST on VPS: `open_position_count=0`, `positions=[]`; `runtime/live_positions.json` also reports `count=0`, `dry_run=false`, `trade_on=true`. Current crypto live-money state is flat, only ATT1 short r001 remains armed as money sleeve; no second crypto sleeve is live.
- FX/CFD coverage blocker was corrected by treating `data_cache/forex_1h` as hourly data with market-closure-aware gates. Strict H1 research for `EURUSD,GBPUSD,USDJPY` is now valid with `--interval-min 60 --min-coverage 0.98 --max-gap-bars 24`. `XAUUSD` remains blocked pending cleaner/full backfill.
- Started local research screen `fx_native_h1_full_grid_20260708`: `EURUSD,GBPUSD,USDJPY` x `trend_pullback,session_breakout_retest,session_range_fade,round_level_sweep`; log `logs/fx_native_h1_full_grid_20260708/run.log`; output `reports/research/fx_native_h1_full_grid_20260708`. First log confirms `EURUSD coverage=0.995732`. No orders, no live risk.
- Started local keep-awake screen `prevacation_keepawake_20260708` with `caffeinate -dimsu -t 432000` so local research screens keep running while the Mac is awake/on power. If the laptop is closed into sleep, local screens can still pause.
- `level_memory_oos_prereg_20260708` remains running and heavy; latest known event is `simulate_base_start rr=1.2`. It is the closest crypto candidate but remains research-only until prereg OOS verdict completes.
- `cascade_window_v1_real_20260708` finished with `combos=0`; new `diag.csv` shows the binding issue is mostly OI/direction/liquidity rarity on 21d data, not only the old contiguous-timing bug. Cascade remains data-collection/research, not a near-term sleeve.
- Pre-pause rule: no risk increase, no `RUNNER_EXCHANGE_TP_ENABLE`, no `PORTFOLIO_HEALTH_AUTOCUT`, no new live sleeve. The only acceptable promotion during/after the pause is strict PASS -> `shadow/risk=0.0`.

## ДОБАВЛЕНО 2026-07-10 (Codex — факты после паузы, LTC/DOT, Alpaca, FX/LM verdicts)
- Created `reports/CODEX_FACTS_AND_PLAN_2026_07_10.md` as the current one-page truth for the next session.
- VPS read-only check: `/root/by-bot` commit `f7ed011`, `bybot.service=active`; direct Bybit REST `open_position_count=0`; `runtime/live_positions.json` reports `count=0`, `dry_run=false`, `trade_on=true`.
- ATT1 clean-window closes since `ATT1_EDGE_START_TS=1783162792`: `BTCUSDT SL -0.4942`, `ADAUSDT manual/profit +1.3645` (not clean autonomous due runner incident), `LTCUSDT SL -0.3983`, `DOTUSDT SL -0.4598`. Net is roughly flat (`+0.0121 USD`) only because of manual ADA; autonomous post-fix ATT1 evidence is weak.
- LTC/DOT are not the ADA runner bug repeating. Runner snapshots were present. LTC reached only about `0.34R` MFE and DOT about `0.37R` MFE, below current `BE/trailing trigger=1R`, so they stopped before profit protection could arm. This is an entry/regime/exit-threshold problem.
- Alpaca LIVE snapshot at `2026-07-10 06:20 UTC`: equity `$488.58`, cash/BP `$246.21`, positions `ABBV,GE,PANW,SCHW`; all have broker-side stop orders. Approx drawdown from `$494.90` funded base is `-1.3%`. Intraday advisory is stale (`3.6d`), which is an observability/refresh issue.
- Level-memory OOS prereg finished `NO_PROMOTION`: base `83` trades, `+11.8122R`, `PF=1.2998`, but stress `PF=1.1086`, only `2/4` positive folds, OOS selector `pass=false`, holdout too thin/missing cache. Real pulse, not robust enough for shadow.
- FX H1 full grid finished with no promotion row. `USDJPY round_level_sweep` is the best diagnostic (`30-31` trades, `PF≈1.25-1.28`, `3/4` folds), but `preflight_go=false` due thin folds. EURUSD rows have only `3` trades; session/range fade is negative. No FX/CFD money.
- Current recommendation: do not raise risk; do not promote a new sleeve. P0 is strict ATT1 exit/regime A/B (`base`, `pure_trail`, `small_tp1`, early BE `0.3/0.5R`, trend-only vs chop-inclusive) before trusting ATT1 further.
- Added research-only `scripts/run_att1_exit_regime_ab_20260710.py`: it loads actual live `configs/att1_short_r001_canary_20260702.env` and overlays only exit/regime variants. Short control run confirmed the harness sees r001 trades; one 180d fold: `base/all_regimes` `210` trades `PF=1.270`, `base/trend_only` `172` trades `PF=1.185`, `early_be_05/all_regimes` `235` trades `PF=1.253`, `early_be_05/trend_only` `190` trades `PF=1.287`. This is not a verdict; full 4-fold screen `att1_exit_regime_ab_20260710` is running.

## ДОБАВЛЕНО 2026-07-10 (Codex — ATT1 exit/regime A/B final)
- ATT1 A/B full run completed: `reports/research/att1_exit_regime_ab_20260710_20260710_064618/summary.md`. This was research-only: no live config, order, or risk changed.
- Harness correction recorded: earlier false `0 trades` behavior came from running against the wrong cache/env path. The new runner forces `data_cache` plus live `configs/att1_short_r001_canary_20260702.env`, so the test now sees actual r001 geometry.
...
## ДОБАВЛЕНО 2026-07-10 (Codex — crisis-recovery frame, TG truth, web fail-closed, FX rewire)
- Created canonical continuation artifacts: `reports/PROJECT_SYSTEM_AND_ROADMAP_2026_07_10.md`, machine-readable `reports/PROJECT_CANONICAL_INDEX_2026_07_10.json`, TG audit `reports/AI_TG_FORENSICS_AUDIT_2026_07_10.md`, and durable HTML report. New chats must read index + roadmap, compare `last_session`, and continue the first unfinished action instead of restarting the audit.
- TG claim audit: weekly live report has 50 mixed historical closes and 29 `missing_candles`, but the latter means only post-hoc `.cache/klines` absence. Distribution: range 21, breakdown 7, flat 1, ATT1 0. ATT1 11 is legacy 7 plus r001 clean 4. Current r001 verdict remains impossible at N=4.
- `scripts/weekly_trade_forensics_ai_report.py` now emits a deterministic cohort/cache notice, injects runtime money-vs-shadow truth and project house rules, and carries the contract into the AI snapshot. Added regression tests.
- Local runtime P0 repairs: direct `scripts/build_ai_full_context.py` now imports repo modules; Alpaca intraday close reconciliation no longer hits undefined `base_url`; v3 shadow launcher is executable. No deploy/order/risk change.
- Web truth-first repair: trading mutations (`enable/disable`, safe mode, reload/SIGHUP) are blocked by both validator and executor until a verified live consumer writes effective-state ACK. Legacy overlay is labeled historical/non-effective. `run_backtest` is only a validated operator-review inbox; no automatic consumer exists. Payload is bounded/normalized.
- Level inventory confirms multiple working components (`market_context`, `unified_levels`, `chart_geometry`, `level_memory`, `render_levels`) but fragmented strategy wiring. Target is one versioned Level Service used identically by research/live/web/forensics; long/short remain separate configs and gates.
- FX native root cause fixed locally: trend pullback used current price as level; trend/breakout retest omitted touch/freshness metadata; full prefix created expensive repeated scans. `bot/fx_setups.py` now uses real `best_retest`, horizontal metadata, fail-closed sloped contract, and bounded causal windows.
- Post-fix fixed-row H1 smoke (`RR=2`, `SL=1 ATR`, hold 120): USDJPY trend pullback 40 trades, `-0.284R`, PF `0.990`, 2/4 folds; EURUSD/GBPUSD trend and all horizontal breakout-retest rows negative. Plumbing/frequency works; no edge/promotion.
- Existing USDJPY `round_level_sweep` lead reclassified: step formula is 10 JPY (big-figure/decade handle), not standard 00/50. Fixed row side split: short 18 trades `+10.6487R` PF `1.946`; long 12 `-4.6731R` PF `0.587`. Next valid step is frozen short-only prereg, not live money.
- Planning ranges: 1-2 sessions for reviewed deploy package/server inventory; 3-7 days for ops normalization and first strict verdicts; 1-2 weeks for level parity/side-specific repair; 2-4 weeks for shadow only if PASS; 6-12 weeks for meaningful clean live/shadow assessment. Stable income has no defensible date before verified edge and sufficient capital.
- Added read-only `scripts/build_repo_drift_manifest.py` plus tests. It classifies names/status/size/mtime without reading file contents and never moves/deletes. Latest local output `reports/repo_drift_manifest_local_20260710.json`: 382 records, including 36 manual-code candidates, 14 runtime/log, 7 archive/backup, and 14 secret-like filenames. These are review classes, not proof of secrets.
- Read-only VPS manifest completed and pulled to `reports/server_repo_drift_manifest_20260710.json`: head `f7ed011`, 877 status records, only 2 tracked changes. They are generated `configs/intraday_config.json` and `configs/portfolio_allocator_latest.env`. No cleanup/restart/deploy occurred. Normalization plan: `reports/SERVER_REPO_NORMALIZATION_PLAN_2026_07_10.md`.
- Allocator generated snapshot exposed an unsafe fallback dependency: approved base still advertised ATT1 0.70, flat 0.30, range 0.25 while server-only operator override reduced actual money to ATT1 0.10. Local `configs/approved_strategy_params.env` is now fail-safe without override: ATT1 short r001 0.10 only; all other sleeve risks zero; side/RSI parity explicit. Added regression tests. Not deployed yet, so live effective state is unchanged.

## ДОБАВЛЕНО 2026-07-10 evening (Codex — web auth recovery, Alpaca safe-hold, ATT1/level-memory wave)
- Web login P0 diagnosed from server truth: VPS `configs/web_config.json` had `0` users, so every password correctly returned `401 Unauthorized`. Restored the existing one-user password/TOTP config, set mode `600`, restarted only `trading-journal-web`, verified `active`, `/ping={"pong":true}`, and one enabled user. `scripts/deploy_private_web_server.sh` now preserves server-owned auth state unless `DEPLOY_WEB_AUTH_CONFIG=1` is explicit.
- Alpaca audit found a research/live parity break. The nominal monthly v38 was refreshed daily and the manager closed stale names every 30 minutes. Direct live fills for Jul 6-9: `7` closed roundtrips, `2` wins / `5` losses, realized about `-$5.716`, gross PF about `0.44`; this is not the 15-trades/year strategy represented by the research artifact.
- Alpaca real account is now in `safe-hold`: `ALPACA_ALLOW_NEW_ENTRIES=0`, stale-position closes off, mid-month rotation off. The daily refresh cron is commented with a backup of crontab. Dry-run verified current positions `ABBV,ABNB,GE,SCHW`, all four covered by broker stops, no planned new buys. Existing positions were not force-closed.
- Fixed same-cycle Alpaca re-entry after software SL/trail: a successful stop close now writes a re-entry block and updates the in-memory block before the buy loop. No live risk or capital was increased.
- Alpaca intraday v1/v3 operational P0s were deployed only to paper/shadow: v1 `base_url` NameError fixed; v3 launcher executable; both dry-runs exit `0`. Deeper v1 audit found its old PnL/equity log is `DATA_INVALID`: one AAPL fill near `+$1.21` was booked repeatedly after the crash, inflating the ledger by roughly `+$442`. Rebuild/idempotent fill ledger remains P0; v1/v3 must not become live-money branches.
- ATT1 entry post-filter diagnostic initially looked promising (`descending slope + RSI 50..70`: 187 retained historical trades, PF `1.429`, WR `60.4%`, 4/4), but the mandatory full strategy rerun changed cooldown/slot availability and rejected the apparent win. Exact rerun: filtered `231` trades, PF `1.327`, WR `57.7%`, `3/4`; base `379`, PF `1.277`, WR `57.9%`, `4/4`. Therefore no ATT1 live change: slightly higher PF is not worth lower net/frequency and a negative fold.
- Level-memory semantic P0 repaired: shallow sustained breaks can no longer be mislabeled as bounces; support `from_above` and resistance `from_below` histories are separate; `.cache/klines` list-format OHLC is now loaded correctly. Regression suite passed.
- First repaired side-specific result: resistance sweep/fade `short-only`, Elder off, `42` trades, `+4.05R`, PF `1.20`, WR `54.8%`, `3/4` folds -> `PASS_EXPLORATION` only. Elder permissive reduced it to `28` trades/PF `1.149` -> FAIL. Support reclaim `long-only`, Elder off: `38` trades, PF `0.871`, `1/4` -> FAIL. Strict Elder and remaining long A/B continue in local screens.
- Stopped wasting VPS research capacity: old liquidity-sweep grid was an exact duplicate of a completed `486/486`, `PASS=0`, max `3` trades run. Both automatic research queues are temporarily disabled with backups; no live bot process was touched. New useful runs are local and bounded.
- Full fast suite after changes: `890 passed`. Pushed Git commits `07b0bdd` (web/Alpaca safety) and `0e7eb73` (ATT1/level-memory research repair) to `origin/codex/dynamic-symbol-filters`.
- Promotion truth: nothing new was enabled for Bybit money. Alpaca only became more conservative. The next crypto candidate is the repaired short resistance sweep/fade, but it still needs stress, untouched-symbol/period validation, M5 maker-fill parity and shadow before canary.
- Strict short resistance-sweep validation completed. Temporal OOS passed (`33` test trades, `+5.411R`, 7/11 positive windows) and unseen-symbol holdout was strong (`33` trades, `+14.1059R`, PF `2.2952`, WR `69.7%`, 75% positive symbols), but the frozen base failed stressed costs (PF `1.003`) and concentration (top-symbol share `38%` versus `<35%`). Verdict `NO_PROMOTION`; do not advance to M5/shadow yet.
- Alpaca refresh research had a second parity defect: `run_equities_monthly_v36_refresh.sh` hard-coded simulator `top_n=3` while v38 live config advertised `4`. Added explicit `EQ_V36_SIM_TOP_N=4` and a research-only no-fetch/no-runtime-publish mode. Started bounded local `alpaca_v38_parity_20260710` replay: top3 control, top4 exact-cardinality, and frozen May-June 2026 forward pulse. No broker calls, orders or live picks are possible in this run.
- Alpaca parity replay finished: same 2024-05..2026-04 cache produced top3 `28` trades/PF `7.3258`/`+53.2838%` and top4 `33` trades/PF `6.7439`/`+50.7502%`, both max monthly DD `-3.856%`. This reproduces the old top4 headline. Local last data day is `2026-04-27`, so May-June forward produced no rows. VPS mirror's newer July summary is top3 (`30` trades, PF `6.4667`, `+63.0633%`) and therefore still not exact four-slot live parity. Isolated fresh-cache runner added, but its Yahoo execution was blocked by current external-network/usage quota.
- Added atomic idempotent Alpaca intraday exit ledger keyed by confirmed broker order ID, deterministic fallback, file lock, atomic ledger-first projection and fill-date booking. Duplicate replay cannot add PnL twice; conflicting same-ID PnL fails closed. `23` focused tests pass. Not deployed: corrupt historical equity log must be backed up/rebuilt first so the new ledger does not preserve false `+$442` as baseline.
- ATT1 reasons now include behavior-neutral entry cards (`r2`, pivots, age, entry/touch/reject ATR distances, body, ATR%). Started four chronological base-only folds in local screen `att1_entry_cards_20260710`; this is cohort generation, not a live filter.
- ATT1 newer cohort completed: base `346` trades, `+12.98`, PF `1.2055`, WR `56.3%`, 3/4, worst DD `0.6258`. Diagnostic `R² >= 0.80` was then rerun through the full executable strategy to account for cooldown/slot changes: `264` trades, `+13.41`, PF `1.2852`, WR `57.6%`, 4/4, worst DD `0.4065`. Frequency fell about 24%. Exact adverse cost stress (`10 bps` fee + `5 bps` slippage per side) failed: `263` trades, `+4.04`, PF `1.0779`, WR `56.2%`, only 1/4 positive folds; latest fold `-0.28`/PF `0.973`. Therefore no live/shadow promotion. Next gate is measured execution cost plus maker/retest replay and a new holdout, not more threshold mining.
- Full suite after the ledger/parity/instrumentation package: `897 passed`.
- Implementation package committed as `23f9446` (`fix: harden Alpaca accounting and research gates`). No new crypto sleeve/risk was deployed; the ledger remains intentionally local until corrupt-history rebuild.

## ДОБАВЛЕНО 2026-07-10 night (fresh data, crypto/FX no-promotion, side queue)
- Exact Git/live truth: local and origin are `0d9443b`; VPS checkout remains `f7ed011`, ten commits behind with a dirty tree. Selected web/Alpaca P0 files were deployed manually, but new idempotent ledger, level-memory/geometry and ATT1 research implementation are not on VPS. Do not claim all Git changes are live and do not blind-pull.
- Direct Bybit truth: exchange positions `0`; only ATT1 has positive effective money risk `0.10`. No second/third money sleeve exists.
- Direct Alpaca truth: real equity about `$486.84`, positions `ABBV,ABNB,GE,SCHW`, broker stops `4/4`, safe-hold flags effective. Paper v1 remains untrustworthy: PLTR has no broker stop and broker/runner entry prices differ.
- Fresh isolated Alpaca cache completed `59/59` through Jul 10. Shifted top4 24m: `31` trades, PF `7.2864`, `+58.0781%`, DD `-3.856%`. Correct April-signal/May-entry forward: TXN `+10.586%`, SBUX `-4.764%`, portfolio `+6.38%`, PF `2.2222`, but only `N=2`; no scale or rotation re-enable.
- Level-memory short fresh clean forward invalidated the previous pulse: `17` trades, `-4.2035R`, PF `0.6171`, stress PF `0.5414`; BTC/ETH/DOT holdout PF `0.9032`. Verdict `NO_PROMOTION`. A first attempt using gap-affected symbols was stopped before interpretation.
- Fresh USDJPY OOS anatomy on 12,341 Dukascopy-derived H1 bars: normal 00/50 short PF `0.963` (stress `0.789`); 1-JPY big-figure short full PF `1.349` but negative validation/stress holdout; legacy 10-JPY short PF `2.306` at only 12 selected trades. FX verdict `NO_PROMOTION`.
- Frozen InPlay side decomposition completed. Long-only: 23 stress trades, `+3.31R`, PF `1.298`, `3/4` folds, concentration `51.1%` -> FAIL. Short-only: 67 stress trades, `+10.15R`, PF `1.410`, `3/4`, unfilled `35.0%`, concentration `36.4%` versus frozen `<35%` -> FAIL. Post-run preflight then found a shared `1,033`-bar M5 gap in ADA/SUI, and the original maker runner compares per-symbol candle indices for portfolio occupancy; the side metrics are hypothesis-only until a clean timestamp-synchronised replay. Short earns one fixed-parameter independent-symbol check at risk zero; long requires an event-expansion redesign.
- Pre-registered that one InPlay short additivity check before outcomes: `configs/research/inplay_short_independent_additivity_20260710.json`. Independent BTC/ETH/BNB/XRP/AVAX share exactly `103,561` M5 timestamps each, coverage `0.998852`, max internal gap `0`; selection was cache-quality-only. Frozen short maker parameters, base/stress costs, 4 chronological folds, final 90-day holdout, symbol breadth and `<35%` concentration are immutable. PASS can advance only to risk-zero shadow.
...
## ДОБАВЛЕНО 2026-07-11 continuation (Codex — fail-closed live contract, truth-first reports/AI, InPlay successor)
- Direct VPS check before any deploy: `bybot.service=active`, fresh heartbeat, Bybit positions `0`, effective ATT1 risk `0.10`, operator override loaded, canary expiry `2026-07-20`. VPS active override inherited `ATT1_RSI_SHORT_MIN=40` instead of r001 `45`; prior four live entries all had RSI above 45, so this is a future parity risk rather than the cause of past stops.
- Git fail-safe repair: approved baseline now permits only ATT1 short risk `0.10`; every other sleeve risk is zero. Active r001+IVB1-shadow override and versioned r001 config both set RSI short minimum `45`. Heartbeat now builds a full ATT1 parameter contract/hash from the actual strategy config through `bot/att1_runtime_contract.py`; an initial undefined-global implementation was caught before VPS deploy and replaced by a dynamic tested helper.
- AI truth repair: added `configs/ai_operator_canonical_state.json`, per-source freshness, heartbeat/broker/canonical precedence and `control_recommendations_allowed=false` on stale/conflicting live truth. Web AI trading mutations remain proposal-only without a verified live consumer; weekly forensics separates mixed historical cohort, clean cohort and post-hoc cache gaps.
- Alpaca reporting repair: broker holdings are separate from research picks; fractional qty, safe-hold, account base/DD, open uPnL and broker stop coverage are visible; damaged intraday v1 history is forced to `DATA_INVALID` until broker-fill reconciliation. Telegram failure returns nonzero. Added weekday 22:10 UTC post-close report and 23:00 delivery watchdog.
- Imbalance audit: no production FVG/order-block module exists. Legacy trade-flow imbalance, snapshot order-book pressure and OHLC liquidity sweeps are separate, unproven mechanisms. FVG/OB may be tested later only as deterministic context with OOS ablation.
- Added research-only `strategies/pump_exhaustion_unwind_short_v1.py`: frozen pre-event horizontal/sloped/liquidity highs; closed-bar event FSM `expanded -> exhausted -> bearish CHoCH -> failed reclaim -> one next-open plan`; immutable event IDs; seen/planned ledgers; side purity; expiry/invalidation. It is explicitly `LIVE_READY=False` and needs persisted restart state plus a frozen prereg runner before shadow. `58` related tests passed. Cache mechanics smoke: BTC 0 events, ETH 2 events/1 plan, DOGE 1 event/0 plans; not performance evidence.
- Implementation commits pushed through `e286534`: fail-closed ATT1 contract, dynamic heartbeat truth, canonical/freshness-aware AI context, truth-first web/weekly reports, Alpaca reports/watchdog and pump-exhaustion successor. Documentation checkpoints commit after that SHA; future sessions must resolve current HEAD. VPS remains `f7ed011` and was 22 implementation commits behind. None of these session changes were deployed; existing Alpaca safe-hold and ATT1 `0.10` live behavior remain unchanged.
- New canonical roadmap: `reports/PROJECT_RECOVERY_TRUTH_AND_ROADMAP_2026_07_11.md`; continuation prompt: `reports/NEXT_CHAT_START_PROMPT_2026_07_11.md`.

## ДОБАВЛЕНО 2026-07-11 verified deploy + frequent crypto/FX V3 verdict (Codex)
- Git implementation checkpoint `115d032` совпал с origin. Targeted VPS SHA audit подтвердил все ATT1/AI/Web/Alpaca-reporting файлы; последний atomic/mtime-safe mirror script также установлен с backup `/root/by-bot-backups/live_mirror_115d032_20260711T161011Z`. VPS Git checkout намеренно не pull/advance: он остаётся `f7ed011` dirty, поэтому deployed-file manifest и server `git rev-parse` пока различаются.
- Direct Bybit audit около `16:10 UTC`: positions `0`, bot active PID `2750296`, fresh heartbeat, единственный money sleeve ATT1; effective contract short-only, risk `0.10`, RSI short min `45`, expiry `2026-07-20`, SHA `fd8048f7b6fd483a6d246969ec5f72782c780a1dcbb9df373f2d6a966161eeb6`. IVB1/Bounce/Midterm включены только как risk-zero telemetry/shadow. Live risk не повышался.
- Runtime watcher P0 закрыт: auto-apply hot reload требует две отдельные авторизации; dynamic router overlay по умолчанию меняет universe/router metadata, но не strategy params; operator override сохраняет приоритет. Stale `allowlist_restart_needed.flag` от 10 апреля остаётся hygiene artifact и не является текущим restart request.
- Web active PID `2750626`, `/ping={pong:true}`, auth config users=`1`, warning journal пуст. Реальный password/TOTP replay не выполнялся. Alpaca post-close/manual TG delivery прошла; weekday scheduled delivery/watchdog впервые должны быть подтверждены после due window 13 июля.
- Direct Alpaca report truth: SAFE-HOLD, equity `$486.93`, holdings `ABBV/ABNB/GE/SCHW`, broker stops `4/4`, new entries/stale rotation off. Fractional native trailing отсутствует; software `+3.5%/3.5%` poll-dependent trail не совпадает с research `BE0.8R+ATR1.5`.
- Immutable frequent-crypto run `reports/research/frequent_crypto_prereg_20260711/20260711_112429` завершил `15/15` cases с data/execution/side integrity PASS. Performance: ARS1 long ADX25 PF base/stress/fresh90 `0.374/0.292/0.821`; ARS1 short `0.682/0.550/0.514`; ASB2 no-descending `0.754/0.524/0.639`. Все три `NO_PROMOTION`; shadow/live eligible `0`. Short ADX-off control stress PF `1.005` — ниже gate, не кандидат после просмотра результата.
- FX/CFD V3 research-only branch реализует failed-break retest short, horizontal range rejection long/short и range-edge expansion/retest long/short. Fail-closed preflight=`DATA_DIAGNOSTICS_ONLY`, promotion-grade symbols `0`; historical macro news and OANDA spread/commission/financing artifacts отсутствуют/unpinned. Performance/demo/live не запускались.
- AI full context refreshed: critical truth allows analysis (`blockers=[]`), money sleeves by heartbeat=`['att1']`, canonical state now records frequent crypto `3/3 NO_PROMOTION`. AI остаётся observer/proposal-only, не владеет live risk.
- Full local regression after all changes: `977 passed`. Durable architecture contract: `reports/ARCHITECTURE_PARITY_AND_MONEY_PATH_2026_07_11.md`; final frequent verdict: `reports/FREQUENT_CRYPTO_VERDICT_2026_07_11.md`.

## ДОБАВЛЕНО 2026-07-11 web hardening + canonical AI deploy (Codex)
- Git/origin security checkpoint: `18050bf`; commits `c307085` и `18050bf` добавили атомарную root-only JWT-ротацию, symlink/entropy tests и убрали логирование первых восьми символов JWT. Полный regression: `981 passed`.
- Canonical AI/report bundle адресно установлен на VPS с backup `/root/by-bot-backups/ai_canonical_3c26464_20260711T162537Z`; `runtime/ai_context/full_context.json` пересобран и видит canonical state `as_of=2026-07-11T16:15:00Z`, frequent crypto `3/3 NO_PROMOTION` и FX V3 `DATA_DIAGNOSTICS_ONLY`. Live-control права ИИ не расширялись.
- Web code установлен адресно с backup `/root/by-bot-backups/web_auth_code_20260711T162806Z`. Новый 256-bit JWT создан в `/root/by-bot/.env.local`, regular root-owned mode `600`; дефолтный marker отсутствует, значение и его префикс не выводились.
- Перезапущен только `trading-journal-web.service`: PID `2750626 -> 2761068`; `bybot.service` не перезапускался. После startup `/ping=200`, `/auth/me=401`, `/api/health=401` без cookie; listener `127.0.0.1:8765`; журнал показывает `JWT secret: configured` без части секрета.
- `WEB_COOKIE_SECURE=0` оставлен намеренно: на VPS нет nginx/caddy/apache и listeners `80/443`; web доступен plain HTTP только через loopback/SSH tunnel. После HTTPS reverse proxy это обязано смениться на `1`.
- Реальный owner password+TOTP flow после ротации ещё не replayed. Если UI снова покажет `Unauthorized`, следующий безопасный шаг — локальный password reset без передачи пароля в чат; сама JWT-ротация исправляет session signing, но не неверный пароль.
- VPS Git checkout остаётся `f7ed011` dirty и теперь на `33` commits позади checkpoint; targeted deployed files являются runtime truth, blind pull/cleanup по-прежнему запрещены.
- Независимый post-change audit в `16:29 UTC`: `bybot.service` сохранил PID `2750296`/start `11:18:48`, direct Bybit REST `retCode=0`, positions `0`; heartbeat age `22s`, `trade_on=true`, `dry_run=false`, ATT1 short-only/risk `0.10`/RSI `45`/expiry Jul 20/hash `fd8048f7...`, единственный money sleeve. Failed systemd units `0`, disk `64%` с `8.5G` free, RAM available около `415MB`, swap около `194/2047MB`. Последняя прямая Alpaca проверка остаётся `16:10 UTC`; после неё менялся только web, не bot/cron/broker state.

## ДОБАВЛЕНО 2026-07-11 targeted manifest + pump persistence/prereg (Codex)
- Ops commit `3818a0a` добавил read-only explicit-file release manifest: SHA256/size/mode, atomic JSON, dirty только boolean, запрет path traversal/symlink/missing inputs; tool не умеет deploy/clean/restart. Первый receipt `f95cd3e` фиксирует семь точно известных web/canonical файлов из deployed checkpoint `4c7f645`; локальный mode `run_web.sh` при этом был нормализован `0711 -> 0755` в соответствии с Git/VPS.
- Research code freeze `e919ec3`: `pump_exhaustion_unwind_short_v1` получил atomic restart-safe state envelope с schema/version, physical short-only identity, source fingerprint, payload checksum, seen/planned ledgers и fail-closed corrupt/hash/symlink handling. Live wiring/orders/risk отсутствуют.
- Frozen prereg commit `202aead`: 720d window, `13` fixed symbols, next-open, base/stress costs, `4` chronological folds, `7d` embargo, untouched `120d` holdout, LOSO/breadth/concentration gates. Source SHA и state fingerprint PASS.
- Strict preflight честно вернул `BLOCKED_FAIL_CLOSED`: все `13` immutable input snapshots пока unpinned; `performance_computed=false`, `promotion_authorized=false`, broker/live calls отсутствовали. Следующий шаг — materialize/hash-pin inputs отдельным pre-outcome commit, затем повторить только preflight.
- Полный local regression после этого пакета: `1019 passed`. Git/origin implementation head `202aead`; VPS Git не pull/advance и отстаёт на `38` commits, research package на VPS не нужен и не разворачивался.
- Read-only 720d cache audit перед загрузкой: BTC/ETH PASS (`coverage=0.999426`, gaps `0`); остальные `11/13` FAIL, WIF rows `0`, ADA/SOL/SUI max gap `1033`. Окно не сокращалось после просмотра. Detached public-data screen `pump_exhaustion_720d_backfill_20260711` запущен для 11 недостающих symbols с `0.75s` polite delay, без ключей/broker calls; после загрузки очередь сама пишет `logs/pump_exhaustion_720d_20260711/coverage.csv`. Performance не поставлен в очередь.

## ДОБАВЛЕНО 2026-07-13 recovery continuation: live truth, accounting P0, strict pump verdict, FX/Alpaca gates
- Fresh read-only VPS truth около `09:04 UTC`: bot/web active, failed units `0`, heartbeat fresh, regime `bear_chop`, Bybit positions `0`; core bot PID не менялся с 11 июля. Единственный money sleeve остаётся ATT1 short-only risk `0.10`. Server tool затем достиг usage quota, поэтому новых deploy/restart не было.
- Ночная ADA ATT1 была одним логическим short trade: entry около `0.1636`, initial qty `249`, partial close `136.95`, residual `113` закрылся trailing. Старый finalizer сохранил только residual closedPnl `+0.13795153`; exact aggregate требует broker-row reconciliation.
- P0 partial-close accounting реализован в `bot/bybit_closed_pnl.py`: exact lifecycle/side/entry matching, order-ID dedup, quantity completeness и 7-day Bybit query windows. Commits `3f6278b` + `12a9abd` pushed; full regression проходил. Targeted release receipt готов, но VPS не deploy — не путать Git и live.
- Immutable pump snapshots материализованы `13/13` (`693ffa3`), затем runner/tests committed/pushed `c282ff6`; hash-bound authorization committed/pushed `55d00cc` до performance. Embedded preflight byte-identical, artifact manifest `11/11` hashes PASS.
- Strict 720d pump result committed/pushed `a8d8bb8`: `NO_PROMOTION`. Stress `N=39`, PF `1.234`, net `+2.922R`, frozen-sizing return `+1.228%`, conservative DD `3.015%`, 3/4 positive folds; holdout `N=6`, PF `6.30`. Failed only `min_trades 39<40` and `holdout 6<10`; gates не ослаблять. ONDO дал `93.4%` final stress net; LOSO без ONDO PF `1.018`, return `-0.127%`, дополнительная fragility. 16/25 active months, 7 red active, 9 zero-trade. Live/shadow не включались.
- FX V3 hardening `864b054`: level respect fail-closed, H1 explicit, strict typed news/currency/coverage and OANDA account/symbol-session/cost validators. Jul11 config сохранён как historical evidence и теперь actionable-blocks до source gate; новый versioned prereg только после owner OANDA/news inputs. Performance/broker/live не было.
- Alpaca four-arm parity methodology frozen in `bb377bc`: true monthly top4, accidental daily negative control, adaptive gated/ungated; one next-open/gap/cost/shared-exit contract, daily MTM/DD, PIT/survivorship/regimes/forward. Evidence `997f205`=`BLOCKED_FAIL_CLOSED`: all nine artifacts unpinned, outcome access false, SAFE-HOLD unchanged.
- Alpaca manual core restart не нужен: cron one-shot читает текущий код на invocation. Scheduled report due 13 Jul `22:10 UTC`, watchdog `23:00 UTC`; до due window подтверждения нет.
- ATT1 logic mechanically coherent, но entry geometry слабая: two pivots make R² trivial, unbroken/first-touch/respect/overshoot guards не mandatory. Это объясняет code-valid, но визуально нелогичные входы. Base live не менять по одной удачной ADA; challenger сначала preregister.
- Bybit key monitor: expiry `2026-08-12`, rotate до начала августа; ATT1 canary expiry `2026-07-20`, explicit review, no auto-extension.
- Full local regression after all implementation/research gates: `1070 passed`. Canonical continuation: `reports/RECOVERY_CHECKPOINT_2026_07_13.md`, `reports/PROJECT_CANONICAL_INDEX_2026_07_13.json`, `reports/NEXT_CHAT_START_PROMPT_2026_07_13.md`.

## ДОБАВЛЕНО 2026-07-13 causal level/event-long foundation + phase-0 freeze (Codex)
- Commit `a98b640` pushed: новый research-only `LevelSnapshot v1` для horizontal H1/H4 resistance/flip. Stable `level_id`, versioned `snapshot_id`, confirmed pivots/respects, zone/timestamps, source/provider/config/payload SHA и tamper revalidation. Старый `unified_levels` остаётся display/diagnostic и не повышен до execution contract.
- В том же commit физически отдельная long-only FSM `event_expansion_retest_long_v1`: expansion -> hold -> first retest -> right-bar higher-low -> later bullish BOS -> next-open plan; atomic restart-safe state store и seen/planned ledgers. План намеренно `executable=false`, `BLOCKED_RESEARCH_MECHANICS`; live/router/broker/risk не подключались. `18` новых focused tests; полный suite `1088 passed`.
- Commit `f07dd01` pushed: provider-independent exact aggregation closed M5 -> M15/H1/H4. Неполный bucket, gap, duplicate, off-grid, open/future tail и invalid OHLCV fail closed. `25` focused tests; research/live label даёт одинаковые canonical bar bytes/hashes при одинаковом input/fingerprint.
- Commit `526492a` pushed: machine-readable phase-0 prereg, standard-library-only preflight, tests и human report. Preflight `integrity_pass=true`, все source/test/dev13 hashes совпали, `57` focused tests PASS, но outcome/live остаются запрещены.
- Девять prereg blockers: performance runner; integrated H1/M15/M5 orchestrator; frozen exits; cost/funding receipts; external8 M5, metadata, liquidity, funding; same-window ATT1 additivity reference. Fixed historical replication cohort без замен: `FIL/UNI/ETC/ICP/TRX/TON/MNT/IMX`. Текущие dev13 данные — mechanics-only; действительно untouched будет только prospective после полного runnable freeze/push.
- Horizontal v1 не заявляет sloped parity. Наклонные уровни должны стать отдельным versioned contract с >=3 confirmed pivots, unbroken interval, projection identity и причинными break/flip timestamps.
- Финальная локальная проверка после implementation, aggregation, prereg и canonical updates: `1127 passed in 19.02s`.

## ДОБАВЛЕНО 2026-07-13 event-long phase-1 causal freeze (Codex)
- `dd427c4` pushed: отдельный research execution contract фиксирует exact next M5 open, immutable stop, actual-open target reanchor, stop-first ambiguity, adverse gaps, TP1/TP2 по `50%`, max hold `96 M5`, base/stress costs и exact funding-event policy. Полнота внешней funding history остаётся обязательным gate.
- `7249b45` pushed: horizontal LevelSnapshot evidence усилен. Pivot больше не равен respect автоматически; обязательны approach-from-below, законченная реакция с minimum ATR move-away и отсутствие close-break; truncated реакция не засчитывается. Source hash теперь идентичен shared closed-bar aggregation contract.
- `6dfd263` pushed: причинный physical long-only MTF successor соединяет closed H1 expansion с отдельными later M15 hold -> first retest -> confirmed higher-low -> strictly later bullish BOS -> exact next M5 boundary. State содержит source/config/provider pins, watermarks, consumed-retest ledger и plan outbox.
- `d7608d8` pushed: отдельный MTF state/outbox store. Один regular `0600` файл, parent/target no-follow, temp/file fsync, atomic replace, directory fsync, tamper detection и atomic ACK. Store single-writer; interprocess lock отсутствует и не должен подразумеваться.
- `1f95e85` pushed: authenticated MTF-to-execution bridge. MTF plan ID, bridge receipt ID и execution plan ID различны; fingerprint связывает M5/H1/M15, event, level, state, provider и config. E2E test проходит exact execution simulator; bridge сам не сохраняет receipt и не ACK-ает outbox.
- При интеграции найден critical downtime defect: один catch-up вызов мог создать план и затем прокрутить более поздние свечи, потеряв exact next-open authority. `72c273d` исправляет это: первый план является transaction boundary; source/count/watermarks замораживаются на `plan.known_at`, tail не читается до durable ACK, после ACK тот же tail доигрывается. Pure core и persisted adapter оба блокируют advance при pending outbox.
- `e05f7b3` pushed: новый immutable phase-1 config/preflight/tests/report. Integrity PASS, `BLOCKED_RESEARCH_RUNNER_DATA`, `PERFORMANCE_FORBIDDEN`, `LIVE_FORBIDDEN`, `97` exact related tests. Phase-0 сохранён неизменным как historical freeze; его старые LevelSnapshot hashes теперь ожидаемо mismatch и не переписываются задним числом.
- Девять remaining blockers: performance runner; durable bridge/trade receipt then ACK runner; funding completeness proof; uniform dev13 window (BTC/ETH missing final `119 M5`); external8 market data, metadata, liquidity, funding; same-window ATT1 reference. Никаких outcome metrics не открывалось.
- `7e93150` pushed: old phase-0 regression test теперь проверяет ожидаемый historical mismatch и сохраняет fail-closed semantics; current authority — phase-1.
- Полный local regression: `1191 passed in 25.53s`. Local/origin implementation head до canonical docs: `7e93150`. VPS deploy, service restart, live risk, broker orders и второй money sleeve отсутствовали.

## ДОБАВЛЕНО 2026-07-14 live P0 deploys + truth checkpoint (Codex)
- Bybit partial-close accounting package (`3f6278b`, `12a9abd`) адресно deployed по release receipt; core контролируемо restarted в flat-окне. После restart service/heartbeat были fresh, broker оставался flat, live risk не менялся. Existing ADA history отдельно требует broker reconciliation.
- Alpaca SAFE_HOLD P0 `cc3ef8a` pushed и targeted deployed: rearm теперь охватывает stale existing holdings. GE protective stop восстановлен; direct broker coverage `ABBV/ABNB/GE/SCHW = 4/4`, без buys и stale closes. Scheduled report 13 июля delivered; watchdog подтвердил delivery.
- Реальная Alpaca authority остаётся monthly v38 SAFE_HOLD. Intraday v1/v3 — отдельные paper/shadow paths; увеличение динамики не является исправлением daily-rotation parity defect.
- Event-long local uniform dev13 virtual crop прошёл input/source integrity и снизил remaining blockers `9 -> 8`; performance не вычислялась, `PERFORMANCE_FORBIDDEN` и `LIVE_FORBIDDEN` сохраняются. До отдельного commit это pending local freeze.
- Dukascopy M5 уже присутствует примерно за 728 дней; FX V2 все шесть side-specific sleeves отрицательны. Promotion-grade symbols `0` до data repair, pinned macro-news и account-specific cost contract. Старый grid не повторять.
- Активного overnight performance/autoresearch на проверке нет; обе canonical queues disabled. Liquidation/lossy-density collectors дают context telemetry, но не replayable L2 proof.
- Alpaca/web broker-truth patch, AI one-shot shadow mission и L2/publicTrade collector остаются local pending и не должны называться live без review/commit/deploy/process receipt.
- Canonical continuation: `reports/RECOVERY_CHECKPOINT_2026_07_14.md`, `reports/PROJECT_CANONICAL_INDEX_2026_07_14.json`, `reports/NEXT_CHAT_START_PROMPT_2026_07_14.md`.

## ДОБАВЛЕНО 2026-07-14 published truth, AI shadow, event input freeze и replayable tape (Codex)
- Alpaca quantity-aware truth/web package `66ffa02` pushed и targeted deployed. SAFE_HOLD one-shot создал authoritative post-action receipt: `ABBV/ABNB/GE/SCHW`, position qty равен remaining protected stop qty по каждому symbol, gaps/under/over-protection отсутствуют. Web service явно restarted, PID `2863782`, startup complete, `/ping={pong:true}` на port `8765`. Successful owner password/TOTP replay всё ещё не доказан; friendly 401 message не является login fix.
- Event-long uniform input amendment `5801cc6` pushed. Outcome-leak guard теперь recursive Unicode-normalized и блокирует PnL/PF/Sharpe/CAGR/returns/drawdown/winrate/trade count/equity aliases; freeze cutoff fail-closed. Deep validation: `13 x 207241` exact closed M5 rows PASS. Остаются `8` blockers; performance/live forbidden.
- AI one-shot shadow package `41da86d` pushed. AI authority физически ограничена `SELECT frozen card` или `ABSTAIN`; feed raw SHA pinned, causal prefix only, deterministic one-bar decision latency, automatic adverse entry/SL/TP/horizon, immutable 0600 receipt. `promotion_authority=false`, post-hoc selection not excluded, no broker/TG/live integration.
- Public Bybit tape package `4db0f4d` pushed. Он не читает keys и не имеет order capability; сохраняет raw `u/seq/cts/T/trade id`, explicit gaps, append-only UTC partitions, deterministic replay, fsync/lock/recovery and zstd completed-day compression. Bybit update IDs проверяются monotonic inside snapshot segment, not incorrectly assumed contiguous `+1`.
- Local bounded data clock started: `screen=l2_ondo_v1_20260714` (ONDO depth50 book+trades, cap 20 GiB) и `screen=trades_micro_v1_20260714` (ONDO/WIF/SUI/DOGE/1000PEPE/FIL trades, cap 8 GiB), both retention stop/min-free 80 GiB. First ONDO replay: snapshot 1, deltas 354, trades 18, gaps 0, PASS. Это данные, не edge.
- Full regression after combined packages: `1283 passed`. Local/origin HEAD match `4db0f4dc3aca5bafd115b426f8644049518ddf90`. Foreign dirty FX edits `bot/fx_setups.py` and `tests/test_fx_setups.py` were not committed.

## ДОБАВЛЕНО 2026-07-15 vNext truth audit, Alpaca/arb corrections and bounded research (Codex)
- Direct VPS truth around `16:28 UTC`: `bybot.service=active`, fresh heartbeat, broker flat, `trade_on=true`, `dry_run=false`, regime `bull_chop`; ATT1 short-only risk `0.10` remains the sole crypto money sleeve. Bybit key status `ok`, expiry `2026-08-12`, rotate by `2026-08-05`.
- Direct Alpaca manager receipt at `16:00 UTC`: monthly v38 `SAFE_HOLD`, equity `$486.77`, positions `ABBV/ABNB/GE/SCHW`, exact broker stop quantity coverage `4/4`, no new orders; SBUX skipped because new entries remain disabled.
- Alpaca gated-adaptive claim corrected: old `2.234%` DD omitted the fall from initial capital. Sparse-endpoint DD is at least `8.205%`; true daily/intramonth DD is unknown. Bear-2022 diagnostic was `-6.54%`, PF `0.280`, N `12`, same-close and 21-session rather than calendar-month. It is not live and not promotion-eligible.
- Arbitrage truth: cross-exchange shadow has 174 closed cycles with mean `-0.0314%`, median `-0.1049%`, p25 `-0.1907%`, WR `33.3%`; verdict `NO-GO`. Historical same-exchange strict screen was only `1.1–1.8% annualized` and `NO-GO`. A fresh public XRP lead (`0.01%/8h` gross funding snapshot) starts paper research, not a return promise or capital authorization.
- Claude Research Station completed 107 variants in about 3h11m: one IS pass, zero forward passes, zero OOS-symbol passes, zero survivors. It remains diagnostic because of mutable cache selection, repeated OOS, no multiple-testing correction and no parity.
- Added and validated a 25+ component machine-readable capability registry, AI stale-truth fail-closed gates, atomic non-overlapping web mirror bundle and physical AI env/deploy quarantine. Local full regression before the new research package: `1301 passed`.
- Preregistered Pattern Atlas v1 on immutable dev13: six horizontal H1 patterns, long/short separate, next-H1-open 6/24/72/168h paths, last 120d sealed, no costs/PnL/promotion claims. `7` focused tests PASS; integrity hashes PASS. Discovery completed with `20,372` observations. Sealed-tail bytes were SHA256-verified, while holdout rows were not decoded/scored/used. One fat-tail lead only: breakout-long 72h N909, mean `+54.5 bps`, median `-49.2 bps`, 10/13 positive symbol means; no promotion.
- Built research-only Bybit cash-carry shadow core: public GET only, default disabled, long spot + short perp, three completed funding observations, four adverse fills, both fee legs, funding/basis/delta guards and idempotent checksummed cycle ledger. `48` related tests PASS. Synthetic cycle net `-$0.431` per `$100` leg after costs, proving mechanics do not manufacture profit. No daemon/network/live deploy; durable collector, quantization, L2/partial recovery and real cycles remain.
- Canonical documents: `reports/PROJECT_VNEXT_AUDIT_AND_7DAY_PLAN_2026_07_15.md`, `reports/RECOVERY_CHECKPOINT_2026_07_15.md`, `reports/PROJECT_CANONICAL_INDEX_2026_07_15.json`, `reports/NEXT_CHAT_START_PROMPT_2026_07_15.md`.
- Independent pre-deploy review closed mixed-source, semantic-conflict and current-vs-cached authority races for both web and Telegram AI, including same-sleeve risk changes. Heartbeat now classifies all 21 executable flags; `/ai_code` cannot read configs; stale auxiliary AI packs are excluded. Alpaca DD handles zero equity. Final full regression `1334 passed`.
- Targeted release `ai_truth_aaf57f1_20260715` deployed nine exact files with SHA PASS and backup `/root/by-bot-backups/ai_truth_aaf57f1_20260715`; core restart followed three direct-flat Bybit confirmations, then the new heartbeat/context and web were verified. Post-state: broker flat, ATT1-only `0.10`, both services active, AI blockers empty, `.env` unchanged, VPS checkout intentionally still `f7ed011` dirty. Receipt: `reports/releases/AI_TRUTH_TARGETED_DEPLOY_RECEIPT_AAF57F1_2026_07_15.json`. Research files and foreign FX edits were excluded.

## ДОБАВЛЕНО 2026-07-15 PM — direct live recheck and three bounded successors (Codex)
- Direct read-only VPS check at `17:26 UTC`: `bybot.service` and `trading-journal-web.service` active; heartbeat fresh, `bull_chop`, `trade_on=true`, `dry_run=false`, allocator not blocked, direct Bybit positions `0`. Effective authority is complete across `21` executable flags; only ATT1 has money authority (`short-only`, risk `0.10`, review 20 July).
- Direct Alpaca report at the same check: live equity `$486.53`, day P&L about `+$2.16`, actual holdings `ABBV/ABNB/GE/SCHW`, broker stop quantity coverage `4/4`, `SAFE_HOLD`, no new entries or forced stale/rotation closes. Intraday remains paper and its old realized ledger remains `DATA_INVALID`.
- Cash-carry v2 adds default-disabled public instruments/orderbook/funding receipts, common spot/perp base quantity, multi-level all-or-none book walk, a hash-chained/fsync journal, restart replay and conservative break-even refusal. Frozen v1 is unchanged. Current-like XRP gives at most `31.92 bps` over 42 settlements versus at least `54 bps + real spread/depth`; BTC/ETH are lower and SOL lacks persistence. Current verdict `NO_ENTRY`; no capital, daemon, key or live integration.
- Pattern Atlas produced exactly one frozen successor: physical long-only prior-20-H1 horizontal breakout, next-H1-open, fixed 72h hold, no post-hoc filters/retest/stops/targets. Base/stress costs, actual funding requirement, four folds, embargo, breadth/concentration and LOSO gates are frozen. Integrity preflight `16 passed`; sealed rows decoded `0`; performance scorer remains a separate not-yet-implemented gate.
- Alpaca exact-parity foundation now has executable pure calendar/next-open/cost/gap/stop-first/shared-exit/daily-MTM/DD primitives and immutable fail-closed materialization. It still lacks six authoritative inputs: official XNYS calendar, PIT universe, PIT market manifest, corporate actions/delistings, broker lifecycle/cost calibration and a new genuinely future forward manifest. Performance was not computed and SAFE_HOLD was not changed.
- Claude Research Station remains useful only as a proposal generator: 107 variants, one IS pass, zero final survivors. Its largest-mutable-cache selection, reused OOS cohort and absent multiple-testing correction prevent promotion use; do not turn it into a blind thousand-grid until frozen inputs and a trial ledger replace these gaps.
- Full regression after all PM research, registry and preservation changes: `1362 passed in 27.96s`. Foreign `bot/fx_setups.py` and `tests/test_fx_setups.py` remained outside the package.

## ДОБАВЛЕНО 2026-07-15 PM — onboard AI canonical refresh and freshness repair (Codex)
- Canonical registry/state targeted-deployed with exact SHA backups, without `.env`, strategy, risk or service restart changes. The rebuilt AI context sees `26` components and `ATT1` as the only money sleeve.
...
- Canonical registry/state with this verdict was targeted-deployed without research code, service restart, `.env`, risk or order changes. AI context as-of `17:58 UTC`: 26 components, blockers empty, only ATT1 money authority. Deploy receipt: `reports/releases/AI_CASHCARRY_CANONICAL_REFRESH_RECEIPT_2026_07_15.json`.

## ДОБАВЛЕНО 2026-07-16 — live recheck, bounded research machines and truth split (Codex)
- Direct read-only VPS check around `04:06 UTC`: core/web active, fresh heartbeat, direct Bybit flat, equity about `$1020.08`, `bull_chop`, approved-env authority not blocked, only ATT1 short x0.10 has money authority. Current ATT1 canary N5, 2W/3L, about `+$0.15`; 72 post-restart attempts produced 0 entries, predominantly trendline refusal. Do not scale before review 20 July and a materially larger clean sample.
- Alpaca mirror `04:02 UTC`: SAFE_HOLD, equity about `$486.34`, ABBV/ABNB/GE/SCHW, broker stops `4/4`, order submit OFF. Forced liquidation is rejected; it adds path/execution risk and does not repair parity.
- Added a five-arm exact successor bakeoff with a genuinely future `2026-08-03`–`2026-11-04` seal. Source pins, seal and SAFE_HOLD checks pass; performance remains blocked by five authoritative inputs. Old `+50–63%` remains diagnostic, not live forecast.
- Added a bounded six-symbol public cash-carry station around frozen v2: public GET only, per-symbol hash journals, restart/replay, funding-aware schedule and hard duration/disk/observation caps. Long launch failed closed on sandbox DNS, so observations/cycles remain zero. Old `$5–15/month per $1000` claim is explicitly withdrawn; current expected income is unknown.
- Implemented a one-shot, long-only, cost/funding/fold/breadth/LOSO/concentration/occupancy/DD scorer for the single Pattern Atlas breakout-72h lead. Public materialization completed 13/13 symbols, 32 API pages and 6,400 funding events before any price row. The only sealed run then returned decisive `NO_PROMOTION`: N155, base/stress PF `0.392/0.281`, stress DD `36.90%`, winsor mean `-249.4 bps`, positive folds `0/4`, symbols `1/13`; invalid/censored 0. Retry, repair and TAO-only rescue are forbidden.
- Audited old Research Station: 107 variants, 1 IS, 0 forward, 0 OOS, 0 survivors. It is not expanded blindly because mutable inputs, reused OOS and missing multiple-testing control would optimize noise. The canonical pipeline is broad causal discovery -> freeze one hypothesis -> sealed time+symbol scoring -> prospective shadow.
- Local Telegram/health observability patch splits fresh runtime portfolio closes from stale historical `strategy_health.json` and prevents duplicate first-pulse startup flags. This patch is local, not targeted-deployed. Existing startup execution snapshot itself was accurate.
- FX V3 remains blocked by two external evidence classes, not API credentials: point-in-time historical macro news and broker/account/session-specific costs. Frozen M5 data means exact immutable files/hashes/timezone/window/closed bars; key alone cannot reconstruct historical spreads/financing.
- New canonical continuation files: `reports/RECOVERY_CHECKPOINT_2026_07_16.md`, `reports/PROJECT_CANONICAL_INDEX_2026_07_16.json`, `reports/NEXT_CHAT_START_PROMPT_2026_07_16.md`.
- Added a Bitget-native public normalization boundary with strict public URL/query allowlists, causal books/time/funding and independent source identity. It cannot enter the frozen Bybit journal. Focused Bitget tests `13 passed`; combined Bitget/station/Bybit research suite `46 passed`. A final Bybit station retry produced 6 DNS failures, 0 observations/shadow positions and durable `PAUSED` receipt; no long process, key, capital, order or VPS mutation remains.

## ДОБАВЛЕНО 2026-07-18 — Web truth/security, live recheck, sloped geometry and screenshot-universe gap (Codex)

- Direct live truth около `06:07 UTC`: core/web active, heartbeat fresh, broker flat, `bull_chop`, global block absent; ATT1 short-only x0.10 remains the sole money sleeve. Canary N6, WR50%, net `+0.4605 USDT`; one recent winning LTC close does not authorize scale. First review requires 20–30 clean broker-reconciled closes.
- ATT1 execution path has no obvious mechanical break, but entry geometry is weak: two pivots make R2 uninformative and mandatory unbroken/first-retest evidence is absent. Any repair is a separate preregistered challenger, never a hidden live rewrite.
- Web/auth hardening commit `6f59938` pushed and targeted-deployed as eight exact files with backup/hash post-check. Config secrets are removed from Git HEAD/ignored, AI code search cannot read configs/secrets and is admin-only, mirror/scanner fail closed on stale truth, heuristic score is not labelled probability, fractional Alpaca quantities and stale chart clearing are fixed, fake generic exchange accounts/tests are removed. Web-only restart; core/risk/orders unchanged.
- Owner login config/email is valid locally and on VPS; `Unauthorized` is password rejection before TOTP. Owner password+TOTP reset is required via `web/setup_totp.py`. Old secret still exists in Git history, so rotation is mandatory; history was not rewritten.
- Causal sloped research primitive commit `5db00d7` pushed: separate support/resistance, >=3 right-confirmed pivots, all-pivot OLS, close-unbroken as-of projection and stable source/config/input/pivot/snapshot identities. Focused `10 passed`; full regression `1438 passed in 32.31s`. It has no signal/performance/order/live authority.
- `5db00d7` canonical/sloped bundle targeted-deployed as three exact files with backup/SHA, no restart/risk/order/private calls. Rebuilt server AI context saw 27 capabilities, 80 setup cards and blockers `[]`; AI remains observer/proposal-only.
- Final map-only refresh from checkpoint commit `de06ad8` targeted-deployed two exact canonical files. Post-context: 28 capabilities, 80 setup cards, blockers `[]`, ATT1 remains the only money sleeve; both services active, no restart/risk/order/private calls. Receipt: `reports/releases/CANONICAL_EVENT_GAP_TARGETED_DEPLOY_RECEIPT_DE06AD8_2026_07_18.json`.
- Fresh mirror scanner sees 20 symbols and H1/H4 only. AKE/BANK/LYN/US screenshot examples are excluded before geometry, so current bot cannot reliably catch them. Frozen next task: separate research-only `event_universe_v1` over full Trading USDT-perp point-in-time universe, closed M5 relative-volume/range ranks, liquidity guards, bounded top-K and hashed prospective receipts; do not widen live-router.
- Screenshot patterns are preserved as causal hypotheses in `reports/SCREENSHOT_SETUP_TRANSLATION_2026_07_18.md`: compression/rising lows -> closed breakout/hold/retest, sweep/reclaim -> horizontal breakout/retest and separate sloped bounce/break consumers. Screenshots themselves are survivorship-selected, not edge proof.
- Public cash-carry clock is active until approximately `2026-07-23 04:37 UTC`. At `06:18 UTC`: 101 cycles, 606 attempts, 570 durable observations, 95 per symbol, all `observe`, economics passes 0, no keys/private calls/capital/orders. Authenticated read-only account receipt confirms spot maker/taker 10/10bps and linear 2/5.5bps; four taker fills = 31bps. Income forecast remains unknown and capital is forbidden.
