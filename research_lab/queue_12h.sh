#!/bin/bash
# ДВЕНАДЦАТИЧАСОВОЙ ПРОГОН: рабочий диапазон стопа по ВСЕМ ногам.
#
# Что уже знаем: ×1 — издержки съедают всё; ×12 — эдж падает вдвое
# (ATT1: +0.0628R при ×6 против +0.0245R при ×12). Оптимум между.
# Здесь ×4 и ×6 — обе стороны, оба флетовых режима, все 49 ног.
# Значения объявлены ДО прогона и не подбираются.
#
# 49 ног × 2 стороны × 2 стопа = 196 прогонов, примерно 10-12 часов.
#
# Запускать: nohup ./research_lab/queue_12h.sh &
cd "$(dirname "$0")/.."
