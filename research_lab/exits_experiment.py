...
ВЕРСИЯ 2. В первой версии порог R считался от ФАКТИЧЕСКОГО риска
сделки (стоп ×6). Это оказалось ошибкой конструкции, а не выбором:
при стопе ×6 первая цель стоит на 0.2 такого R, и позиция закрывается
задолго до порога 1.0. Безубыток и трейлинг не срабатывали НИ РАЗУ,
и все четыре варианта дали побайтово одинаковый результат. Это был
не нулевой эффект, а неработающий тест.

Теперь порог считается от ШТАТНОГО расстояния до стопа стратегии —
ровно так, как в живом боте. При стопе ×6 порог 1.0 штатного R
достигается на 0.167 фактического, то есть заметно раньше первой цели.
Объявлено заново, до прогона версии 2.
...
def simulate(bars, atr, i, side, sl0, tps, f1, use_be, use_trail):
    ...
    for j in range(e, min(e + HOLD, len(bars))):
        ...
        # ход в свою сторону в единицах ШТАТНОГО риска (risk / MULT),
        # как в живом боте, а не в единицах расширенного стопа
        native = risk / MULT
        run = ((entry - best) if short else (best - entry)) / native
        if use_be and not be_on and run >= BE_TRIGGER:
            be = entry - BE_LOCK * native if short else entry + BE_LOCK * native
