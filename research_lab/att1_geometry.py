...
files=sorted(os.listdir(D)); random.seed(11); random.shuffle(files)
NF=int(sys.argv[1]) if len(sys.argv)>1 else 25
files=files[:NF]
def to_h1(rows):
    out=[]; cur=None; h=None
    for b in rows:
...
for fn in files:
    ...
    CACHE[fn]=(h1, all_pivots(h1), atr_series(h1))
print(f"монет загружено: {len(CACHE)}")
def run(minp, unbroken):
    tr=[]
    for fn,(h1,(PH,PL),AT) in CACHE.items():
        n=len(h1)
        for i in range(140, n-49):
            a=AT[i]
            if not a or a<=0: continue
            cur=h1[i][4]; o=h1[i][1]; hh=h1[i][2]; ll=h1[i][3]
            rng=hh-ll
            ...
            for side,PV in (("short",PH),("long",PL)):
                ...
                pts=[p for p in PV if p[0] <= i-3][-minp:]
                if len(pts)<minp: continue
...
print("\nпивотов | unbroken |    n | ожидание R | win% | PF    | 1-я пол | 2-я пол")
for mp in (2,3,4):
    for ub in (False,True):
        s=summ(run(mp,ub))
        if not s: print(f"{mp:>7} | {str(ub):>8} | мало сделок"); continue
        mark=" <<<" if s['exp']>0 and s['h1']>0 and s['h2']>0 else ""
        print(f"{mp:>7} | {str(ub):>8} | {s['n']:>4} | {s['exp']:>10} | {s['win']:>4} | {s['pf']:>5} | {s['h1']:>7} | {s['h2']:>7}{mark}")
