...
BTCETHMidtermShortV2Strategy = _import_strategy_class("btc_eth_midterm_short_v2", "BTCETHMidtermShortV2Strategy")
...
SlopedBreakRetestV1Strategy = _import_strategy_class("sloped_break_retest_v1", "SlopedBreakRetestV1Strategy")
SlopedBreakRetestV2Strategy = _import_strategy_class("sloped_break_retest_v2", "SlopedBreakRetestV2Strategy")
SlopedBreakRetestV3Strategy = _import_strategy_class("sloped_break_retest_v3", "SlopedBreakRetestV3Strategy")
SlopedResistanceChochV1Strategy = _import_strategy_class("sloped_resistance_choch_v1", "SlopedResistanceChochV1Strategy")
...
AltSupportBounceV1Strategy = _import_strategy_class("alt_support_bounce_v1", "AltSupportBounceV1Strategy")
AltRangeScalpV1Strategy = _import_strategy_class("alt_range_scalp_v1", "AltRangeScalpV1Strategy")
AltVWAPMeanReversionV1Strategy = _import_strategy_class("alt_vwap_mean_reversion_v1", "AltVWAPMeanReversionV1Strategy")
PumpFadeV2Strategy = _import_strategy_class("pump_fade_v2", "PumpFadeV2Strategy")
PumpFadeV3Strategy = _import_strategy_class("pump_fade_v3", "PumpFadeV3Strategy")
ElderTripleScreenV2Strategy = _import_strategy_class("elder_triple_screen_v2", "ElderTripleScreenV2Strategy")
ElderTripleScreenV3Strategy = _import_strategy_class("elder_triple_screen_v3", "ElderTripleScreenV3Strategy")
...
def _directional_regime_at_bar(store: KlineStore, i: int) -> str:
    ...
    try:
        ...
        if len(c) < 80:
            ...
            ef = _ema(c[-90:], 20)
            es = _ema(c[-140:], 50)
            direction = "BEAR" if math.isfinite(ef) and math.isfinite(es) and ef < es else "BULL"
    except Exception:
        direction = "BULL"
    phase = "CHOP" if base == "flat" else "TREND"
...
def _explicit_strategy_risk_mult(strategy_name: str) -> Optional[float]:
    ...
    env_overrides = {
        ...
        "pump_fade_v3": "PUMP_FADE_RISK_MULT",
        "elder_triple_screen_v2": ("ELDER_RISK_MULT", "ELDER_V2_RISK_MULT"),
        "alt_vwap_mean_reversion_v1": "VWAP_RISK_MULT",
        "alt_liquidity_sweep_reversal_v1": "LQH1_RISK_MULT",
        "alt_liquidity_sweep_reversal_v2": "LQH2_RISK_MULT",
        ...
        for env_key in env_keys:
            ...
            if raw:
                try:
                    ...
                    pass
    if st == "impulse_volume_breakout_v1":
        for env_key in ("IVB1_RISK_MULT", "IMPULSE_RISK_MULT"):
            raw = str(os.getenv(env_key, "") or "").strip()
            if not raw:
                continue
            try:
...
def main():
    ...
    regime_trend_set = _csv_set(
        ...
        "inplay_breakout,btc_eth_midterm_pullback,trend_breakout,trend_pullback,trend_pullback_be_trail,sr_break_retest_volume_v1,sloped_break_retest_v1,sloped_break_retest_v2,sloped_break_retest_v3,btc_eth_trend_follow",
    )
    ...
    sloped_break_retest_v1 = {sym: SlopedBreakRetestV1Strategy() for sym in symbols} if "sloped_break_retest_v1" in strategies else {}
    sloped_break_retest_v2 = {sym: SlopedBreakRetestV2Strategy() for sym in symbols} if "sloped_break_retest_v2" in strategies else {}
    sloped_break_retest_v3 = {sym: SlopedBreakRetestV3Strategy() for sym in symbols} if "sloped_break_retest_v3" in strategies else {}
    sloped_resistance_choch_v1 = {sym: SlopedResistanceChochV1Strategy() for sym in symbols} if "sloped_resistance_choch_v1" in strategies else {}
    ...
    btc_eth_midterm_short_v2 = {sym: BTCETHMidtermShortV2Strategy() for sym in symbols} if "btc_eth_midterm_short_v2" in strategies else {}
    ...
    alt_support_bounce_v1 = {sym: AltSupportBounceV1Strategy() for sym in symbols} if "alt_support_bounce_v1" in strategies else {}
    alt_range_scalp_v1 = {sym: AltRangeScalpV1Strategy() for sym in symbols} if "alt_range_scalp_v1" in strategies else {}
    alt_vwap_mean_reversion_v1 = {sym: AltVWAPMeanReversionV1Strategy() for sym in symbols} if "alt_vwap_mean_reversion_v1" in strategies else {}
    pump_fade_v2 = {sym: PumpFadeV2Strategy() for sym in symbols} if "pump_fade_v2" in strategies else {}
    pump_fade_v3 = {sym: PumpFadeV3Strategy() for sym in symbols} if "pump_fade_v3" in strategies else {}
    elder_triple_screen_v2 = {sym: ElderTripleScreenV2Strategy() for sym in symbols} if "elder_triple_screen_v2" in strategies else {}
    elder_triple_screen_v3 = {sym: ElderTripleScreenV3Strategy() for sym in symbols} if "elder_triple_screen_v3" in strategies else {}
    ...
    alt_horizontal_break_v1 = {sym: AltHorizontalBreakV1Strategy() for sym in symbols} if "alt_horizontal_break_v1" in strategies else {}
    ...
    def selector(sym: str, store: KlineStore, ts_ms: int, last_price: float):
        ...
        for st in strategies:
            ...
            if st == "bounce":
            ...
            elif st == "pump_fade_v4r":
                i = getattr(store, 'i5', getattr(store, 'i', None))
                if i is None:
                    raise AttributeError('KlineStore missing current index (expected i5)')
                bar = store.c5[int(i)]
                sig = pump_fade_v4r[sym].maybe_signal(sym, ts_ms, bar.o, bar.h, bar.l, bar.c, bar.v)
            elif st == "alt_inplay_breakdown_v2":
                i = getattr(store, 'i5', getattr(store, 'i', None))
                if i is None:
                    raise AttributeError('KlineStore missing current index (expected i5)')
                bar = store.c5[int(i)]
                sig = alt_inplay_breakdown_v2[sym].maybe_signal(store, ts_ms, bar.o, bar.h, bar.l, bar.c, bar.v)
            elif st == "alt_support_bounce_v1":
                i = getattr(store, 'i5', getattr(store, 'i', None))
                if i is None:
                    raise AttributeError('KlineStore missing current index (expected i5)')
                bar = store.c5[int(i)]
                sig = alt_support_bounce_v1[sym].maybe_signal(store, ts_ms, bar.o, bar.h, bar.l, bar.c, bar.v)
            elif st == "alt_range_scalp_v1":
                i = getattr(store, 'i5', getattr(store, 'i', None))
                if i is None:
                    raise AttributeError('KlineStore missing current index (expected i5)')
                bar = store.c5[int(i)]
                sig = alt_range_scalp_v1[sym].maybe_signal(store, ts_ms, bar.o, bar.h, bar.l, bar.c, bar.v)
            elif st == "alt_vwap_mean_reversion_v1":
                i = getattr(store, 'i5', getattr(store, 'i', None))
                if i is None:
                    raise AttributeError('KlineStore missing current index (expected i5)')
                bar = store.c5[int(i)]
                sig = alt_vwap_mean_reversion_v1[sym].maybe_signal(store, ts_ms, bar.o, bar.h, bar.l, bar.c, bar.v)
            elif st == "pump_fade_v2":
                i = getattr(store, 'i5', getattr(store, 'i', None))
                if i is None:
                    raise AttributeError('KlineStore missing current index (expected i5)')
                bar = store.c5[int(i)]
                sig = pump_fade_v2[sym].maybe_signal(sym, ts_ms, bar.o, bar.h, bar.l, bar.c, bar.v)
            elif st == "pump_fade_v3":
                i = getattr(store, 'i5', getattr(store, 'i', None))
                if i is None:
                    raise AttributeError('KlineStore missing current index (expected i5)')
                bar = store.c5[int(i)]
                sig = pump_fade_v3[sym].maybe_signal(sym, ts_ms, bar.o, bar.h, bar.l, bar.c, bar.v)
            elif st == "elder_triple_screen_v2":
                i = getattr(store, 'i5', getattr(store, 'i', None))
                if i is None:
                    raise AttributeError('KlineStore missing current index (expected i5)')
                bar = store.c5[int(i)]
                sig = elder_triple_screen_v2[sym].maybe_signal(store, ts_ms, bar.o, bar.h, bar.l, bar.c, bar.v)
            elif st == "elder_triple_screen_v3":
                i = getattr(store, 'i5', getattr(store, 'i', None))
                if i is None:
                    raise AttributeError('KlineStore missing current index (expected i5)')
                bar = store.c5[int(i)]
                sig = elder_triple_screen_v3[sym].maybe_signal(store, ts_ms, bar.o, bar.h, bar.l, bar.c, bar.v)
            elif st == "pump_fade_simple":
                i = getattr(store, 'i5', getattr(store, 'i', None))
                if i is None:
                    raise AttributeError('KlineStore missing current index (expected i5)')
                bar = store.c5[int(i)]
                sig = pump_fade_simple[sym].maybe_signal(sym, ts_ms, bar.o, bar.h, bar.l, bar.c, bar.v)
            elif st == "pump_momentum_v1":
                i = getattr(store, 'i5', getattr(store, 'i', None))
                if i is None:
                    raise AttributeError('KlineStore missing current index (expected i5)')
                bar = store.c5[int(i)]
                sig = pump_momentum_v1[sym].maybe_signal(sym, ts_ms, bar.o, bar.h, bar.l, bar.c, bar.v)
            elif st == "impulse_volume_breakout_v1":
                i = getattr(store, 'i5', getattr(store, 'i', None))
                if i is None:
                    raise AttributeError('KlineStore missing current index (expected i5)')
                bar = store.c5[int(i)]
            ...
            elif st == "sloped_break_retest_v1":
                i = getattr(store, 'i5', getattr(store, 'i', None))
                if i is None:
                    raise AttributeError('KlineStore missing current index (expected i5)')
                bar = store.c5[int(i)]
                sig = sloped_break_retest_v1[sym].maybe_signal(store, ts_ms, bar.o, bar.h, bar.l, bar.c, bar.v)
            elif st == "sloped_break_retest_v2":
                i = getattr(store, 'i5', getattr(store, 'i', None))
                if i is None:
                    raise AttributeError('KlineStore missing current index (expected i5)')
                bar = store.c5[int(i)]
                sig = sloped_break_retest_v2[sym].maybe_signal(store, ts_ms, bar.o, bar.h, bar.l, bar.c, bar.v)
            elif st == "sloped_break_retest_v3":
                i = getattr(store, 'i5', getattr(store, 'i', None))
                if i is None:
                    raise AttributeError('KlineStore missing current index (expected i5)')
                bar = store.c5[int(i)]
                sig = sloped_break_retest_v3[sym].maybe_signal(store, ts_ms, bar.o, bar.h, bar.l, bar.c, bar.v)
            elif st == "sloped_resistance_choch_v1":
                i = getattr(store, 'i5', getattr(store, 'i', None))
                if i is None:
                    raise AttributeError('KlineStore missing current index (expected i5)')
                bar = store.c5[int(i)]
                sig = sloped_resistance_choch_v1[sym].maybe_signal(store, ts_ms, bar.o, bar.h, bar.l, bar.c, bar.v)
            ...
            elif st == "btc_eth_midterm_pullback_v2":
                i = getattr(store, 'i5', getattr(store, 'i', None))
                if i is None:
                    raise AttributeError('KlineStore missing current index (expected i5)')
                bar = store.c5[int(i)]
                sig = btc_eth_midterm_pullback_v2[sym].maybe_signal(store, ts_ms, bar.o, bar.h, bar.l, bar.c, bar.v)
            elif st == "btc_eth_midterm_v3":
                i = getattr(store, 'i5', getattr(store, 'i', None))
                if i is None:
                    raise AttributeError('KlineStore missing current index (expected i5)')
                bar = store.c5[int(i)]
            ...
            elif st == "btc_eth_midterm_short_v2":
                i = getattr(store, 'i5', getattr(store, 'i', None))
                if i is None:
                    raise AttributeError('KlineStore missing current index (expected i5)')
                bar = store.c5[int(i)]
                sig = btc_eth_midterm_short_v2[sym].maybe_signal(store, ts_ms, bar.o, bar.h, bar.l, bar.c, bar.v)
            ...
            elif st == "alt_support_bounce_v2":
                i = getattr(store, 'i5', getattr(store, 'i', None))
                if i is None:
                    raise AttributeError('KlineStore missing current index (expected i5)')
                bar = store.c5[int(i)]
                sig = alt_support_bounce_v2[sym].maybe_signal(store, ts_ms, bar.o, bar.h, bar.l, bar.c, bar.v)
            elif st == "alt_channel_bounce_v1":
                i = getattr(store, 'i5', getattr(store, 'i', None))
                if i is None:
                    raise AttributeError('KlineStore missing current index (expected i5)')
                bar = store.c5[int(i)]
            ...
            elif st == "funding_rate_reversion_v1":
                i = getattr(store, 'i5', getattr(store, 'i', None))
                if i is None:
                    raise AttributeError('KlineStore missing current index (expected i5)')
                bar = store.c5[int(i)]
                sig = funding_rate_reversion_v1[sym].maybe_signal(store, ts_ms, bar.o, bar.h, bar.l, bar.c, bar.v)
            elif st == "liquidation_cascade_entry_v1":
                i = getattr(store, 'i5', getattr(store, 'i', None))
                if i is None:
                    raise AttributeError('KlineStore missing current index (expected i5)')
                bar = store.c5[int(i)]
                sig = liquidation_cascade_entry_v1[sym].maybe_signal(store, ts_ms, bar.o, bar.h, bar.l, bar.c, bar.v)
            elif st == "alt_liquidity_sweep_reversal_v1":
                i = getattr(store, 'i5', getattr(store, 'i', None))
                if i is None:
                    raise AttributeError('KlineStore missing current index (expected i5)')
                bar = store.c5[int(i)]
                sig = alt_liquidity_sweep_reversal_v1[sym].maybe_signal(store, ts_ms, bar.o, bar.h, bar.l, bar.c, bar.v)
            elif st == "alt_liquidity_sweep_reversal_v2":
                i = getattr(store, 'i5', getattr(store, 'i', None))
                if i is None:
                    raise AttributeError('KlineStore missing current index (expected i5)')
                try:
                    regime_hint = _directional_regime_at_bar(store, int(i))
                except Exception:
                    regime_hint = None
                sig = alt_liquidity_sweep_reversal_v2[sym].signal(store, sym, int(i), regime=regime_hint)
            elif st == "alt_spike_rejection_v1":
                i = getattr(store, 'i5', getattr(store, 'i', None))
                if i is None:
                    raise AttributeError('KlineStore missing current index (expected i5)')
                try:
                ...
                try:
                    ...
                    regime_hint = None
                sig = alt_bear_regime_continuation_v1[sym].signal(store, sym, int(i), regime=regime_hint)
            elif st == "alt_whale_print_follow_v1":
                i = getattr(store, 'i5', getattr(store, 'i', None))
                if i is None:
                    raise AttributeError('KlineStore missing current index (expected i5)')
                try:
                    ...
                    regime_hint = None
                sig = alt_whale_print_follow_v1[sym].signal(store, sym, int(i), regime=regime_hint)
            elif st == "alt_trendline_touch_v1":
                i = getattr(store, 'i5', getattr(store, 'i', None))
                if i is None:
                    raise AttributeError('KlineStore missing current index (expected i5)')
                bar = store.c5[int(i)]
                ...
                if i is None:
                    raise AttributeError('KlineStore missing current index (expected i5)')
                bar = store.c5[int(i)]
                sig = alt_sloped_momentum_v1[sym].maybe_signal(store, ts_ms, bar.o, bar.h, bar.l, bar.c, bar.v)
            elif st == "alt_volume_spike_momentum_v1":
                i = getattr(store, 'i5', getattr(store, 'i', None))
                if i is None:
                    raise AttributeError('KlineStore missing current index (expected i5)')
                bar = store.c5[int(i)]
                sig = alt_volume_spike_momentum_v1[sym].maybe_signal(store, ts_ms, bar.o, bar.h, bar.l, bar.c, bar.v)
            elif st == "alt_slope_break_v1":
                i = getattr(store, 'i5', getattr(store, 'i', None))
                if i is None:
                    raise AttributeError('KlineStore missing current index (expected i5)')
                bar = store.c5[int(i)]
                sig = alt_slope_break_v1[sym].maybe_signal(store, ts_ms, bar.o, bar.h, bar.l, bar.c, bar.v)
            elif st == "alt_horizontal_break_v1":
                i = getattr(store, 'i5', getattr(store, 'i', None))
                if i is None:
                    raise AttributeError('KlineStore missing current index (expected i5)')
                bar = store.c5[int(i)]
                sig = alt_horizontal_break_v1[sym].maybe_signal(store, ts_ms, bar.o, bar.h, bar.l, bar.c, bar.v)
            else:
                sig = None
            if sig is not None:
