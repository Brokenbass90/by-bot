...
class KlineStore:
    ...
    def __init__(self, symbol: str, candles_5m: List[Candle], base_interval_min: int = 5):
        ...
        self.c1w = aggregate_candles_to_interval(candles_5m, 10080)
        self._interval_map: Dict[str, Tuple[int, List[Candle], List[int]]] = {
            ...
            "10080": (10080, self.c1w, [int(c.ts) for c in self.c1w]),
        }
    ...
    def _slice(self, interval: str, limit: int) -> List[Candle]:
        ...
        if meta is None:
            raise ValueError(f"Unsupported interval {interval}; expected 1/3/5/15/60/240/1440/10080")
