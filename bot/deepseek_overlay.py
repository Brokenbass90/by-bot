...
class DeepSeekOverlay:
    ...
    def ask(self, question: str, snapshot: dict[str, Any]) -> str:
        ...
        system_prompt = (
            ...
            "6. micro_scalper_v1 — скальпинг 5m, сессия UTC 7-17. ⚠️ ДОБАВЛЕНА, ждёт autoresearch.\n"
            "   По умолчанию ВЫКЛ (ENABLE_MICRO_SCALPER_TRADING=0). Включить после autoresearch.\n"
            "   Монеты: BTC, ETH, SOL (MICRO_SCALPER_SYMBOL_ALLOWLIST). Потенциал: +50-100 сделок/мес.\n\n"
            "7. alt_support_reclaim_v1 (ASR1) — LONG от зон поддержки. ⚠️ ДОБАВЛЕНА, ждёт autoresearch.\n"
            "   Зеркало ARF1: там SHORT от сопротивления → здесь LONG от поддержки.\n"
            "   По умолчанию ВЫКЛ (ENABLE_SUPPORT_RECLAIM_TRADING=0). Включить после autoresearch.\n"
            "   Монеты: BTC, ETH, SOL, LINK (SUPPORT_RECLAIM_SYMBOL_ALLOWLIST).\n\n"
