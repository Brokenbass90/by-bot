"""Live wrapper for AltTrendlineTouchV1Strategy (ATT1).
...
class _ATT1Store:
    """Minimal store adapter that AltTrendlineTouchV1Strategy expects."""
...
class ATT1LiveEngine:
    """Per-symbol AltTrendlineTouchV1Strategy instances for live trading.

    Usage in live bot::

        engine = ATT1LiveEngine(fetch_klines_func)
    ...
    def __init__(self, fetch_klines):
        self._fetch = fetch_klines
        self._stores: Dict[str, _ATT1Store] = {}
        self._strategies: Dict[str, AltTrendlineTouchV1Strategy] = {}
        self._no_signal_reasons: Dict[str, str] = {}

    def _get_store(self, symbol: str) -> _ATT1Store:
        if symbol not in self._stores:
            self._stores[symbol] = _ATT1Store(symbol, self._fetch)
        return self._stores[symbol]

    def _get_strategy(self, symbol: str) -> AltTrendlineTouchV1Strategy:
        if symbol not in self._strategies:
            self._strategies[symbol] = AltTrendlineTouchV1Strategy()
