...
OUT="backtest_runs/srv1_suite_${TS}"
mkdir -p "$OUT"
REP="$OUT/report.txt"

SYMS="${SRV1_SYMBOLS:-BTCUSDT,ETHUSDT,SOLUSDT,ADAUSDT,LINKUSDT,DOGEUSDT,LTCUSDT,BCHUSDT,ATOMUSDT,AVAXUSDT}"
DAYS="${SRV1_DAYS:-180}"
END="${SRV1_END_DATE:-2026-03-01}"
RISK="${SRV1_RISK_PCT:-0.005}"
LEV="${SRV1_LEVERAGE:-3}"
MAXPOS="${SRV1_MAX_POSITIONS:-3}"
...
run_one() {
  ...
  python3 backtest/run_portfolio.py \
    --symbols "$SYMS" \
    --strategies sr_break_retest_volume_v1 \
    --days "$DAYS" --end "$END" \
    --tag "$tag" \
...
echo "srv1 suite start: $(date -u '+%Y-%m-%d %H:%M:%S UTC')" | tee "$REP"
echo "symbols=$SYMS days=$DAYS end=$END" | tee -a "$REP"

run_one "srv1_base_${DAYS}d" 6 2
run_one "srv1_stress_${DAYS}d" 10 10

echo "\nsrv1 suite done: $(date -u '+%Y-%m-%d %H:%M:%S UTC')" | tee -a "$REP"
