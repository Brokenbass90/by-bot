...
ENV_PATH = ROOT / ".env"
APPROVED_ENV_PATH = ROOT / "configs" / "approved_strategy_params.env"
...
def _write_disabled_allocator_snapshot(
    *,
    base_env: Dict[str, str],
    approved_env_loaded: bool,
    policy: Dict[str, Any],
    health: Dict[str, Any],
    now: datetime,
    out_env: Path,
    ...
    """Write an honest operator snapshot when allocator overlay is intentionally off."""
    ...
    global_mult = max(
        0.0,
        _safe_float(
            base_env.get("ALLOCATOR_GLOBAL_RISK_MULT")
            or base_env.get("ORCH_GLOBAL_RISK_MULT")
            or base_env.get("GLOBAL_RISK_MULT"),
            1.0,
        ),
    )
    ...
    lines = [
        ...
        "# Allocator overlay is intentionally disabled; approved/base env is the source of truth.",
        ...
        "PORTFOLIO_ALLOCATOR_PROFILE_VERSION=approved_env",
        f"PORTFOLIO_ALLOCATOR_GENERATED_AT_UTC={generated_at}",
        f"PORTFOLIO_ALLOCATOR_REGIME={base_env.get('PORTFOLIO_ALLOCATOR_REGIME') or 'env_controlled'}",
        f"PORTFOLIO_ALLOCATOR_OVERALL_HEALTH={overall_health}",
        "ALLOCATOR_MODE=disabled",
        "ALLOCATOR_EFFECTIVE_MODE=approved_env",
        "ALLOCATOR_HAIRCUT_STRENGTH=0.0000",
    ...
    enabled_symbol_sets: Dict[str, set[str]] = {}
    issues: List[str] = ["allocator_overlay_disabled"]
    if approved_env_loaded:
        issues.append("using_approved_strategy_params_env")
    ...
    for sleeve in list(policy.get("sleeves") or []):
        ...
        enabled = _env_enabled(base_env, enable_env, False) if enable_env else False
        risk_mult = max(0.0, _safe_float(base_env.get(risk_env), 0.0)) if enabled else 0.0
        symbols = _csv_symbols(base_env.get(symbol_env_key, "")) if symbol_env_key else []
        health_status, health_notes = _sleeve_health_status(
            sleeve,
            health_map,
            missing_status=missing_health_status,
        )
        notes = ["approved_env_control"] if approved_env_loaded else ["base_env_control"]
        ...
        sleeve_states[name] = {
            ...
            "enabled": bool(enabled and risk_mult > 0.0),
            "symbol_count": len(symbols),
            "symbols": symbols,
            "health_status": health_status,
            "base_risk_mult": risk_mult,
            "count_mult": 1.0,
            "health_mult": 1.0,
            "raw_health_mult": 1.0,
            "final_risk_mult": risk_mult if enabled else 0.0,
            "overlap_mult": 1.0,
        ...
        if enabled and risk_mult > 0.0:
            enabled_symbol_sets[name] = set(symbols)

        lines.extend(
            [
                f"{enable_env}={_bool_to_env(enabled)}",
                f"{risk_env}={risk_mult:.4f}",
                f"ALLOCATOR_STATUS_{name.upper()}={health_status}",
                f"ALLOCATOR_COUNT_{name.upper()}={len(symbols)}",
                f"ALLOCATOR_OVERLAP_{name.upper()}=0.0000",
                "",
            ]
        )
    ...
    health_summary = {
        ...
        "active_watch_sleeves": [
            name
            for name, state in sleeve_states.items()
            if bool(state.get("enabled")) and str(state.get("health_status") or "OK").upper() == "WATCH"
        ],
        "missing_strategy_health_status": missing_health_status,
        "active_status_counts": {
            "OK": sum(1 for state in sleeve_states.values() if bool(state.get("enabled")) and state.get("health_status") == "OK"),
            "WATCH": sum(1 for state in sleeve_states.values() if bool(state.get("enabled")) and state.get("health_status") == "WATCH"),
            "PAUSE": sum(1 for state in sleeve_states.values() if bool(state.get("enabled")) and state.get("health_status") == "PAUSE"),
            "KILL": sum(1 for state in sleeve_states.values() if bool(state.get("enabled")) and state.get("health_status") == "KILL"),
        },
    }

    lines.extend(
        [
            "PORTFOLIO_ALLOCATOR_STATUS=disabled",
            "PORTFOLIO_ALLOCATOR_SAFE_MODE=0",
            "PORTFOLIO_ALLOCATOR_DEGRADED=0",
            f"ALLOCATOR_GLOBAL_RISK_MULT={global_mult:.4f}",
            "ALLOCATOR_PORTFOLIO_OVERLAP_RATIO=0.0000",
            "ALLOCATOR_PORTFOLIO_OVERLAP_MULT=1.0000",
            "ALLOCATOR_PORTFOLIO_OVERLAP_MULT_RAW=1.0000",
            "ALLOCATOR_HARD_BLOCK_NEW_ENTRIES=0",
            "ALLOCATOR_SAFE_MODE_REASON=\"\"",
            "",
        ]
    )
    ...
    state_obj = {
        ...
        "profile_version": "approved_env",
        "timestamp_utc": generated_at,
        "allocator_mode": "disabled",
        "allocator_effective_mode": "approved_env",
        "haircut_strength": 0.0,
        ...
        "regime": base_env.get("PORTFOLIO_ALLOCATOR_REGIME") or "env_controlled",
        "overall_health": overall_health,
        "allocator_global_risk_mult": global_mult,
        "base_global_risk_mult": global_mult,
        "portfolio_overlap_ratio": 0.0,
        "portfolio_overlap_mult": 1.0,
        "raw_portfolio_overlap_mult": 1.0,
        "degraded_reasons": [],
        "safe_mode_reasons": [],
        ...
        "paths": {
            ...
            "approved_env_file": str(APPROVED_ENV_PATH) if approved_env_loaded else "",
        ...
        "exposure": {
            ...
            "portfolio_overlap_mult": 1.0,
            "raw_portfolio_overlap_mult": 1.0,
            "portfolio_overlap_tiers": [],
            "sleeve_overlap_tiers": [],
        },
    ...
    print(
        "Allocator status: disabled | source=approved_env | "
        f"global_risk={global_mult:.2f} | hard_block=0"
    )
...
def main() -> int:
    ...
    approved_env = _parse_env(APPROVED_ENV_PATH)
    if approved_env:
        base_env.update(approved_env)
    ...
    allocator_overlay_enabled = _env_enabled(base_env, "PORTFOLIO_ALLOCATOR_ENABLE", True)
    if not allocator_overlay_enabled:
        return _write_disabled_allocator_snapshot(
            base_env=base_env,
            approved_env_loaded=bool(approved_env),
            policy=policy,
            health=health,
            now=now,
            out_env=out_env,
