#!/usr/bin/env bash
# =============================================================================
#  run_midterm_short_v1_backtests.sh
#  Annual + WF validation runner for btc_eth_midterm_short_v1.
#
#  Run on server:
#    bash scripts/run_midterm_short_v1_backtests.sh
#
#  Optional env:
#    TAG_BASE=midterm_short_v1_fix1440_20260417
...
TAG_BASE="${TAG_BASE:-midterm_short_v1_$(date +%Y%m%d)}"
...
"$PYTHON_BIN" scripts/run_generic_wf.py \
  --strategy btc_eth_midterm_short_v1 \
  --symbols "$SYMBOLS" \
  --tag "$WF_TAG" \
  --end "$END" \
