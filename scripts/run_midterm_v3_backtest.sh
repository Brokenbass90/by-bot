...
TAG="${TAG:-midterm_v3_btceth_$(date +%Y%m%d)}"
SYMBOLS="${SYMBOLS:-BTCUSDT,ETHUSDT}"
DAYS="${DAYS:-365}"
END="${END:-2026-03-31}"
RUN_V1_COMPARE="${RUN_V1_COMPARE:-1}"
RUN_BEAR_STACK="${RUN_BEAR_STACK:-1}"
...
if [ "$RUN_V1_COMPARE" = "1" ]; then
  ...
  echo "▶ Test 3 skipped (RUN_V1_COMPARE=0)"
fi
...
if [ "$RUN_BEAR_STACK" = "1" ]; then
  ...
  echo "▶ Test 4 skipped (RUN_BEAR_STACK=0)"
fi
