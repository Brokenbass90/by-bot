...
def main() -> int:
    ap = argparse.ArgumentParser(description="Check Forex CSV readiness for pilot")
    ap.add_argument("--pairs", default="EURUSD,GBPUSD,USDJPY")
    ap.add_argument("--data-dir", default="data_cache/forex")
    ap.add_argument("--out", default="docs/forex_data_status.csv")
    args = ap.parse_args()
    ...
    print(f"pairs={len(rows)} ready={ready}")
    for x in rows:
        print(
            f"{x['pair']}: exists={x['exists']} rows={x['rows']} "
