...
cd "$ROOT" || exit 2
"$ROOT/.venv/bin/python" "$ROOT/scripts/run_event_universe_v1.py" status --run-root "$RUN_ROOT" >/dev/null || exit 2
echo "research-only event-universe supervisor start: $(date -u +%Y-%m-%dT%H:%M:%SZ)"

while [ "$attempt" -le "$MAX_PROCESS_ATTEMPTS" ]; do
  args=(
    "$ROOT/.venv/bin/python"
    "$ROOT/scripts/run_event_universe_v1.py"
    run
    --run-root "$RUN_ROOT"
