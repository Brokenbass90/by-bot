...
run_profile() {
  ...
  TOP_N="$TOP_N" \
  bash scripts/run_live_parity_backtest.sh >/tmp/live_parity_matrix_"${profile}".log 2>&1
...
cp "$TMP_CSV" docs/live_parity_matrix_latest.csv
...
echo "latest=docs/live_parity_matrix_latest.csv"
