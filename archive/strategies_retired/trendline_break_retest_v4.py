...
@dataclass
class TrendlineBreakRetestV4Config:
...
class TrendlineBreakRetestV4Strategy:
    """Trendline break-retest with trend/volatility guards (v4)."""

    def __init__(self, cfg: Optional[TrendlineBreakRetestV4Config] = None):
        self.cfg = cfg or TrendlineBreakRetestV4Config()
        self.cfg.lookback_bars = _env_int("TLB4_LOOKBACK_BARS", self.cfg.lookback_bars)
        self.cfg.swing_n = _env_int("TLB4_SWING_N", self.cfg.swing_n)
        self.cfg.min_touches = _env_int("TLB4_MIN_TOUCHES", self.cfg.min_touches)
        self.cfg.touch_tol_atr = _env_float("TLB4_TOUCH_TOL_ATR", self.cfg.touch_tol_atr)
        self.cfg.atr_period = _env_int("TLB4_ATR_PERIOD", self.cfg.atr_period)
        self.cfg.consolidation_bars = _env_int("TLB4_CONS_BARS", self.cfg.consolidation_bars)
        self.cfg.max_cons_range_atr = _env_float("TLB4_MAX_CONS_RANGE_ATR", self.cfg.max_cons_range_atr)
        self.cfg.breakout_atr_mult = _env_float("TLB4_BREAKOUT_ATR_MULT", self.cfg.breakout_atr_mult)
        self.cfg.min_breakout_ext_atr = _env_float("TLB4_MIN_BREAKOUT_EXT_ATR", self.cfg.min_breakout_ext_atr)
        self.cfg.retest_window_bars = _env_int("TLB4_RETEST_WINDOW_BARS", self.cfg.retest_window_bars)
        self.cfg.retest_pullback_atr = _env_float("TLB4_RETEST_PULLBACK_ATR", self.cfg.retest_pullback_atr)
        self.cfg.retest_min_body_frac = _env_float("TLB4_RETEST_MIN_BODY_FRAC", self.cfg.retest_min_body_frac)
        self.cfg.bos_lookback_bars = _env_int("TLB4_BOS_LOOKBACK_BARS", self.cfg.bos_lookback_bars)
        self.cfg.bos_atr_mult = _env_float("TLB4_BOS_ATR_MULT", self.cfg.bos_atr_mult)
        self.cfg.vol_sma_bars = _env_int("TLB4_VOL_SMA_BARS", self.cfg.vol_sma_bars)
        self.cfg.vol_breakout_mult = _env_float("TLB4_VOL_BREAKOUT_MULT", self.cfg.vol_breakout_mult)
        self.cfg.trend_ema_fast = _env_int("TLB4_TREND_EMA_FAST", self.cfg.trend_ema_fast)
        self.cfg.trend_ema_slow = _env_int("TLB4_TREND_EMA_SLOW", self.cfg.trend_ema_slow)
        self.cfg.trend_min_gap_pct = _env_float("TLB4_TREND_MIN_GAP_PCT", self.cfg.trend_min_gap_pct)
        self.cfg.trend_min_slope_pct = _env_float("TLB4_TREND_MIN_SLOPE_PCT", self.cfg.trend_min_slope_pct)
        self.cfg.min_atr_pct = _env_float("TLB4_MIN_ATR_PCT", self.cfg.min_atr_pct)
        self.cfg.max_atr_pct = _env_float("TLB4_MAX_ATR_PCT", self.cfg.max_atr_pct)
        self.cfg.sl_atr_mult = _env_float("TLB4_SL_ATR_MULT", self.cfg.sl_atr_mult)
    ...
    def _trend_bias(self) -> int:
        ...
        need = max(self.cfg.trend_ema_slow + 8, 240)
        if len(self._c) < need:
            return 1
        sampled = self._c[::12]  # 5m -> 1h proxy
        if len(sampled) < self.cfg.trend_ema_slow + 6:
            return 1
        ...
        if not (math.isfinite(ef) and math.isfinite(es) and math.isfinite(es_prev)):
            return 1
        px = max(1e-12, sampled[-1])
        gap = abs(ef - es) / px * 100.0
        slope = (es - es_prev) / max(1e-12, abs(es_prev)) * 100.0
        if gap < self.cfg.trend_min_gap_pct:
            return 1
        if ef > es and slope >= self.cfg.trend_min_slope_pct:
            return 2
        if ef < es and slope <= -self.cfg.trend_min_slope_pct:
            return 0
        return 1
            ...
            if idx > int(pd["expire_idx"]):
                ...
                if touched and rejected and trend == 2:
                    ...
                    if risk > 0:
                        ...
                        self._day_signals += 1
                        return TradeSignal(
                            strategy="trendline_break_retest_v4",
                            symbol=getattr(store, "symbol", ""),
                            side="long",
                            entry=entry,
                            sl=sl,
                            tp=entry + self.cfg.rr * risk,
                            reason=f"tlb4_long t={int(pd['touches'])}",
                        )
                ...
                if touched and rejected and trend == 0:
                    ...
                    if risk > 0:
                        ...
                        return TradeSignal(
                            ...
                            sl=sl,
                            tp=entry - self.cfg.rr * risk,
                            reason=f"tlb4_short t={int(pd['touches'])}",
                        )
