...
class LiveKlineStore:
    """Minimal store adapter for InPlayBreakoutWrapper in live mode."""
...
class BreakoutLiveEngine:
    """Creates per-symbol InPlayBreakoutWrapper instances and returns TradeSignal."""

    def __init__(self, fetch_klines: Callable[[str, str, int], Any]):
        self._fetch = fetch_klines
        self._stores: Dict[str, LiveKlineStore] = {}
        self._wrappers: Dict[str, InPlayBreakoutWrapper] = {}
    ...
    def signal(self, symbol: str, price: float, ts_ms: int) -> Optional[TradeSignal]:
        ...
        if symbol not in self._wrappers:
            self._wrappers[symbol] = InPlayBreakoutWrapper()
