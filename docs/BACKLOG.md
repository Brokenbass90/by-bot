...
|---|---|---|---|---|
| P0-NEW | **Crypto strict live-vs-static_v1 parity.** Fixed diagnostic returns `FAIL`: live adds `sloped,asm1`, expands static symbols, and falls back to `MAX_POSITIONS=1`. ARF1 full-package winner replay is confirmed: five slots `+77.57% / PF 1.646 / DD 5.16% / 2` red months; four slots `+76.55% / PF 1.636 / DD 5.00% / 3` red months. Filters still explain current zero entries; drift would corrupt first fills. | Codex | `scripts/compare_static_v1_live_parity_inputs.py`, `scripts/compare_live_pulse_vs_backtest.py`, `scripts/weekly_live_vs_backtest_report.py` | Prepare reviewed four-sleeve/fixed-symbol five-slot shadow policy; strict `PASS` plus capped aggregate risk before proposing a live change |
...
| P1-1h | **Full-package internal-filter sweep. Done/replay passed.** `package_breakdown_rsi_v1` best accepted `+74.43%`, PF `1.596`, DD `5.16%`, 1 red month. `package_arf1_flat_touch_v1` finished `48/48`; winner `r002` replay at five slots confirmed `+77.57%`, PF `1.646`, DD `5.16%`, 419 trades, 2 red months. Four-slot replay gives `+76.55%`, PF `1.636`, DD `5.00%`, but 3 red months. | Codex | No blind promotion: prepare strict fixed-symbol/four-sleeve five-slot shadow policy and total-risk cap review |
