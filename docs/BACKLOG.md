...
|---|---|---|---|---|
| P0-NEW | **Crypto decision parity after input parity.** 2026-05-25 server check: static-v1/live sleeves and required symbols now match на `100%`, allocator `safe_mode=False`, но live всё ещё даёт `0` entries. Сопоставить backtest entry timestamps с точными live no-signal filters и чинить только подтверждённый internal blocker. | Codex | `scripts/compare_static_v1_live_parity_inputs.py`, `scripts/compare_live_pulse_vs_backtest.py`, `scripts/weekly_live_vs_backtest_report.py` | На стабильном post-fix окне объяснены все missed static-v1 entries; затем live/test повторяет ≥80% eligible decisions до promotion новых sleeves |
...
| P0.5-4 | Running 2026-05-25: local `att1_density_v3_more_pivots_v1` sweep reached at least `535/864`; best observed remains interim `r421` `+38.37%`, PF `1.376`, DD `3.99%` (not final). Server `bear_regime_continuation_v1_initial_sweep` is active against the currently deployed baseline implementation. ATT1 must pass full-package replay before promotion. | Codex | `logs/att1_density_v3_more_pivots_20260525.nohup.log`, `runtime/research_nightly/` |
| P0.5-9 | Prepared, not yet completed: Elder Revived diagnostic on 360d/6 symbols with 6bps fee + 2bps slippage. Claude drafts for Elder, ARF1, BRC1 indicators and the MTPB v1 runner default are local challengers only: not deployed and not valid replacements until baseline/challenger + full-package replay. `att1_short_slope_v1` (18 implemented combos) is queued in detached local research to start after ATT1 v3 completes. | Codex | `docs/CODEX_HANDOFF_20260525.md`, `configs/autoresearch/`, `logs/att1_short_slope_after_v3_20260525.nohup.log` |
...
| P1-8a | **Alpaca market data freshness fix.** Root cause confirmed 2026-05-25: Alpaca bars request without `start` returned zero recent bars; v3 shadow remained on `2026-05-18 19:30 UTC`. Patch adds bounded recent-window API requests for M5 and daily/SPY data; keep v3 DRY_RUN. | Codex | `scripts/equities_alpaca_intraday_bridge.py` | Server dry-run sees last available market bar `2026-05-22` before US open on 2026-05-26 |
| P1-8b | Fixed/observe 2026-05-25: `v3_shadow` defaults to dry-run; monthly bridge protects intraday `pending_close_positions` after a real conflict on `META/PANW`. Remaining blocker before deposit: intraday ledger currently books estimated P&L at close submission before fill (`PANW/META` accepted on US holiday, still open). Record realized P&L only after confirmed fill and observe one complete close cycle. | Codex | `scripts/equities_alpaca_paper_bridge.py`, `scripts/equities_alpaca_intraday_bridge.py`, `logs/alpaca_intraday_dynamic_v1.log` |
