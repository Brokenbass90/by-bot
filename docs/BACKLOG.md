...
| P1-1h | **Full-package internal-filter sweep.** Running locally, not on VPS: `package_breakdown_rsi_v1` (45 rows) then `package_arf1_flat_touch_v1` (48 rows) in `screen` session `crypto_package_sweeps_20260526` with `caffeinate`. First breakdown candidate failed (`+69.34%`, PF `1.528`, DD `5.38%`). | Codex | Promote no live changes unless a full-package row beats server baseline and preserves red-month/DD gates |
