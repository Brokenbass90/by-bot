...
| P1-1g | **Server/git static-v1 reconciliation.** Server rerun on 2026-05-26 gives `+73.96%`, PF `1.591`, DD `5.16%`, 436 trades vs local `+70.17%`, PF `1.545`, DD `6.23%`, 445 trades. Server has modified/untracked research/runtime strategy files; inventory the exact diff and choose one canonical baseline before any crypto live promotion. | Codex | Same code revision reproduces the same annual benchmark locally and on server; no blind clean redeploy |
...
- 2026-05-26: Deployed Setup Scanner candlestick chart and honest read-only monitor. Exact static-v1 server rerun produced `+73.96%`, PF `1.591`, DD `5.16%`, 436 trades, exposing server/git drift versus the local `+70.17%` result; package sweep gates now use the stronger server benchmark and reconciliation is required before live promotion.
