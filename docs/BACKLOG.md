...
| P0.5-4 | Done 2026-05-26: local `att1_density_v3_more_pivots_v1` finished `864/864`; best `r259` `+38.48%`, PF `1.384`, DD `3.97%`, WR `59.6%`. Next step is full-package replay versus `crypto_income_static_v1`, not live deployment. | Codex | `logs/att1_density_v3_more_pivots_20260525.nohup.log`, `runtime/research_nightly/` |
...
## Изменения в backlog
- 2026-05-26: Deployed Alpaca reporting ownership truth: daily digest/web/AI no longer present `GOOGL` monthly ownership or `META/PANW` pending close fills as clean intraday performance. ATT1 density sweep finished with replay candidate `r259`; ATT1 slope failed; Alpaca v39 remains research-only after bear stress failure.
