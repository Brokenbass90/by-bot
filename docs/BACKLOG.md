...
| P0.5-9 | Prepared, not yet completed: Elder Revived diagnostic on 360d/6 symbols with 6bps fee + 2bps slippage. Claude draft first failed runner contract (`params` missing); compatibility was patched locally for research only, then long run deferred so ATT1 keeps compute priority. `att1_short_slope_v1` (18 implemented combos) and `arf1_filter_v1` (36 combos) are queued research-only configs. | Codex | `docs/CODEX_HANDOFF_20260525.md`, `configs/autoresearch/` |
