...
| P0-12 | **Alpaca monthly software trailing arming.** Fixed/deployed to paper bridge 2026-05-26 in `89c6f8d`: fractional `GOOGL` reached HWM `+6.40%`, then gave back `5.78%`, but old logic did not trail-close because current gain had already fallen below activation threshold. Trail now arms from recorded peak gain and reports `peak_gain_pct`. | Codex | `scripts/equities_alpaca_paper_bridge.py`, `tests/test_alpaca_monthly_trailing.py` | Next US-session paper tick shows `trail_stop_close` or correctly updated HWM; confirm broker fill and re-entry block before deposit |
...
| P1-1h | **Full-package internal-filter sweep.** Running locally, not on VPS: `package_breakdown_rsi_v1` (45 rows) then `package_arf1_flat_touch_v1` (48 rows) in `screen` session `crypto_package_sweeps_20260526` with `caffeinate`. At `29/45`, small interim PASS results exist: best accepted net `+74.43%`, PF `1.596`, DD `5.16%`; higher net rows currently fail negative-month gate. | Codex | Promote no live changes unless all rows finish, a full-package winner beats server baseline and repeated replay preserves red-month/DD gates |
...
| P1-8f | **Alpaca profit-lock research using OHLC/high-water.** Close-only v40 draft is rejected (`24m -2.07%`, PF `1.006`; bear-2022 `-15.34%`, PF `0.413`) and cannot answer the observed `GOOGL` peak giveback. Build a research-only OHLC/high-water trailing and SPY/regime-gated v39 overlay; model conservative stop-first ordering and fees/slippage. | Codex | `strategies/alpaca_dynamic_v3_event.py`, `scripts/alpaca_v3_event_backtest.py`; do not paper-promote from close-only output |
...
- 2026-05-26: Fixed and deployed Alpaca monthly software-trailing arming bug (`89c6f8d`). Saved paper evidence showed `GOOGL` peak `+6.40%` followed by `5.78%` giveback without a close; the bridge now arms from HWM peak, with a unit regression test. Added OHLC/high-water v39 research requirement; Claude v40 close-only draft is rejected.
