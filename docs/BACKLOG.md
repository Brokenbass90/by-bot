...
| P1-8b | **Prepared for deploy/observe 2026-05-25:** `v3_shadow` defaults to dry-run; monthly bridge protects pending intraday closes. New bridge patch keeps manager-close positions as `close_pending`, avoids repeat closes, and books realized P&L only after a filled exit is found. Today's pre-patch `PANW/META` accepted orders still require manual fill reconciliation and ledger cleanup. | Codex | `scripts/equities_alpaca_paper_bridge.py`, `scripts/equities_alpaca_intraday_bridge.py`, `logs/alpaca_intraday_dynamic_v1.log` | Deploy in paper only; observe one complete post-patch filled close before deposit |
