...
|---|---|---|---|---|
| P0-NEW | **Crypto strict live-vs-static_v1 parity.** Fixed diagnostic returns `FAIL`: live adds `sloped,asm1`, expands static symbols, and falls back to `MAX_POSITIONS=1`. Fixed-cache sensitivity completed: `1` slot `+30.76% / PF 1.330`; `3` `+62.79% / PF 1.478`; `4` `+74.41% / PF 1.600` but 3 red months; `5` baseline `+73.96% / PF 1.591` with 2 red months. Filters still explain current zero entries; drift would corrupt first fills. | Codex | `scripts/compare_static_v1_live_parity_inputs.py`, `scripts/compare_live_pulse_vs_backtest.py`, `scripts/weekly_live_vs_backtest_report.py` | Replay final ARF winner at `4/5` slots; reviewed four-sleeve/fixed-symbol policy has strict `PASS`; only then propose a tested live configuration with capped total risk |
...
- 2026-05-26: Deployed strict parity diagnostic (`e339ee2`). It corrected a false `PASS`: static-v1 required inputs are covered, but current live has extra sleeves `sloped,asm1`, extra router symbols, and one-slot default concurrency. Slot sensitivity quantified the damage: `+30.76% / PF 1.330` at one slot versus `+73.96% / PF 1.591` at five; four slots recover return but add a red month. No trade settings changed.
