...
- First fixed-sweep verdict:
  ...
  - additivity:
    - `ATT1+BRC1` 180d: `+43.71`, PF `1.767`, DD `2.81`, `0` red months
    - `ATT1+BRC1` 365d: `+48.78`, PF `1.387`, DD `7.03`, `4` red months
...
## 2026-04-16 (ночь) — Годовой анализ + Deploy Script

### Итоги годового бэктеста (май 2025 → апрель 2026)

Полный анализ: `docs/ANNUAL_ANALYSIS_20260416.md`

| Показатель | Значение |
|-----------|---------|
| Итоговая доходность | **+44.51%/год** (на $100 базы) |
| Зелёных месяцев | 10 из 12 |
| Красных месяцев | 2 (сентябрь −5.8%, декабрь −15.9%) |
| Общих сделок | 769 |

**Рейтинг стратегий:**

| Стратегия | P&L | PF | Сделок | Вердикт |
|-----------|-----|-----|--------|---------|
| ASB1 | +18.6% | 1.397 | 43 | 🟢 СИЛЬНАЯ |
| HZBO1 | +12.0% | 1.486 | 62 | 🟢 СИЛЬНАЯ |
| Elder v2 | +6.9% | 1.098 | 250 | 🟡 СЛАБАЯ (много сделок, 6 красных мес.) |
| ATT1 | +5.6% | 1.225 | 135 | 🟡 НОРМАЛЬНАЯ |
| IVB1 | −2.5% | 0.388 | 11 | 🔴 УБЫТОЧНАЯ (отключить) |

**Ключевые выводы:**
- Декабрь 2025 (BTC ATH $108k): −15.9% — критический провал, оркестратор бы его предотвратил (~0% вместо −15.9%)
- С оркестратором: ~+60%/год вместо +44.51%
- IVB1 убыточен даже после macro filter: avg_win $0.39 < avg_loss $0.58 — фундаментальная проблема TP/SL

### Фиксы выполнены

| # | Fix | Файл | Статус |
|---|-----|------|--------|
| 1 | IVB1 отключить | `configs/core3_live_canary_20260411_sloped_momentum.env` | ✅ в deploy script |
| 2 | Elder risk 0.60→0.40 | `configs/core3_live_canary_20260411_sloped_momentum.env` | ✅ в deploy script |
| 3 | DEGRADED allocator | `touch + rebuild` | ✅ в deploy script |
| 4 | Запустить оркестратор | `nohup + --env-out` | ✅ в deploy script |

### Deploy Script

**Файл:** `scripts/codex_deploy_20260416.sh`
...
### 2026-04-17 Evening Checkpoint

- Crypto live-core now has a first honest stitched annual baseline: `current_crypto_livecore_20260417` finished at `103.44`, PF `1.1318`, DD `5.5863`, `120` trades over `12` monthly windows with allocator `ok` throughout.
- `ATT1-only` finished at `101.33`, PF `1.2557`, DD `1.2937`, `28` trades. Conclusion: `ATT1` is a positive sleeve, but too sparse to carry the whole core alone.
