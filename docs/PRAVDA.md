...
## Инцидент безопасности MT5

- Во время параллельной работы в `signal_copy/config.py` попал неплейсхолдерный MT5-токен. Commit `5754ced` успел попасть в remote-ветку.
- Активная локальная ветка переписана на очищенный hash `4e55586`; в ней токен берётся только из локального env, execution по умолчанию выключен, allowlist счёта пуст, live запрещён.
- До использования MT5 нужно отозвать старый токен и создать новый. Remote-ветку нужно заменить очищенной историей через `force-with-lease`; это требует отдельного явного owner approval.
