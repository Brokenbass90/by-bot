...
4. **Signal parity ещё не закрыт.** Latest weekly replay нашёл backtest entries при нуле real entries; нужно сопоставить timestamps/decision path и проверить, применяется ли в replay тот же `NO_ENTRY_HOURS_UTC`, прежде чем менять пороги стратегий.
