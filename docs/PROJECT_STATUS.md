...
- **Allocator:** global_risk=0.8, hard_block=False, но strict static-v1 parity `FAIL`: extra sleeves `sloped, asm1`; extra symbols в `att1`, `flat`, `breakdown`; live effective concurrency default `MAX_POSITIONS=1`. Fixed-cache sensitivity: `1` slot `+30.76% / PF 1.330 / DD 5.68% / 2 red months`; `3` slots `+62.79% / PF 1.478 / DD 5.36% / 3 red months`; `4` slots `+74.41% / PF 1.600 / DD 5.03% / 3 red months`; `5` slots baseline `+73.96% / PF 1.591 / DD 5.16% / 2 red months`.
...
- `crypto_income_static_v1` slot sensitivity — finished locally on fixed cache: one-slot live default destroys much of the package return; four slots recover benchmark-level return but currently add one red month. Replay final ARF winner at `4` and `5` slots before proposing controlled live parameters.
