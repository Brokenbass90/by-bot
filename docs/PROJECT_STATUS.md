...
- Prepared 2026-05-25: bounded read-only `signal_decisions.jsonl` trace for static-v1 sleeves (`midterm/att1/flat/breakdown`) and delivery through `crypto_blocker` into operator AI context. This provides per-symbol decision evidence without changing orders, risk or enabled strategies.
...
- 2026-05-25 evening: prepared bounded per-decision trace for static-v1 live sleeves and `crypto_blocker` AI exposure; once deployed and sampled, it becomes the evidence for the next targeted filter replay rather than blind parameter relaxation.
