...
**Last updated:** 2026-05-26 evening (ARF1 package replay complete + strict live recovery gate)
...
## Однострочник
Bybit perpetuals bot + Alpaca equities. Live equity ≈ $123 Bybit + $1000 Alpaca paper. **2026-05-26:** `bybot` active и не hard-blocked, но строгая проверка доказала, что live пока НЕ равен benchmark: required static-v1 inputs покрыты на `100%`, однако allocator добавляет sleeves `sloped, asm1` и дополнительные symbols в `att1/flat/breakdown`; runtime trade-account настроен на `MAX_POSITIONS=3`, `leverage=3`, effective risk `0.44%`. Проверенный strict-three-slot ARF1 challenger улучшает сопоставимый baseline с `+62.79% / PF 1.478 / DD 5.36% / 3` красных месяца до `+67.11% / PF 1.530 / DD 4.82% / 2`; при пяти слотах research ceiling составляет `+77.57% / PF 1.646 / DD 5.16% / 2`, но это уже другая экспозиция. Текущий ноль entries всё ещё возникает до order path, внутри filters (`breakdown RSI/support`, `flat same_bar/range`, `ATT1 trendline`). Live не меняется до reviewed strict-three-slot canary: только четыре доказанных sleeve, фиксированные symbols, `MAX_OPEN_PORTFOLIO_RISK_PCT` и `MAX_SAME_DIRECTION_POSITIONS` перед любым включением. Alpaca v38 monthly paper: fixed HWM trailing сработал на GOOGL; следующий gate показал позицию закрытой и без cleanup-conflict. v39/v40 остаются research-only.
