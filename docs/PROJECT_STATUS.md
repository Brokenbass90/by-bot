...
**Last updated:** 2026-06-04 (live protection repaired + strict crypto baseline reconciliation)
...
## 2026-06-04 Control Review
- **Bybit protection:** the manually closed BTC short was a restored `bootstrap` position, not an entry opened by the main strategy runner. It had an exchange-side SL but no TP. Root cause: restored positions were treated as manually locked when either protection side existed. Deployed fix now treats partial bootstrap protection as incomplete, preserves the existing side, and fills the missing TP/SL. Manual close was reasonable; the same partial-protection state should now be repaired instead of ignored.
- **Crypto live is operational:** `bybot.service` is active and recent normal strategy entries/fills prove Bybit order access works. Post-thaw closed strategy trades include one winning ETH breakdown followed by losing ETH/ADA breakdown and BTC ATT1 trades. The current sample is too small and net-negative, so filters must not be loosened blindly.
- **Static-v1 reconciliation:** strict no-gap 365d replay on a newly downloaded exact Bybit dataset produced `+65.89% / PF 1.462 / DD 7.49% / 486 trades / 3 negative months`. It changed trade counts across every sleeve versus the older server artifact `+73.96% / PF 1.591 / DD 5.16% / 436 trades`, strongly indicating that the older result depended on different/fallback cache provenance. The strict result is now the reproducible promotion baseline.
- **Strict research data gate:** `backtest/run_portfolio.py` now supports `BACKTEST_REQUIRE_EXACT_CACHE=1` and rejects missing, partial, or gapped exact slices. Candidate package research must use the same validated 365-day cache; silent fallback to another candle window is no longer acceptable.
- **Next crypto candidates:** BRC1 annual/package replay, ASB1, bounded ATT1, recent breakdown repairs, MTPB recovery, GS1, and PFS1 did not beat the strict package gate. PFS1 now has timestamped no-lookahead funding data, but all 12 focused variants made zero additive trades and reproduced the baseline exactly, so it needs a redesign instead of a live wire-up. IVB1 v2 risk-fixed `r010` is the first candidate to survive every current gate: five-slot annual `+74.51% / PF 1.528 / WR 59.2% / DD 5.52% / 493 trades / 1` negative month versus `+65.89% / PF 1.462 / DD 7.49% / 3`; live-style strict3 `+64.90% / PF 1.452 / DD 5.22% / 2` negative months versus strict3 baseline `+59.31% / PF 1.409 / DD 10.08% / 4`. It improved both independent half-year segments: weak segment `+5.05% -> +16.33%` with DD `10.08% -> 5.22%`, and strong segment `+41.89% -> +45.85%`; the latter added one small red month. Telemetry-only IVB1 shadow was deployed `2026-06-04 17:29 UTC` with `risk=0`: it evaluates signals and reports dedicated counters, but returns before order submission. Observe it for 7-14 days before proposing any capital.
- **Research reproducibility repair:** additive package specs had omitted strict-baseline `ARF1_MIN_RSI` and `ARF1_REJECT_BELOW_RES_ATR`, while autoresearch resume reused completed rows by tag after env/spec changes. Package validation now enforces declared baseline parity, and resume requires a SHA256 fingerprint over command/base-env/overrides. The previously quoted pre-parity IVB1/MTPB/GS1/PFS1 rows are not promotion evidence; the metrics above are the corrected reruns.
- **Strategy health provenance:** `strategy_health_review.py` is installed and runs daily. It correctly marks the current tiny live samples as insufficient and performs no demotions. The older `configs/strategy_health.json` remains historical because `equity_curve_autopilot.py` refuses to use untrusted research/sweep artifacts as its baseline. That refusal is safe; the next task is publishing an explicitly reviewed strict baseline and separating live-observation freshness from historical backtest dates.
- **Claude session-log review:** useful diagnostic and pipeline ideas were retained, but its broad `git add .`, optimistic APR math, and unvalidated live wiring recommendations were rejected. Only scoped, reviewed changes enter commits or live.
- **Alpaca v38 paper:** on `2026-06-04 14:00 UTC`, QCOM software trailing armed from a saved HWM (`peak +10.44%`) and submitted a paper close after giveback while still around `+3.73%`. The follow-up paper gate verified that QCOM left positions, cash increased, a re-entry block was recorded, and there was no cleanup conflict. v38 paper mechanics have passed. The bridge now rejects live orders unless `monthly_v38` account role, explicit live confirmation, and a bounded capital override (default maximum `$500`) are present. Remaining work is operational: create a monthly-only live credential profile, read-only preflight it, and request explicit approval for the canary. v39/v40 remain research-only.
- **Cross-exchange arb:** the honest ROI report has `0` closed `settlement_execution_v2` cycles and `5` open shadow cycles. First expected close is `2026-06-05 06:15 UTC`; no monthly/annual projection and no `$1000` live funding until closed-cycle evidence exists.

## 2026-06-01 Morning Session
- **Alpaca v38:** May paper cycle already ran; June refresh completed at `2026-06-01 09:34 UTC`. New current-cycle picks: `DDOG,QCOM,NOW`. Backtest refresh summary now shows `+58.15%` compounded over 24 calendar months, PF `6.62`, WR `80.0%`, max monthly DD `-3.86%`. Paper bridge selected DDOG as hold-existing and planned QCOM/NOW buys with broker protection, but both new protected buys ended with `entry_not_filled_before_stop status=accepted`; wait for the next market-hour paper gate/fill verification before any `$500` real deposit.
...
- **Alpaca intraday:** 360d replay for v1 completed: `-0.60%`, PF `0.976`, WR `43.75%`, DD `8.04%`, 112 trades, 7 red months. Intraday v1 is not a real-money candidate. v3 shadow replay is still running.
- **Arbitrage research:** `package_funding_harvest_v1` completed after a metrics compatibility fix. Best current Bybit-only funding result is about `3% APR` (`~0.25%/month`) on the tested basket: useful as a stable cash-yield layer, not enough for the target `2-3%/month`. Same-exchange Bybit basis grid on BTC/ETH/SOL returned zero trades even at `0.02%` entry threshold. Added `scripts/cross_exchange_funding_scan.py`, deployed to server, installed 15m cron, and injected top opportunities into AI full-context. First high-liquidity snapshot found large current funding spreads across Bybit/Binance/Bitget, but these are snapshot annualizations and require persistence/orderbook/fee validation before use.
...
- **Alpaca paper:** trailing fix относится к `v38 hybrid top4 monthly`, не к v39. На run `2026-05-26 14:00 UTC` сохранённый HWM GOOGL `$406.49` при entry `$382.03` (`peak +6.40%`) вооружил software trail и отправил close. Run `14:30 UTC` уже не содержит GOOGL в positions, содержит re-entry block до `2026-06-16`, не содержит duplicate cleanup; intraday-owned `NFLX` корректно защищена от monthly cleanup. Осталось сверить заполненный paper ledger/итог цикла и ротацию paper credentials перед депозитом.
- **Owner reserve:** $2000 в крипте, ждёт Фаза 1 trigger
- **Regime:** `bear_chop` на свежем server heartbeat `2026-05-26 14:46 UTC`; прежний snapshot был `bear_trend`.
- **Allocator:** global_risk=0.8, hard_block=False, но strict static-v1 parity `FAIL`: extra sleeves `sloped, asm1`; extra symbols в `att1`, `flat`, `breakdown`. Effective running account config verified without exposing keys: `MAX_POSITIONS=3`, `leverage=3`, effective risk `0.44%` (`orch=0.55`, allocator=`0.80`). `MAX_OPEN_PORTFOLIO_RISK_PCT` и `MAX_SAME_DIRECTION_POSITIONS` сейчас не установлены, то есть отдельные aggregate/direction caps выключены. Fixed-cache baseline sensitivity: `1` slot `+30.76% / PF 1.330 / DD 5.68% / 2 red months`; current-style `3` slots `+62.79% / PF 1.478 / DD 5.36% / 3 red months`; `4` slots `+74.41% / PF 1.600 / DD 5.03% / 3 red months`; `5` slots baseline `+73.96% / PF 1.591 / DD 5.16% / 2 red months`. Confirmed ARF1 challenger: current-compatible `3` slots `+67.11% / PF 1.530 / DD 4.82% / 2 red months`; `5` slots `+77.57% / PF 1.646 / DD 5.16% / 2 red months`; `4` slots `+76.55% / PF 1.636 / DD 5.00% / 3 red months`.
...
- `package_arf1_flat_touch_v1` — finished `48/48` on mirrored server candle-cache. Winner `r002`: `ARF1_MIN_RSI=48`, `ARF1_REJECT_BELOW_RES_ATR=0.08`, `ARF1_RES_TOUCH_BUFFER_ATR=0.35`; repeated full-package replay at `MAX_POSITIONS=5` reproduced `+77.57%`, PF `1.646`, DD `5.16%`, 419 trades, 2 red months.
- `crypto_income_static_v1` slot sensitivity — finished locally on fixed cache: live's verified three slots are materially weaker than the five-slot benchmark (`+62.79% / PF 1.478` versus `+73.96% / PF 1.591`). ARF1 winner replay at the existing three-slot concurrency gives `+67.11%`, PF `1.530`, DD `4.82%`, 432 trades, 2 red months, so it is the conservative recovery candidate; its review-only config is `configs/crypto_income_arf1_r002_strict3_shadow_candidate.env`. Five-slot promotion is deferred until strict parity and aggregate-risk caps are proven.
- `bear_regime_continuation_v1_fast_90d_v1` — clean server-mirror sweep finished `64/64` on 2026-05-26. Best `r005`: `+6.15%`, PF `4.800`, WR `67.7%`, DD `0.40%`, 31 trades, `0` red months; overrides `PULLBACK_BARS=3`, `PULLBACK_MIN_PCT=0.25`, `RSI_MIN=45`, `RSI_MAX=68`, `MIN_REJECT_WICK_RATIO=0.8`, `RR=1.3`. Promising bear sleeve, but requires 365d and additive full-package replay before any live proposal.
- `elder_canonical_rewrite_v1` — bounded clean-mirror probe (`256` candidates maximum) started after BRC1. First 16 candidates are decisively negative (`PF ~0.28-0.32`, DD >83%); retain research-only and reject unless the remaining probe finds a qualitatively different passing pocket.
...
0. **P0-NEW: strict live-vs-static parity.** Previous `input_symbol_parity_pct=100.0` measured coverage only and masked drift. Patched/deployed read-only check on 2026-05-26 returns `verdict=FAIL`: extra live sleeves `sloped,asm1` and extra symbols in `att1/flat/breakdown`. Runtime verification shows live uses `MAX_POSITIONS=3`; matching ARF1 `r002` replay at the same concurrency passed (`+67.11%`, PF `1.530`, DD `4.82%`, 2 red months). Before any live change require strict four-sleeve/fixed-symbol policy plus explicit aggregate/direction caps because both caps are currently unset.
...
11. **Alpaca monthly software trail bug found/fixed in paper.** Current bridge armed trailing from the current remaining gain instead of the recorded high-water gain, allowing a fractional winner to give back the protected move. `GOOGL` evidence: peak `+6.40%`, later drop `5.78%`; pre-fix `trail_triggered=[]`, post-fix function check `True`. Observe an actual paper close/fill after US market opens before deposit.
12. **Alpaca v39 trailing/regime shortcut rejected.** New OHLC/high-water runner confirms that a simple trail is not a safe promotion path: default OHLC 24m `+55.54%`, PF `1.611`, DD `16.27%` versus close-only `+70.37%`, PF `1.850`, DD `13.29%`. A 24m `SPY SMA100` candidate reached `+72.24%`, PF `1.931`, DD `8.86%`, but failed OOS (`+6.07%`) and bear-2022; with warm-up plus risk-off exit all bear variants remained materially negative. Keep v39 research-only.
...
## История изменений
- 2026-06-01 midday: Alpaca v38 June refresh completed with fresh picks `DDOG,QCOM,NOW` (`+58.15%/24m`, PF `6.62`, WR `80%`, DD `-3.86%`), but the first paper protected buys for `QCOM/NOW` were accepted and not filled before stop-protection wait, so the `$500` real deposit gate remains pending until the next market-hour paper gate proves fills + broker protection. Added and deployed `scripts/cross_exchange_funding_validate.py`, a research-only orderbook/fees/persistence validator for public Bybit/Binance/Bitget funding spreads; first `$100/leg`, 24h validation passed `GUNUSDT`, `HOMEUSDT`, `LABUSDT`, `HUSDT`, `ZORAUSDT`, and the validated layer is now included in AI full-context. Installed validator cron every 15 minutes. Added/deployed `scripts/cross_exchange_funding_shadow.py`, which opens virtual pairs after persistence and tracks price drift + expected funding with no keys/orders; first shadow opens 5 pairs and starts negative by design because full roundtrip costs are reserved upfront. BRC1 package sweep is still running and currently failing the package baseline (`PF ~1.37-1.45`, DD `8.95-11.49%` for first rows). Local laptop sweeps for `package_breakdown_rsi_v1` and `package_arf1_flat_touch_v1` were started in detached screens, but local cache differs from the server dataset, so only server replays are promotion-authoritative.
- 2026-05-31: applied the approved `crypto_income_arf1_r002_strict3_live_canary.env` to the server as `configs/approved_strategy_params.env` and restarted `bybot` only after `open_trades=0`. Runtime bot logs confirm canary flags: `att1=True`, `flat=True`, `breakdown=True`, `midterm=True`; `sloped/asb1/hzbo1/bounce1/asm1/brc1/elder=False`. Also deployed and verified the web `useFetch` null-safe fix (`deps is not iterable` black screen gone locally and on server index). The operator/AI snapshot still reads stale allocator files, so its sleeve/risk commentary can be misleading until that context builder is patched to include actual runtime canary flags.
- 2026-05-31: closed the stale `brc1-elder-research` heartbeat. The watched clean-mirror directory no longer contains result artifacts and no research screen is running. Latest recorded evidence remains: BRC1 `r005` is promising on 90d (`+6.15%`, PF `4.800`, DD `0.40%`, 31 trades, 0 red months) but needs 365d/full-package gates; Elder canonical rewrite remains research-only because the recorded bounded probe started sharply negative and no passing final artifact is available.
...
- 2026-05-26 midday: diagnosed Alpaca monthly fractional trailing failure from real paper state. `GOOGL` recorded HWM `$406.49` from entry `$382.03` (`+6.40%`) but was not closed after a `5.78%` drop because software trailing tested current gain instead of peak gain. Commit `89c6f8d` is pushed and deployed to the paper bridge with unit coverage; next market-hour run must verify the paper close/fill and re-entry block.
- 2026-05-26 midday: close-only comparison rejects Claude's Alpaca v40 draft as a candidate (`24m -2.07%`, PF `1.006`; OOS 12m `+7.30%`, PF `1.256`; bear-2022 `-15.34%`, PF `0.413`). It reduces bear loss versus v39 but removes too much productive return. Next Alpaca research must simulate OHLC/high-water trailing and a regime gate on top of v39 rather than promote v40.
- 2026-05-26 midday: added the research-only OHLC/high-water `v39` validator. It reproduces the GOOGL-style peak-giveback question and rejects a quick v39 promotion: simple OHLC trailing worsens 24m metrics, while the attractive 24m `SPY SMA100` variant fails OOS and bear stress. `v38` with the corrected paper trail stays the near-term deposit candidate; more active Alpaca requires a redesigned defensive sleeve/cash policy.
- 2026-05-26 evening: completed full-package ARF1 flat-touch sweep and independent replay on the server-mirrored candle dataset. `r002` (`MIN_RSI=48`, `REJECT_BELOW_RES_ATR=0.08`, `RES_TOUCH_BUFFER_ATR=0.35`) improves the proven package at five slots to `+77.57%`, PF `1.646`, DD `5.16%`, 419 trades, 2 red months. Four slots slightly lower DD but adds a third red month. No crypto live changes made; next promotion gate is strict four-sleeve/fixed-symbol shadow with capped aggregate risk.
- 2026-05-26 evening: verified a review-only strict-three-slot config with ARF1 `r002`, matching current live concurrency: `+67.11%`, PF `1.530`, DD `4.82%`, 432 trades, 2 red months versus three-slot baseline `+62.79%`, PF `1.478`, DD `5.36%`, 3 red months. Also confirmed aggregate and same-direction portfolio caps are not configured in the live process. No live toggle or restart performed.
- 2026-05-26 evening: queued next clean research sleeves. BRC1 fast 90d sweep completed with a strong `r005` candidate (`+6.15%`, PF `4.800`, DD `0.40%`, 31 trades, 0 red months); next gate is annual plus package-additivity replay. Elder bounded probe is running and initially sharply negative.
