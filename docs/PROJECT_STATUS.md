...
- **Allocator:** global_risk=0.8, hard_block=False, но strict static-v1 parity `FAIL`: extra sleeves `sloped, asm1`; extra symbols в `att1`, `flat`, `breakdown`; live effective concurrency default `MAX_POSITIONS=1`. Fixed-cache baseline sensitivity: `1` slot `+30.76% / PF 1.330 / DD 5.68% / 2 red months`; `3` slots `+62.79% / PF 1.478 / DD 5.36% / 3 red months`; `4` slots `+74.41% / PF 1.600 / DD 5.03% / 3 red months`; `5` slots baseline `+73.96% / PF 1.591 / DD 5.16% / 2 red months`. Confirmed ARF1 challenger: `5` slots `+77.57% / PF 1.646 / DD 5.16% / 2 red months`; `4` slots `+76.55% / PF 1.636 / DD 5.00% / 3 red months`.
...
- `crypto_income_static_v1` slot sensitivity — finished locally on fixed cache: one-slot live default destroys much of package return. ARF1 winner replay at `MAX_POSITIONS=4` gives `+76.55%`, PF `1.636`, DD `5.00%`, 423 trades, but 3 red months; use five-slot challenger only after strict policy parity and capped-total-risk review.
...
0. **P0-NEW: strict live-vs-static parity.** Previous `input_symbol_parity_pct=100.0` measured coverage only and masked drift. Patched/deployed read-only check on 2026-05-26 now returns `verdict=FAIL`: extra live sleeves `sloped,asm1`, extra symbols in `att1/flat/breakdown`, and effective live concurrency defaults to `MAX_POSITIONS=1` while confirmed ARF1 challenger uses `5`. Challenger replay passed (`+77.57%`, PF `1.646`, DD `5.16%`, 2 red months); still need strict four-sleeve/fixed-symbol shadow and capped-total-risk review before any live change.
