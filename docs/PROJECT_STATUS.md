...
## Однострочник
Bybit perpetuals bot + Alpaca equities. Live equity ≈ $123 Bybit + $1000 Alpaca paper. **2026-05-26:** `bybot` active и не hard-blocked, но строгая проверка доказала, что live пока НЕ равен benchmark: required static-v1 inputs покрыты на `100%`, однако allocator добавляет sleeves `sloped, asm1` и дополнительные symbols в `att1/flat/breakdown`; runtime trade-account настроен на `MAX_POSITIONS=3`, `leverage=3`, effective risk `0.44%`. Трёхслотовый fixed-cache benchmark слабее пятислотового (`+62.79% / PF 1.478` против `+73.96% / PF 1.591`). Полный ARF1 package sweep завершён и replay подтверждён: при пяти слотах challenger даёт `+77.57% / PF 1.646 / DD 5.16% / 2` красных месяца, при четырёх `+76.55% / PF 1.636 / DD 5.00% / 3` красных месяца. Текущий ноль entries всё ещё возникает до order path, внутри filters (`breakdown RSI/support`, `flat same_bar/range`, `ATT1 trendline`). Live не меняется до strict four-sleeve/fixed-symbol shadow и проверки capped total risk для пяти слотов. Alpaca v38 monthly paper: fixed HWM trailing сработал на GOOGL; следующий gate показал позицию закрытой и без cleanup-conflict. v39/v40 остаются research-only.
...
- **Allocator:** global_risk=0.8, hard_block=False, но strict static-v1 parity `FAIL`: extra sleeves `sloped, asm1`; extra symbols в `att1`, `flat`, `breakdown`. Effective running account config verified without exposing keys: `MAX_POSITIONS=3`, `leverage=3`, effective risk `0.44%` (`orch=0.55`, allocator=`0.80`). Fixed-cache baseline sensitivity: `1` slot `+30.76% / PF 1.330 / DD 5.68% / 2 red months`; current-style `3` slots `+62.79% / PF 1.478 / DD 5.36% / 3 red months`; `4` slots `+74.41% / PF 1.600 / DD 5.03% / 3 red months`; `5` slots baseline `+73.96% / PF 1.591 / DD 5.16% / 2 red months`. Confirmed ARF1 challenger: `5` slots `+77.57% / PF 1.646 / DD 5.16% / 2 red months`; `4` slots `+76.55% / PF 1.636 / DD 5.00% / 3 red months`.
...
- `crypto_income_static_v1` slot sensitivity — finished locally on fixed cache: live's verified three slots are materially weaker than the five-slot benchmark (`+62.79% / PF 1.478` versus `+73.96% / PF 1.591`). ARF1 winner replay at `MAX_POSITIONS=4` gives `+76.55%`, PF `1.636`, DD `5.00%`, 423 trades, but 3 red months; use five-slot challenger only after strict policy parity and capped-total-risk review.
