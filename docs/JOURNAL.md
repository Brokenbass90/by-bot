...
- 2026-04-11 07:15 UTC — took a clean morning snapshot before the Codex update so we do not lose the trading truth in a fog of half-remembered messages. The most important answer is finally concrete: Alpaca intraday has genuinely started paper-trading, not just reporting ideas. The server-side [configs/intraday_state.json](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/configs/intraday_state.json) currently holds an `NFLX` long with a real Alpaca order id, so counting the demo test from `2026-04-10` is now fair. Monthly Alpaca is not fully “gold standard” yet, but it is clearly out of stale mode: [runtime/equities_monthly_v36/current_cycle_summary.csv](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/runtime/equities_monthly_v36/current_cycle_summary.csv) shows a fresh current cycle on `2026-04-10` with `AMD`, `AMZN`, and `BAC`, and the remaining weakness is observability and order-flow polish, not lack of candidates. Overnight research also gave us one genuinely strong package and one cleaner sleeve answer. The strong package is `bear_chop + range`, with the best row in [autoresearch_20260410_193638_bear_chop_plus_range_probe_v1/results.csv](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/backtest_runs/autoresearch_20260410_193638_bear_chop_plus_range_probe_v1/results.csv) reaching `+25.39%`, PF `1.945`, DD `3.5311`, `124` trades, and `3` red months; the cleaner sleeve answer is [autoresearch_20260410_193638_flat_live_universe_repair_v2/results.csv](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/backtest_runs/autoresearch_20260410_193638_flat_live_universe_repair_v2/results.csv), whose best row gives `+11.78%`, PF `1.513`, DD `3.4029`, `82` trades, `2` red months on the repaired live-like universe. For the update itself I paused both sides of research: local workers are stopped, and the server slow queue is disabled (`/root/by-bot/configs/research_nightly_queue.json` now has `"enabled": false`) so we can resume cleanly after the tool update instead of inheriting another round of half-finished runs. 
- 2026-04-11 10:55 UTC — resumed only the important frontiers after the Codex restart and turned two “unknowns” into concrete truths. First, I restarted the runs that actually matter for the next income decision: `flat_live_frequency_v3`, `bear_chop_core_repair_v1`, `ivb1_wider_universe_v1`, plus corrected dynamic annual/walk-forward validations on the `bear_chop + range` candidate. Second, I stopped treating Elder as a vague “maybe rewrite later” problem and rewrote [strategies/elder_triple_screen_v2.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/strategies/elder_triple_screen_v2.py) toward a more canonical Triple Screen interpretation: trend can now come from MACD histogram slope, Screen 2 can use Force Index pullbacks, and Screen 3 uses breakout above/below the previous bar with an ATR buffer instead of the older reclaim logic. That rewrite is now being judged by a new broad annual probe in [configs/autoresearch/elder_canonical_rewrite_v1.json](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/configs/autoresearch/elder_canonical_rewrite_v1.json); early rows are still zero-trade fails, so Elder remains open research, not a promotion sleeve. The bigger practical breakthrough came from range. I finally traced why `range_scalp` kept disappearing from the “honest” dynamic annual/walk-forward even when the static package looked strong: it was being killed by health truth, not by the router or by missing env flags. In [configs/strategy_health.json](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/configs/strategy_health.json) `alt_range_scalp_v1` was still marked `PAUSE` as research-only, so allocator was correctly dropping it from full-stack replay. I created [configs/strategy_health_range_candidate.json](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/configs/strategy_health_range_candidate.json) to revalidate it as a promotion candidate, fixed [scripts/run_control_plane_replay.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/scripts/run_control_plane_replay.py) so sleeve summaries count from actual `sleeves_active`, and relaunched the dynamic annual/walk-forward with the range candidate health override. That immediately produced the first honest full-stack windows where `range_scalp` is truly present instead of silently filtered out: `w01` = `bull_chop`, PF `2.346`, net `+0.84`, sleeves `flat,range_scalp,impulse`; `w02` = `bear_chop`, PF `4.505`, net `+2.06`, sleeves `flat,range_scalp`. This is the first real proof that the strongest static package (`bear_chop + range`) can survive the dynamic replay layer. 
- 2026-04-11 11:05 UTC — fixed a blind spot in Alpaca observability instead of letting the operator keep hallucinating stale states. The trading logic itself was already in a better place than the snapshot suggested: server current-cycle files showed a fresh monthly set (`AMD;AMZN;BAC`) and intraday had a real paper position in `configs/intraday_state.json`, but `bot/operator_snapshot.py` still relied too heavily on `latest_advisory.json`, which is sometimes missing or lagging. I patched the snapshot builder so it can reconstruct a truthful monthly fallback from `current_cycle_picks.csv` plus `latest_refresh.env`, and fall back to `configs/intraday_state.json` for open intraday positions when the advisory file is absent. That means our operator view should stop saying “nothing is happening” in cases where the runtime files already prove otherwise. 
- 2026-04-11 09:46 UTC — added a slow self-audit layer so the server can tell us what is actually blocking money instead of only printing raw status. I implemented [scripts/build_self_audit_report.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/scripts/build_self_audit_report.py) to combine the live `operator_snapshot` with recent runtime diagnostic counters and emit a compact `runtime/self_audit/latest.{json,txt}` report with findings and next actions. I then wired that summary back into [bot/operator_snapshot.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/bot/operator_snapshot.py), added it to [scripts/setup_server_crons.sh](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/scripts/setup_server_crons.sh), updated [scripts/deploy_foundation.sh](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/scripts/deploy_foundation.sh), and redeployed the whole foundation to the live server. The first report said exactly what we needed it to say: not “everything is dead”, but “allocator only has a mild overlap haircut, Alpaca monthly is in `selected_current_cycle`, Alpaca intraday is already holding `NFLX`, and the real next bottleneck is still entry-rate.” That is a much healthier place to iterate from than the old cycle of stale panic. 
- 2026-04-11 17:25 UTC — ATT1/ASM1 stopped being “paper candidates” and became real live-stack candidates. I wired both sleeves into [smart_pump_reversal_bot.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/smart_pump_reversal_bot.py): runtime flags, engine init/lazy-init, scheduling, execution, startup universe messages, and control-plane hot-reload support are all there now. I also extended [bot/allowlist_watcher.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/bot/allowlist_watcher.py) and [bot/diagnostics.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/bot/diagnostics.py) so ATT1/ASM1 symbol updates and runtime counters no longer disappear into static env files. The subtle but important second fix was allocator policy: until now [configs/portfolio_allocator_policy.json](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/configs/portfolio_allocator_policy.json) did not even know ATT1/ASM1 existed, so any full-stack “validation” would have silently ignored them. I added both sleeves there (and to the `no_sloped` variant), then launched honest dynamic annual and walk-forward validations on [configs/core3_live_canary_20260411_sloped_momentum.env](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/configs/core3_live_canary_20260411_sloped_momentum.env). Early windows are still dominated by the already-proven `flat + range + impulse` mix, which is fine; the real win is that ATT1/ASM1 are finally being judged inside the same stack we would eventually promote, instead of living forever in isolated sweeps.
- 2026-04-11 17:32 UTC — found and fixed the last ATT1/ASM1 visibility gap, then deliberately shifted one research slot from Elder to inplay breakout expansion. The first sloped `v1` dynamic run taught us something important: even after wiring ATT1/ASM1 into live code and allocator policy, the historical router still was not emitting `ATT1_SYMBOL_ALLOWLIST` / `ASM1_SYMBOL_ALLOWLIST` because [configs/strategy_profile_registry.json](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/configs/strategy_profile_registry.json) did not contain those profiles yet. I added regime-aware ATT1/ASM1 profiles, verified that router now produces real symbol lists for them and allocator now enables them inside the stack, then restarted clean `v2` dynamic annual/walk-forward runs. The early answer is useful and humbling: ATT1/ASM1 add frequency fast, but quality in the first windows is weak, so they are still research, not promotion. Because the user asked for a harder push on “inplay” style setups, I also stopped the active Elder canonical rewrite run and launched [configs/autoresearch/breakout_expansion_v1.json](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/configs/autoresearch/breakout_expansion_v1.json). That gives us a clean next question to answer: can the ARIA-style cascade-highs breakout family survive a full-year crypto test, or is it another visually compelling but weak edge? Finally, I promoted “operator gets smarter and more web-aware” from a loose wish to an explicit roadmap block in [docs/ROADMAP.md](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/docs/ROADMAP.md), so we stop pretending that richer operator intelligence will happen automatically.
- 2026-04-11 18:05 UTC — converted two user demands into actual work instead of parking them in chat. First, I verified that our equities cache already contains `70` symbols, which means “try a truly broader dynamic Alpaca universe” is no longer blocked by data coverage. Rather than shoving that straight into live paper, I created [configs/alpaca_intraday_dynamic_v2_broad.env](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/configs/alpaca_intraday_dynamic_v2_broad.env) plus [scripts/run_equities_intraday_dynamic_annual_segments_broad.sh](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/scripts/run_equities_intraday_dynamic_annual_segments_broad.sh), which use full-cache discovery and a larger balanced breakout/reversion watchlist for segmented annual research. Second, I stopped hand-waving about ARIA-like setups and created [configs/autoresearch/inplay_breakout_retest_focus_v1.json](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/configs/autoresearch/inplay_breakout_retest_focus_v1.json), a retest-focused breakout frontier that tests real pullback/reclaim behavior instead of “just buy before the level and hope”. This is the right split for now: broad Alpaca gets a safer research lane, and crypto inplay gets a sharper hypothesis instead of a vibes-based strategy.
- 2026-04-11 19:00 UTC — the broad Alpaca experiment already paid for itself by revealing a plumbing bug, and the first inplay retest branch was honest enough to kill quickly. In the broad annual flow, [scripts/run_equities_intraday_dynamic_research.sh](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/scripts/run_equities_intraday_dynamic_research.sh) was still parsing tickers from the default `configs/intraday_config.json` even after the builder wrote a brand new broad config file. That would have made the whole “broad market” result half-fake, so I fixed the script to export and honor `CFG_PATH`, then restarted the annual segments. The rerun now really uses the expanded `NKE/MDB/NFLX/TSLA/SNOW/PLTR/HD/NVDA/AMD/AAPL/JPM/COST/SBUX/GS` type watchlists, which is the first real proof that we can let Alpaca search a wider dynamic equity field without pretending. On the crypto side, [configs/autoresearch/inplay_breakout_retest_focus_v1.json](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/configs/autoresearch/inplay_breakout_retest_focus_v1.json) stayed weak across row after row (low trade count, PF ~0.52, slightly negative net), so I killed it instead of getting emotionally attached. Its replacement is [configs/autoresearch/inplay_first_touch_bounce_v1.json](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/configs/autoresearch/inplay_first_touch_bounce_v1.json), which finally tests the user’s more realistic intuition directly: maybe the money is in the first reclaim/bounce of the trend leg, not in the actual breakout retest. 
- 2026-04-11 19:12 UTC — another broad-Alpaca hidden bug surfaced, and this one was more structural than the config-path mistake. [scripts/build_equities_intraday_watchlist.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/scripts/build_equities_intraday_watchlist.py) was still hard-coded for `78` bars/day, which only makes sense for true 5m sessions, while our equities cache in `data_cache/equities_1h` contains hourly candles. That mismatch caused `_load_metrics()` to reject the broad universe as if history were missing, which in practice can masquerade as “dynamic broad market found nothing” when the real problem is just the selector’s assumptions. I changed the builder to infer bars/day from the actual data, then used the same patch pass to let broad dynamic Alpaca emit mature variant classes (`breakout_continuation:quality_guard` and `grid_reversion:safe_winrate`) instead of only raw breakout/reversion tags. That means the broad annual now tests a more adult version of intraday Alpaca: wider stock search plus better-behaved tactics, not just a noisier version of the current bridge.
- 2026-04-11 18:20 UTC — I tightened the feedback loop on both the crypto idea-factory and the equities autonomy work. `Inplay` still does not deserve emotional attachment: [configs/autoresearch/inplay_breakout_retest_focus_v1.json](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/configs/autoresearch/inplay_breakout_retest_focus_v1.json) remained stuck around `PF ~0.51-0.52` with too few trades and slightly negative net, and the replacement [configs/autoresearch/inplay_first_touch_bounce_v1.json](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/configs/autoresearch/inplay_first_touch_bounce_v1.json) is also coming back negative enough that it looks like a research branch, not the next promotion sleeve. On the Alpaca side the good news is more structural: the restarted broad annual now clearly produces real per-segment dynamic watchlists from [configs/alpaca_intraday_dynamic_v2_broad.env](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/configs/alpaca_intraday_dynamic_v2_broad.env) instead of pretending the old shortlist is “the market”. The next blocker I hit there was not the selector but the fetcher: [scripts/equities_fetch_extended_universe.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/scripts/equities_fetch_extended_universe.py) was asking Yahoo for impossible 1h history horizons, so I capped it to the actual ~700-day window and turned the extended-universe refresh into a viable autonomy tool rather than a noisy failure mode. I then rotated the freed research slot away from weak `inplay` and back into honest dynamic annual/walk-forward validation for the stronger `range` promotion package.
- 2026-04-11 21:55 UTC — the next important truth was not another strategy, but a harness bug that could have poisoned the strategy decision itself. When I tried to compare `range-only` against `range + sloped`, the outputs suspiciously stayed identical. The reason was simple and dangerous: [scripts/run_dynamic_crypto_annual.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/scripts/run_dynamic_crypto_annual.py) and [scripts/run_dynamic_crypto_walkforward.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/scripts/run_dynamic_crypto_walkforward.py) were only reading the base env file, so runtime overrides like `ENABLE_ATT1_TRADING=0` never reached allocator/router replay. I fixed both harnesses to merge explicit runtime strategy/config overrides from the process environment before building historical windows. After that I relaunched a corrected `dynamic_core3_range_only_v2_{annual,wf360}` A/B so tomorrow we can judge `range` without hidden `ATT1/ASM1` contamination. I also reopened the “Claude strategies” front properly instead of letting it fade into memory: the standalone evidence is still strong (`ASM1` best sweep `+26.45%`, PF `1.531`, `171` trades, DD `4.57`; `ATT1` best sweep `+25.62%`, PF `1.263`, `336` trades, DD `6.44`), so I launched a fresh overnight honest full-stack validation on [configs/core3_live_canary_20260411_sloped_momentum.env](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/configs/core3_live_canary_20260411_sloped_momentum.env) as `dynamic_core3_sloped_momentum_v3_{annual,wf360}`. That is the right way to answer whether those strategies expand the real portfolio instead of only looking good alone.
- 2026-04-12 08:20 UTC — the morning answer is finally more specific than “range maybe, slopes maybe.” The honest overnight full-stack result says the Claude sloped package is real: `dynamic_core3_sloped_momentum_v3_annual` finished `+57.59%`, PF `1.349`, DD `7.03%`, `625` trades, `5` red months, with WF pass ratio `0.6818`. The corrected `range-only` package is still valid, but much smaller: `+19.52%`, PF `1.478`, DD `5.20%`, `182` trades, `4` red months, WF pass ratio `0.5455`. That changes the conversation. The next step is no longer “do we keep rerunning the same old portfolio?” but “how do we tame the sloped package into something promotable?” I answered that by creating [configs/core3_live_canary_20260412_sloped_balanced.env](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/configs/core3_live_canary_20260412_sloped_balanced.env), where `ASM1` stays full and `ATT1` is cut to `0.5x` risk / `1` slot. Fresh annual and WF360 are now running on that balanced variant. On the equity side, I also rechecked broad `Alpaca intraday` and kept the line on truth: the broad selector is fixed and genuinely dynamic now, but the current outputs are still scan/gate evidence and cannot yet be honestly sold as a stitched `% annual return`.
- 2026-04-12 09:45 UTC — the next hour was useful because it turned “can we smooth the sloped package?” into a real answer. The `balanced` variant came back at `+52.04%`, PF `1.319`, DD `7.02%`, `613` trades, still with `5` red months. That means halving `ATT1` did preserve most of the edge, but it did not solve the red-month count by itself. The good part is that the red months remain shallow (roughly `-0.14%` to `-1.22%`), so we are fighting correlation and regime fit, not catastrophic blow-ups. I immediately pushed one step further by creating [configs/core3_live_canary_20260412_sloped_guarded.env](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/configs/core3_live_canary_20260412_sloped_guarded.env) with `ATT1=0.35x` and `IVB1=0.75x`, then launched fresh annual + WF360 on that guarded package. At the same time I stopped two idea branches that were eating CPU without earning trust: `sloped_break_retest_v1_probe` is still stuck in zero/runner-crash territory, and `sloped_resistance_choch_v1_probe` is still producing zero-trade failures. That leaves the active effort on what is actually improving the core stack rather than on chart patterns we merely want to believe in.
- 2026-04-12 10:10 UTC — I found a real bug behind the fake “guarded equals balanced” result, which is exactly the kind of thing that can make strategy work feel like theater if we do not catch it. The issue was not in the dynamic harness this time, but in [backtest/run_portfolio.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/backtest/run_portfolio.py): `_allocator_risk_mult()` did not map `alt_trendline_touch_v1` to `ATT1_RISK_MULT` or `alt_sloped_momentum_v1` to `ASM1_RISK_MULT`, and it still used the legacy `IMPULSE_RISK_MULT` path instead of the live-style `IVB1_RISK_MULT`. That meant my guarded risk trims were not actually changing position sizing for the very sleeves I was trying to smooth. I fixed the mapping, recompiled the engine, and relaunched `dynamic_core3_sloped_guarded_v2_{annual,wf360}`. The first windows already differ from the old false clone, which is exactly what we wanted to see: `annual w01` improved from `pf=0.813 net=-2.12` to `pf=0.900 net=-1.00`, and `wf w01` is now a small positive window instead of an untrusted copy. That does not prove guarded is the final answer yet, but it proves the experiment is finally honest.
- 2026-04-12 10:18 UTC — I finished the first honest red-month autopsy on the sloped package and used it to open a cleaner A/B instead of piling on more guesses. The red months are not random. `ATT1` keeps surfacing as the recurring weak sleeve, while `ASM1` is the stronger annual contributor. The bad pockets cluster around specific symbol/regime mixes (`SOL/LTC/ETH` in spring, `DOT/ADA` in early fall, `ETH/XRP` in late winter), which is exactly the kind of evidence we need before changing live config. I turned that into [configs/core3_live_canary_20260412_sloped_asm1_core.env](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/configs/core3_live_canary_20260412_sloped_asm1_core.env) and launched `dynamic_core3_sloped_asm1_core_v1_{annual,wf360}` so the next comparison is no longer “balanced versus guarded maybe”, but “does removing `ATT1` materially improve month smoothness while preserving enough of the sloped uplift to justify promotion?”
- 2026-04-12 10:32 UTC — I pushed the Alpaca intraday lane one step closer to the full-market autonomy we actually want. Broad scan results have been telling us something consistent: raw `trend_retest_session_v1` and `trend_pullback_rebound_v1` look more alive on the wider equity universe than the older bridge-only `breakout/grid` pair. So instead of pretending the bridge is already broad-aware enough, I added `trend_retest` and `trend_pullback_rebound` class support, including `quality_guard` variants, to [scripts/equities_alpaca_intraday_bridge.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/scripts/equities_alpaca_intraday_bridge.py). Then I created [configs/alpaca_intraday_dynamic_v3_trend_broad.env](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/configs/alpaca_intraday_dynamic_v3_trend_broad.env) and launched a fresh segmented annual broad run. That gives us a real chance to end today with something better than “broad scan raw cents”: either a stitched-quality candidate built from the trend families the broad universe actually likes, or a clean negative answer that saves us from more weeks of running the wrong equity intraday lane.
- 2026-04-12 10:36 UTC — `ASM1-core` gave us the right kind of disappointment: useful, directional, but not final. It lowered red months from `5` to `4`, improved some ugly windows sharply, and showed that the `ATT1` problem is real. But it also dropped total return too far (`+32.22%` annual) and still left one genuinely bad month (`2026-01` about `-2.52%`). So I did not take that as “remove ATT1 forever”; I used it as proof that we need smarter symbol hygiene, not a blunt amputation. I built [configs/core3_live_canary_20260412_sloped_guarded_v3.env](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/configs/core3_live_canary_20260412_sloped_guarded_v3.env), where `ATT1` is restricted to `BTC/ETH/LINK`, `ASM1` drops `XRP`, and `IVB1` is trimmed slightly again. Annual + WF360 are now running on that package, and that feels like the first refinement directly grounded in the month-level autopsy instead of generic optimism.
- 2026-04-17 07:25 UTC | Codex | Re-centered the project around truthful live state instead of stale control-plane noise. Updated [configs/strategy_health.json](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/configs/strategy_health.json) so allocator health now reflects the actually deployed sleeves (`ATT1`, `ASB1`, `HZBO1`, `Elder`, `range_scalp`) instead of treating them as missing or old research artifacts. Also refreshed [docs/ROADMAP.md](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/docs/ROADMAP.md) and [docs/WORKLOG.md](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/docs/WORKLOG.md) so the next sessions inherit one clear plan. | Key findings: `midterm_v3` on server is no longer a hypothetical. The annual BTC+ETH check is complete: `macd_shorts` barely passes the minimum canary gate (`31` trades, PF `1.115`), `macd_rsi` clearly fails, and legacy `v1` still benchmarks higher (`PF 1.279`). The allocator may still stay `degraded`, but after this reconciliation it should only be for a real reason: portfolio overlap haircut, not missing-health drift. | Next: rebuild allocator state on server from the updated health file, verify that the remaining `degraded` reason is only overlap, then continue the promotion queue in order: `Elder` volume filter, `IVB1 WF-22`, `TS132 WF-22`, and only then the full-year portfolio pass.
- 2026-04-17 07:34 UTC | Codex | Closed the allocator-truth loop all the way to the live server: uploaded the reconciled [strategy_health.json](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/configs/strategy_health.json) to `64.226.73.119` and rebuilt `portfolio_allocator_state.json` there. | Key findings: the allocator is now "in order" in the important sense. `overall_health=OK`, `active_watch_sleeves=[]`, and the fake degraded reason is gone. The remaining `degraded` state is purely intentional risk control: `portfolio_overlap:0.48` with `asb1 + att1 + elder_ts + flat + hzbo1 + range_scalp` enabled together. | Next: do not waste time on more health-file cleanup. Move directly to either overlap reduction work or the next promotion queue (`Elder vol filter`, `IVB1 WF-22`, `TS132 WF-22`, then full-year portfolio pass).
- 2026-04-17 07:32 UTC | Codex | Started the next quiet server-side research slot instead of letting the machine sit idle after the allocator fix: launched `elder_triple_screen_v2` annual validation with the volume-confirmation path as the next promotion question. | Key findings: the run is active on `64.226.73.119` as `.venv/bin/python backtest/run_portfolio.py --strategies elder_triple_screen_v2 --symbols BTCUSDT,ETHUSDT,SOLUSDT,LINKUSDT --days 365 --end 2026-03-31 --tag elder_vol_test_20260417`. This keeps server pressure controlled while advancing the highest-value unresolved live decision after allocator truth. | Next: collect the Elder result, then either promote `ETS2_VOL_CONFIRM=1` or reject it, and only after that open the next heavy slot for `IVB1 WF-22`.
- 2026-04-17 07:54 UTC | Codex | The next sequence resolved three practical unknowns at once. First, `Elder` volume-filter did not merely "not help enough"; it failed cleanly: `PF 0.895`, `-37.49%`, `1222` trades, `50.68%` max DD on the annual server run. That is a strong rejection, not a borderline pass. Second, Alpaca demo is not dormant infrastructure anymore: the server has active paper crons, `ALPACA_SEND_ORDERS=1`, and the latest intraday advisory still manages three open paper positions (`NFLX`, `COST`, `META`). Third, the first `IVB1 WF-22` launch exposed another real ops bug: `scripts/run_generic_wf.py` existed locally but was not part of the server deploy surface, so the first attempt failed before any strategy logic ran. I patched [scripts/deploy_foundation.sh](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/scripts/deploy_foundation.sh), uploaded the missing file, and relaunched a baseline `IVB1` walk-forward using the repaired defaults (`RR=2.2`, `SL_ATR=0.75`). | Key findings: our front is now clearer. `Elder vol filter` is rejected. `IVB1` is the current live crypto research slot. Alpaca paper is already in real test mode, but monthly and intraday should be evaluated separately because monthly is much stronger historically while intraday is the one actively trading now. | Next: let the `IVB1` baseline WF print its first windows, then decide whether to expand into a full sweep or move directly to `TS132 WF-22`.
- 2026-04-17 09:20 UTC | Codex | Closed the last practical blockers for a real Alpaca paper start and repaired a false-negative in the crypto research harness. The monthly Alpaca bridge now auto-discovers fresh runtime picks instead of getting stuck on stale research `picks.csv`, and it explicitly protects intraday-managed symbols from stale-position cleanup on the shared paper account. The intraday launcher also no longer forces `--live` into explicit `--dry-run` checks. On the server, `alpaca_paper_launch_both.sh` now passes end-to-end, the monthly dry-run reads the current `2026-04` picks, and a manual monthly autopilot run has already submitted paper buys for `AMD` and `AMZN` while leaving `COST/META/NFLX` untouched. Separately, I verified that the first `IVB1 WF-22` “all TIMEOUT” result was not strategy truth but tool bias: a single 45-day `impulse_volume_breakout_v1` window completes, while `scripts/run_generic_wf.py` was still hard-coded to kill each window after `300s`. I made timeout configurable, uploaded the fix, and relaunched `IVB1` WF with lower parallelism and a `900s` timeout so the next verdict is about strategy quality rather than infrastructure impatience. | Key findings: Alpaca paper is now genuinely live on both branches, and the current crypto blocker has shifted from allocator truth to finishing honest validation for `IVB1` and then `TS132`. | Next: watch the relaunched `IVB1` WF for the first valid windows, then proceed through `TS132 WF-22` and the first full-year portfolio pass with a clean research toolchain.
- 2026-04-17 11:44 UTC | Codex | Moved the project from “paper is alive” to “paper is running the best current version” and opened the next durable crypto queue on the server. I deployed the newer Alpaca monthly improvements from the local branch, pinned the monthly paper env to the intended stronger profile (`3` positions, `0.90` target alloc, stop-loss, trailing, weighted sizing, ATR sizing, mid-month rotation), and refreshed the cycle. The new monthly run kept the existing `AMD/AMZN` pending paper orders, confirmed `AMD/AMZN/BAC` as the current-cycle picks, and added a new accepted `BAC` paper buy at its score-weighted size. On the crypto side I wired `btc_eth_midterm_short_v1` into [backtest/run_portfolio.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/backtest/run_portfolio.py), recorded it as an explicit `WATCH` in [configs/strategy_health.json](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/configs/strategy_health.json), and launched protected server queues for its annual + WF validation as well as `Elder` base retests. I also fixed the live `CLUSDT` failure at the practical layer: once Bybit says a symbol is unsupported, [smart_pump_reversal_bot.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/smart_pump_reversal_bot.py) now blacklists it in-memory and removes it from the next cached symbol universe. The bot was restarted during a verified `open_trades=0` window and is healthy again under `systemd`. | Key findings: Alpaca paper is now running the stronger monthly profile on the shared demo account, `IVB1` has already resolved from “all timeout” into a real but weak WF result, and the next unresolved crypto answers are now `midterm_short_v1` plus `Elder` base. | Next: let the server queues complete, then decide whether `IVB1` stays rejected, whether `midterm_short_v1` deserves promotion work, and whether `Elder` survives only as a bull-regime sleeve or gets removed entirely.
- 2026-04-17 12:09 UTC | Codex | Closed the loop on two important truth drifts and one live-risk error. First, [backtest/engine.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/backtest/engine.py) now supports `1440m` aggregation, which removes the fake infrastructure failure that blocked `btc_eth_midterm_short_v1`; the strategy now has a dedicated durable runner in [scripts/run_midterm_short_v1_backtests.sh](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/scripts/run_midterm_short_v1_backtests.sh), and the server queue is live again on the repaired engine. Second, the AI/operator Alpaca monthly block is no longer stale-zero fiction: [bot/operator_snapshot.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/bot/operator_snapshot.py) now falls back to the actual paper config when runtime advisory data is absent, so the server snapshot reports `capital=500` and `per_position=150` for the live paper sleeve. Third, and most materially for the live crypto stack, `Elder` is now out: server retests failed in both `2024` and `2023`, so I moved it to `PAUSE`, zeroed its allocator risk across regimes, rebuilt allocator state, and restarted `bybot` cleanly. | Key findings: allocator is back to `ok` with `global_risk=1.00`, `elder=False` is now real in the running process, and the active live sleeves are reduced to the healthier `att1`, `flat`, and `range_scalp` set while `midterm_short_v1` moves through honest validation. | Next: collect `midterm_short_v1` annual + WF results, then choose between `TS132 WF-22` and the first aggregated current-portfolio annual pass.
- 2026-04-17 12:23 UTC | Codex | Closed the next two open loops instead of leaving them in “almost done” state. The repaired `btc_eth_midterm_short_v1` queue has already finished on the server and the verdict is a hard reject: annual BTC+ETH result `43` trades, PF `0.163`, net `-1.79`; WF-22 `0/22` windows above PF `1`, with effectively zero usable activity. I moved the strategy from `WATCH` to `PAUSE` so allocator/operator truth stops pretending it is a realistic candidate. I also turned the “safe operator mode” request into code rather than a note: [configs/operator_capabilities.json](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/configs/operator_capabilities.json) now defines the bounded capability set, [bot/operator_snapshot.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/bot/operator_snapshot.py) exposes it as `[operator_controls]`, and [bot/deepseek_action_executor.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/bot/deepseek_action_executor.py) refuses AI-driven server deploy unless `DEEPSEEK_EXECUTOR_ALLOW_SERVER_DEPLOY=1` is explicitly turned on. | Key findings: the operator can now safely read summaries, launch whitelist research, compare results, and propose changes without silently taking over deployment; `midterm_short_v1` is no longer an open question, it is rejected in the current form. | Next: move the next validation slot to `TS132 WF-22`, then produce the first aggregated current-portfolio annual report.
- 2026-04-17 12:55 UTC | Codex | Cleaned up one of the last operator-facing lies before opening the next validation slot. The old runtime summary looked useful, but it was conflating raw config flags, allocator-active sleeves, and counters that reset on restart under a misleading `stats (24h)` heading. I changed [bot/diagnostics.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/bot/diagnostics.py) to hide zero-value counter spam by default and changed [smart_pump_reversal_bot.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/smart_pump_reversal_bot.py) so operator/TG status now separates `strategy-flags`, `allocator-active`, and `runtime-diag (since restart ...)`. I also added explicit roadmap language for the staged web UI we know we will need later: read-only dashboard first, safe control surface second, 2FA/RBAC/audit log from the start, and multi-account isolation as the system grows. Finally, I exposed self-audit as an explicit operator capability in [configs/operator_capabilities.json](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/configs/operator_capabilities.json), which makes the existing bounded bug-hunt lane easier to reason about from the control plane. | Key findings: the stale `elder=True` style operator confusion is now addressable in code rather than just manually explained, and the next heavy slot can now move to `TS132 WF-22` without dragging old reporting ambiguity behind it. | Next: run `TS132 WF-22` durably on the server, then use that result to decide the first aggregated annual report for the current crypto portfolio.
- 2026-04-17 12:52 UTC | Codex | Pushed the reporting fix all the way through to live instead of leaving it as a local cleanup. I updated [scripts/deploy_foundation.sh](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/scripts/deploy_foundation.sh) so it now carries the operator capability policy and the new [scripts/run_ts132_wf22.sh](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/scripts/run_ts132_wf22.sh) helper, then redeployed the server foundation. The restarted bot now emits the cleaner startup truth block on the server, and I immediately used the freshly deployed helper to launch `TS132 WF-22` in a durable `nohup` server run. | Key findings: we are no longer waiting idly for “sometime later” to start the next validation gate; the next promotion decision is already computing on server, and the operator-facing status format is finally less misleading after restarts. | Next: watch `TS132` for the first real windows, then open the first aggregated annual pass for the current crypto portfolio.
- 2026-04-17 13:15 UTC | Codex | The first `TS132` walk-forward taught us something useful, but not about `TS132` itself. All windows came back `NO_SUMMARY`, yet direct single-window backtests for `triple_screen_v132` on the same server produced valid `summary.csv` files even with `CACHE_ONLY=1`. That means the first failure was almost certainly a harness/low-memory/concurrency issue, not an honest 22-window reject. I repaired [scripts/run_generic_wf.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/scripts/run_generic_wf.py) so it now looks in both `stdout` and `stderr` for summary paths and reports actual subprocess exit codes instead of flattening everything into `NO_SUMMARY`. I also cut [scripts/run_ts132_wf22.sh](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/scripts/run_ts132_wf22.sh) down to `workers=1` by default, redeployed those scripts directly to the server, and relaunched `TS132` as `ts132_wf22_20260417_retry1`. While setting up the promised aggregated annual, I found another deploy-surface omission: `run_dynamic_crypto_annual.py` was not on the server. I uploaded it together with [run_control_plane_replay.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/scripts/run_control_plane_replay.py), patched [scripts/deploy_foundation.sh](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/scripts/deploy_foundation.sh) so future deployments keep those files in sync, and queued the annual report to start only after the current `TS132` processes exit. | Key findings: the next portfolio truth is still in motion, but we already avoided one false reject and one future deploy footgun. | Next: wait for the repaired `TS132` windows, then let the queued annual report start and finally assemble the first honest current-portfolio summary.
- 2026-04-17 14:38 UTC | Codex | Repaired the stitched annual after its first real infrastructure failure. The first run had already shown that the current crypto core is not “universally dead” — after 10 windows the stitched curve was still above water (`102.07` vs `100.00`) — but it crashed on window `11/12` because historical router replay introduced `LTCUSDT` while `BACKTEST_CACHE_ONLY=1` had no local cache slice for that symbol. I fixed [scripts/run_dynamic_crypto_annual.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/scripts/run_dynamic_crypto_annual.py) so each window now filters its router universe through the available cache before spawning [backtest/run_portfolio.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/backtest/run_portfolio.py), while still recording which symbols were requested versus dropped. I added a smoke regression, reran the local suite (`18/18` green), redeployed the foundation, and relaunched the stitched annual on server under `current_crypto_portfolio_annual_fixcache_20260417`. | Key findings: the next portfolio report should now fail only on strategy logic, not on a single missing cache file. | Next: let the repaired annual finish, then classify the surviving crypto core versus research-only sleeves from one consistent stitched report.
- 2026-04-17 15:15 UTC | Codex | The next server hour gave the clearest crypto answer of the day. The “fixed” stitched annual with `VWAP_MR` was not just weak; it was toxic, collapsing to `21.28` equity after `8/12` windows. The comparable `novwap` run, by contrast, completed all `12` windows and ended at `102.73`, which is still too weak for celebration but enough to prove that the crypto system is not dead and that `VWAP_MR` was poisoning the stitched stack. I also found why allocator alerts had come back even after we had cleaned them earlier: [scripts/equity_curve_autopilot.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/scripts/equity_curve_autopilot.py) had rewritten `configs/strategy_health.json` into a sparse WATCH-heavy file, dropping curated entries like `alt_resistance_fade_v1`, `ASB1`, `HZBO1`, and `VWAP_MR=PAUSE`. I patched the autopilot to preserve existing entries and to stop downgrading sleeves to `WATCH` purely because the market was quiet and recent trade count was below threshold. I also fixed the `dynamic annual` CSV writer to include the new cache-audit fields, synced the curated `strategy_health.json` back to the server, rebuilt allocator, and restored live control-plane truth to `OK` with only the intended live core enabled: `att1`, `flat`, `range_scalp`. On that clean baseline I launched `current_crypto_livecore_20260417`, a stitched annual that removes `breakdown/asb1/hzbo1/vwap` so we can finally quote a number for the actual live core rather than for a stale mixed stack. | Key findings: the right rehabilitation path is now explicit — stop treating `VWAP_MR` as a harmless sidecar, stop letting `equity_curve_autopilot` erase curated live health, and measure the real live core directly. | Next: wait for the first `livecore` windows, then queue isolated annuals for `ATT1`, `flat`, and `range_scalp`.
- 2026-04-17 15:42 UTC | Codex | The first direct live-core windows finally gave the kind of signal we can build on instead of argue about. `current_crypto_livecore_20260417` opened with two positive stitched windows in a row: `w01` at `+1.05`, PF `1.325`, DD `1.42`, `16` trades and `w02` at `+1.44`, PF `1.617`, DD `0.83`, `15` trades. That is still early, but it is materially different from the `VWAP_MR`-contaminated stack and is the first fresh server-side evidence that `att1 + flat + range_scalp` is a viable rehabilitation core rather than just a placeholder. I then turned the next attribution step into a durable queue by adding [scripts/run_crypto_rehab_queue.sh](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/scripts/run_crypto_rehab_queue.sh) and launching it on the server. The queue waits for `current_crypto_livecore_20260417`, then runs stitched annuals for `ATT1 only`, `flat only`, and `range only` sequentially. | Key findings: we now have both a cleaner live control-plane and an automatic next-step measurement path; the rehab plan is finally attached to running jobs, not just notes. | Next: let `livecore` continue, then read the queued isolated annuals to decide which sleeve to harden first.
- 2026-04-17 19:40 UTC | Codex | The evening pass turned the rehab story from “promising early windows” into an actual stitched baseline and an overnight research queue that can survive minor failures. The completed `current_crypto_livecore_20260417` run finished at `103.44` from `100.00` with PF `1.1318`, max DD `5.5863`, `120` trades, and allocator `ok` in all `12` windows. That is not the 100%+ dream portfolio yet, but it is the first honest yearly number for the *actual* live core (`att1 + flat + range_scalp`) and it is firmly better than the `VWAP_MR`-poisoned stitched stack. I also wired the newly handed-off research candidates [strategies/elder_triple_screen_v3.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/strategies/elder_triple_screen_v3.py) and [strategies/btc_eth_midterm_short_v2.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/strategies/btc_eth_midterm_short_v2.py) into [backtest/run_portfolio.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/backtest/run_portfolio.py), added durable launchers for them, and discovered that the first rehab queue could still die after `ATT1-only` because `set -e` treated a nonzero annual exit as fatal. I repaired [scripts/run_crypto_rehab_queue.sh](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/scripts/run_crypto_rehab_queue.sh) so it skips already completed runs, logs failures without killing the whole chain, and continues through the night. The first attribution readout already sharpened the picture: `ATT1-only` finished at `101.33`, PF `1.2557`, DD `1.2937`, `28` trades, which means `ATT1` is a real positive sleeve but too sparse to explain the live-core result on its own. After the repaired restart the server queue has already moved on to `current_crypto_flat_only_20260417`, with `range`, `elder_v3`, and `midterm_short_v2` queued behind it. | Key findings: the first truthful crypto hierarchy is now visible — `ATT1` is real but sparse, the live-core mix is modestly profitable over a stitched year, and tonight’s remaining uncertainty is mostly about how much `flat`/`range` contribute plus whether either of the new research rewrites deserves another chance. | Next: collect `flat` and `range` isolated annuals, then read the overnight `elder_v3` / `midterm_short_v2` validation results before deciding which sleeve is worth active hardening next.
- 2026-04-18 05:10 UTC | Codex | Closed one honest research question and one operator-facing product question in the same pass. `btc_eth_midterm_short_v2` is no longer waiting on weekly-engine support; after adding `10080m` to [backtest/engine.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/backtest/engine.py) and redeploying the fix, the server run completed with `0` trades on annual and `0/22` productive windows on WF. That makes it a real reject, not a harness victim. On the web side, I repaired the local dashboard until it could be exercised like an operator tool rather than admired as code: [web/routes/ai_routes.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/web/routes/ai_routes.py) no longer crashes on import and now requires admin auth for all AI endpoints, [web/auth.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/web/auth.py) now hashes new passwords with `pbkdf2_sha256` so setup is not broken by local `bcrypt`, and the AI context / allocator API now read runtime allocator truth instead of static policy. I verified the full local path with a temporary admin account: login, TOTP, `/auth/me`, `/api/status`, `/api/allocator`, and `/api/ai/context` all work on the running dashboard at `127.0.0.1:8765`. | Key findings: `midterm_short_v2` does not deserve more waiting in the current form, and the web dashboard is finally in a state where it can be inspected locally without immediately lying or crashing. | Next: use the repaired dashboard as a read-only operator surface, then keep crypto effort focused on sleeves that can still add real alpha instead of reviving dead midterm/Elder branches by hope alone.
- 2026-04-18 05:35 UTC | Codex | Made two pragmatic fixes that unblock real usage rather than more discussion. First, [web/setup_totp.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/web/setup_totp.py) now writes a QR PNG into `runtime/web_totp_qr/` during user setup, so the intended onboarding path is finally realistic: run the setup command, scan the QR in Google Authenticator, confirm with one 6-digit code, then use that same 6-digit code on the web login screen. Second, `funding_rate_reversion_v1` now has a real cron-friendly data path: [scripts/funding_rate_fetcher.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/scripts/funding_rate_fetcher.py) gained `--once`, [strategies/funding_rate_reversion_v1.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/strategies/funding_rate_reversion_v1.py) can read `configs/funding_rates_latest.json`, and [scripts/setup_server_crons.sh](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/scripts/setup_server_crons.sh) now includes a 5-minute snapshot refresh. One more truth surfaced while checking the fresh allocator work: `policy v7` is real, but it does not mean five new sleeves are live. Only `micro_scalp` is actually wired through the current live bot; the other new slots still need explicit live integration before they are more than policy/backtest entries. | Key findings: web onboarding is now user-realistic instead of engineer-realistic, `funding_rev` can finally consume live data, and `policy v7` should be read as “registered candidates” rather than “five extra live sleeves”. | Next: let the user create the real web account with QR, then choose whether to wire `funding_rev` or one of the bear/bidirectional v7 sleeves into the live bot first.
- 2026-04-18 06:20 UTC | Codex | Closed the last onboarding trap and packaged the next rehab lane so it can run durably on the server instead of pretending local cache-only results are truth. [web/setup_totp.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/web/setup_totp.py) now lazy-imports `qrcode`, auto-opens the generated QR PNG on macOS, and accepts `--admin`, which matters because otherwise the real owner account would have been created as a non-admin while the temporary `local-admin@example.com` still existed. I then created [configs/crypto_rehab_multiyear_queue.json](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/configs/crypto_rehab_multiyear_queue.json), [scripts/run_crypto_rehab_multiyear_queue.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/scripts/run_crypto_rehab_multiyear_queue.py), and [scripts/run_crypto_rehab_multiyear_queue.sh](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/scripts/run_crypto_rehab_multiyear_queue.sh) to run a resumable `2022/2023/2024/2025/2026-YTD` annual matrix followed by targeted autoresearch for the strongest sleeves. A local smoke run started and wrote progress/results files, but returned `0` ATT1 trades across multiple yearly windows under `BACKTEST_CACHE_ONLY=1`, which is a local cache-surface warning, not a credible verdict. I therefore treated the local run as runner validation only and prepared the queue for server deployment instead. I also extended [scripts/deploy_foundation.sh](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/scripts/deploy_foundation.sh) and [scripts/server_status.sh](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/scripts/server_status.sh) so the funding fetcher and rehab queue can be deployed and monitored cleanly. | Key findings: the real owner web account can now be created safely, and the next crypto alpha-rebuild step has been turned into a durable server-side process with explicit progress files rather than another ad-hoc notebook of ideas. | Next: create the owner account, revoke the temporary local admin, deploy the queue to the server, and let the server produce the first honest multiyear rehab matrix.
- 2026-04-18 09:55 UTC | Codex | Repaired the biggest remaining web-truth gap after the first real owner login. The local dashboard looked “polished but wrong” because it mixed a one-shot server mirror with stale local backtest trades and static policy rows. I fixed [web/routes/data_routes.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/web/routes/data_routes.py) and [web/routes/ai_routes.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/web/routes/ai_routes.py) so they now read from `WEB_RUNTIME_ROOT`, report `runtime_root/data_mode/sources`, and stop importing `backtest_runs/**/trades.csv` unless explicitly asked. I added [scripts/sync_web_live_mirror.sh](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/scripts/sync_web_live_mirror.sh) to mirror the live server’s heartbeat, allocator/regime/operator state, Alpaca runtime files, and `trades.csv` into `runtime/live_mirror/`, then upgraded [scripts/run_web.sh](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/scripts/run_web.sh) so a local live-mirror session refreshes that mirror every 60 seconds while the web process is running. On the frontend side [web/static/index.html](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/web/static/index.html) now labels `live mirror` vs `local runtime`, explains `Last 20` as the last 20 PnL points for a sleeve sparkline, renames the ambiguous `Journal` screen to `Project Log`, and shows policy-versus-runtime badges on the `Regime` page instead of pretending that every policy row is live. I also confirmed that the new real owner account `brokenbass1990@gmail.com` is now the only configured local web user and that `local-admin@example.com` no longer authenticates. | Key findings: the local dashboard can now be treated as a read-only mirror of live truth rather than a mashup of live/runtime/backtest artifacts, and the AI chat process now starts with the `.env` key material instead of booting “unconfigured”. | Next: relaunch the local web session against the fresh live mirror, sanity-check the browser again, and only then decide what deserves server deployment versus staying local-only.
- 2026-04-18 11:15 UTC | Codex | Pushed the web from “trustworthy mirror” to “first usable operator console”. The Alpaca page now has both paper branches instead of only the monthly story, because [scripts/sync_web_live_mirror.sh](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/scripts/sync_web_live_mirror.sh) mirrors `intraday_state.json` and [web/routes/data_routes.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/web/routes/data_routes.py) / [web/static/index.html](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/web/static/index.html) render those live paper positions beside `AMD/AMZN/BAC`. I also started collapsing the web and Telegram assistant split: [web/routes/ai_routes.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/web/routes/ai_routes.py) now persists a shared message history on disk and the frontend input grows with long prompts instead of clipping the operator into one-line questions. This still is not “server-ready SaaS”, because we do not yet have a consolidated live trade export, HTTPS/JWT hardening, or the full geometry overlay in the trade modal. But it is finally a tool that can be operated honestly instead of just demoed. | Key findings: the remaining web gaps are now explicit product choices, not hidden truth bugs. | Next: either wire a real live trade export into `Trades/Strategies/P&L`, or turn to server-grade web hardening before any public deployment.
- 2026-04-18 12:25 UTC | Codex | Closed the next truth gap in the web by teaching it to read real close events when the live bot has not produced a neat `trades.csv`. The mirror now syncs `runtime/live_trade_events.jsonl`, and the web reconstructs closed trades from those raw events for `Trades`, `Strategies`, `Equity`, and the admin P&L summaries. On the current mirror that yields `4` honest Bybit close records instead of an empty screen; the dataset is small and slightly ugly (`-1.026089` net, `1` win, `3` losses), but it is real. I also checked server storage because the user asked whether the web can safely accumulate data: the server still has `19G` free on a `24G` volume, so runtime/web/log truth is not the pressure point; large backtest archives remain the thing that will need explicit retention caps. | Key findings: we are now past the “empty or lying” stage for the main operator screens, and the next web bottlenecks are richer geometry overlays plus production hardening, not basic data ingestion. | Next: refresh the browser to confirm the new live trade rows, then decide whether the next slice is chart-geometry overlays or server-grade HTTPS/JWT/TOTP hardening.
- 2026-05-05 12:45 UTC — post-absence truth pass: server is healthy but crypto live has been flat for 5 days. Active live sleeves remain `att1 + flat + midterm`; no restart/deploy to the trading bot was made. Acceptance result: current canary v2 `+1.93 / PF 1.109 / DD 5.779` over 60d; Claude's literal v2.1 merge is identical; intended v2.1 set `ATT1+flat+midterm+support_bounce+IVB1+inplay` improves to `+7.70 / PF 1.431 / DD 3.592`, mostly from `ATT1`, with modest `support_bounce` help and weak/noisy side sleeves. Deployed web/AI truth fixes: `open_trades=0` is now framed as flat/no open positions, not offline; AI prompt now admits server backtest/autoresearch exists; web Strategies page now shows active live sleeves separately from historical closed trades. Added and installed the Friday weekly live-vs-backtest report cron, smoke-tested successfully with a 1-day report.
- 2026-05-05 12:50 UTC — after confirming `open_trades=0`, restarted `bybot.service` only to load the updated AI operator truth prompt. Queued `canary_v2_1_180d` acceptance to run after current support_bounce/IVB1 research finishes.
- 2026-05-05 13:25 UTC — added the first trade-forensics layer: [trade_forensics_report.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/scripts/trade_forensics_report.py). It reads backtest `trades.csv`, live `runtime/live_trade_events.jsonl`, and cached 5m klines, then writes Markdown + JSONL reports with MFE/MAE, post-exit movement, and labels such as `stop_then_reversed`, `entry_failed_fast`, `gave_back_profit`, and `tp_then_continued`. Server smoke passed on the intended canary v2.1 60d run: `70` trades, `0` missing candles, report at `/root/by-bot/reports/trade_forensics/trade_forensics_20260505_131931_canary_v2_1_intended_60d.md`. Main readout: `ATT1` remains the only clearly strong sleeve in that 60d set (`+8.0344`, PF `2.010`), `support_bounce` is modest but additive (`+1.4015`, PF `1.270`), while `flat` and `midterm` are weak in this slice. Live forensics over the last 120d confirms the web's old `range/inplay_breakdown` losses are old April data, not current active-sleeve trades.
- 2026-05-05 17:10 UTC — promoted trade forensics from a manual script into a weekly AI report. Added [weekly_trade_forensics_ai_report.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/scripts/weekly_trade_forensics_ai_report.py), deployed it to the server, smoke-tested both no-AI and DeepSeek modes, and installed the cron `45 7 * * 5 ... weekly_trade_forensics_ai_report.py --telegram --ai`. This means every Friday after the live-vs-backtest comparison the server will run forensic analysis on the latest backtest and recent live closes, then send a Telegram digest plus AI interpretation. Important caveat from the smoke: tiny samples can make DeepSeek over-dramatic, so the cron uses the default broader trade limit rather than the 5-trade smoke window.
- 2026-05-05 17:15 UTC — found why the next crypto acceptance step had not advanced: two old autoresearch parent processes (`support_bounce_v1_bull_sweep_v1`, `impulse_volume_breakout_v1_annual_repair_fetch_v2`) were alive for `5d+` at `0% CPU`, blocking the queued `canary_v2_1_180d` comparison. Stopped only those stale research parents; their active `run_portfolio` children are still computing normally (`~30% CPU`) and should finish the current rows before the acceptance queue can be restarted/continued. Trading bot was not touched.
- 2026-05-05 18:15 UTC — `canary_v2_1` 180d acceptance completed. Baseline live canary v2 (`ATT1+flat+midterm`) was not dead on 180d: `+30.09`, PF `1.570`, DD `4.78`, `184` trades. Intended v2.1 (`+support_bounce+IVB1+inplay`) improved net/activity to `+37.75`, PF `1.500`, DD `5.00`, `241` trades. Attribution is decisive: `ATT1 +23.74`, `support_bounce +16.67`, `midterm +1.39`, while `flat -0.61`, `IVB1 -2.20`, `inplay -1.25`. This means the immediate live rescue candidate should focus on `ATT1 + support_bounce` first; `IVB1/inplay` need repair, not promotion. Launched a sequential server comparison of clean 180d rescue combos: `ATT1+support`, `ATT1+support+midterm`, `ATT1+support+flat`, `ATT1+support+flat+midterm` in `logs/acceptance_20260505/rescue_combos_180d.log`.
- 2026-05-06 06:50 UTC — morning truth pass after the failed overnight research. The 2026-05-05 overnight queue did not produce ranked winners: it hung on the first `spike_rejection` autoresearch row for ~11h, leaving only a header-only `results.csv`. Stopped the stuck research process without touching the trading bot. Root cause was infrastructure/strategy-runtime, not a useful market verdict: new rescue sleeves had fragile candle access against `KlineStore` and `spike_rejection` could do expensive per-bar history slicing; the autoresearch runner also had no row timeout. Fixed [run_strategy_autoresearch.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/scripts/run_strategy_autoresearch.py), [alt_spike_rejection_v1.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/strategies/alt_spike_rejection_v1.py), [alt_bear_regime_continuation_v1.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/strategies/alt_bear_regime_continuation_v1.py), and [alt_whale_print_follow_v1.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/strategies/alt_whale_print_follow_v1.py). Local smokes after the fix: `bear_regime_continuation` on ADA 90d gave `4` trades, PF `1.405`, net `+0.26`; `whale_print_follow` on BTC 90d gave `10` trades, PF `1.396`, net `+0.54`; `spike_rejection` was fast but still too sparse on the 30d BTC smoke. Deployed the fixes to the server, compiled cleanly, pushed commit `2971906`, and started a safer server queue with 90d fast specs for `bear_regime_continuation`, `whale_print_follow`, and `spike_rejection` under `logs/fast_rescue_20260506/server_fast_queue.log`. Live bot status remains healthy (`bybot.service` active, fresh heartbeat, safe_mode `0`, global_risk `1.0`) but commercially quiet; active server `.env` still shows `ATT1 + flat + midterm`, with `ASB1/IVB1/inplay` disabled until a stronger recent-market candidate passes.
- 2026-05-06 07:35 UTC — first usable result from the fixed rescue search. Local fast 90d sweep for `alt_bear_regime_continuation_v1` completed with `64/64` passing rows. Best row `r005`: `31` trades, net `+6.15`, PF `4.800`, WR `67.7%`, DD `0.398%` on `SOL/ADA/DOT/LINK/SUI/LTC`, ending `2026-04-30`. Saved exact parameters in [brc1_fast_r005_candidate_20260506.env](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/configs/brc1_fast_r005_candidate_20260506.env). This is the first fresh-market short-continuation candidate worth promoting through the next gates, but it is still research-only: it needs 180d/365d standalone, portfolio additivity vs current live core, and live wiring before any real risk. `spike_rejection` completed with `0/48` passes (best PF `0.826`, net `-1.44`) and is not a live candidate. `whale_print_follow` was stopped after `60/96` rows because it had `0` passes and remained deeply negative (best partial PF `0.821`, net `-6.52`); it should be rewritten or reframed as a filter, not promoted.
- 2026-05-06 07:45 UTC — stopped the duplicate server-side fast rescue queue after confirming it was not productive on the droplet. The same BRC1 fast rows that pass locally were timing out at `600s` per row on the server, with subprocess logs showing only command/cache metadata and no summary output. Killed the autoresearch parent and remaining `run_portfolio` child. This is now a server backtest-performance/cache investigation item; it does not invalidate the local BRC1 result. Live trading service remained active and was not restarted.
- 2026-05-06 08:45 UTC — added the first safe live/shadow wiring for the fresh short-continuation candidate `alt_bear_regime_continuation_v1` (`BRC1`) in [smart_pump_reversal_bot.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/smart_pump_reversal_bot.py). This does not promote it to real-risk live trading by itself: defaults are `ENABLE_BRC1_TRADING=0` and `BRC1_RISK_MULT=0.0`. If enabled later with zero risk, it can emit Telegram/operator shadow signals and counters without placing orders. Also fixed BRC1 hot-reload integration so regime overlays and allocator overlays can see/clear/init the sleeve correctly instead of the control plane silently ignoring it. Syntax compile passed and commit `5ae767d` was pushed to GitHub. Server state checked immediately after: `bybot.service` active, heartbeat fresh, safe_mode `0`, active research processes `0`, and no trading-bot restart was made.
- 2026-05-06 08:50 UTC — confirmed a real deployment hygiene blocker on the server: `/root/by-bot` has many local modified and untracked files, so a normal `git pull` is unsafe. This explains part of the accumulated "tails" problem. Current rule: do not overwrite server files with Git until the server dirty tree is audited or specific files are deployed with backups. Local `docs/CODEX_DIRTY_WORKTREE_AUDIT_20260506.md` now records the cleanup policy; server dirty cleanup is a separate task before the next full foundation deploy.
- 2026-05-06 09:05 UTC — fixed the BRC1/backtest performance problem instead of letting more "overnight" jobs silently burn time. The slow path had two pieces: BRC1 recalculated the same high-timeframe trend filter on every 5m bar, and [backtest/run_portfolio.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/backtest/run_portfolio.py) built full-history close arrays for each regime check. Added a per-hour HTF cache in [alt_bear_regime_continuation_v1.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/strategies/alt_bear_regime_continuation_v1.py) and changed the regime helpers to use bounded recent windows. Result: BRC1 180d/365d standalone now finishes in seconds. Validation verdict for the exact `r005` params: `180d +7.62`, PF `1.967`, DD `2.02`, `73` trades, `2` red months; `365d +4.44`, PF `1.239`, DD `4.96`, `132` trades, `8` red months. Conclusion: BRC1 is useful for the recent weak/bear phase, but not yet annual-robust enough for real-risk promotion. Next step is portfolio additivity/shadow only, or improve its long-history filters before tiny live risk.
- 2026-05-06 09:25 UTC — completed first BRC1 additivity pass. On `180d`, `ATT1+BRC1` is strong: `+43.71`, PF `1.767`, DD `2.81`, `277` trades, `0` red months; attribution `ATT1 +38.09`, `BRC1 +5.61`. Adding `support_bounce` improves 180d net to `+53.09` but raises DD to `4.80`. On `365d`, the cleaner `ATT1+BRC1` package is still viable but less beautiful: `+48.78`, PF `1.387`, DD `7.03`, `545` trades, `4` red months; attribution `ATT1 +45.60`, `BRC1 +3.18`. The `ATT1+support_bounce+BRC1` 365d package is worse: `+40.39`, PF `1.215`, DD `16.99`, `6` red months, with `support_bounce` roughly flat over the full year. Decision: do not promote support_bounce on annual evidence yet; keep BRC1 as shadow/phase candidate and compare `ATT1+BRC1` versus current canary v2 in the final live candidate review.
- 2026-05-06 11:25 UTC — completed the final live-candidate sanity check before changing risk. The current live core (`ATT1+flat+midterm`) is still stronger over the full year than the tempting recent-market `ATT1+BRC1` replacement: `180d +25.52`, PF `1.506`, DD `4.33`, `205` trades, `1` red month; `365d +52.99`, PF `1.376`, DD `5.30`, `521` trades, `1` red month. Attribution on the 365d core is `ATT1 +39.47`, `flat +5.17`, `midterm +8.36`. This means the right action is not to replace the core with BRC1 yet; it is to keep the proven annual core live and run BRC1 as a zero-risk bear-phase shadow sleeve. I deployed that exact BRC1 shadow setup to the server with backups under `/root/by-bot/runtime/server_backups/20260506_brc1_shadow`, confirmed `bybot.service` active, allocator `ok`, safe_mode `0`, and `ENABLE_BRC1_TRADING=1` with `BRC1_RISK_MULT=0.0` in the live process environment. Also patched runtime status truth so `strategy-flags`, `status_full`, and live diagnostics show BRC1 as `shadow` instead of hiding it.
- 2026-05-06 11:45 UTC — started the targeted control-plane layer audit for the user's core concern: whether router/allocator/orchestrator helps or chokes the strategy edge. The audit is intentionally bounded to `att1`, `flat_arf1`, `breakdown_v1`, and `elder_v3`, comparing `fixed_all_regimes` against `full_stack`. This is the next real verdict source for foundation quality; do not infer foundation guilt from live silence alone until this audit returns the retention table.
- 2026-05-06 12:08 UTC — completed the targeted control-plane layer audit: [report.md](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/backtest_runs/control_plane_layer_audit_20260506_114514/report.md). Verdict by sleeve: `ATT1` is preserved by the stack (`fixed +34.90`, PF `1.328`, `418` trades, DD `6.50` -> `full_stack +28.24`, PF `1.336`, `369` trades, DD `5.16`), so the foundation is not killing our main edge. `flat_arf1` is reduced but still positive (`fixed +11.41`, PF `1.646`, `67` trades -> `full_stack +6.60`, PF `1.560`, `49` trades); this is a candidate for careful frequency relaxation, not a proof that safety must be removed. `breakdown_v1` is negative even before full stack on this annual dynamic replay (`fixed -7.02`, PF `0.886`, `300` trades; `full_stack -5.80`, PF `0.751`, `141` trades), so it needs strategy repair/phase rules rather than direct live promotion. `elder_v3` produced `0` trades in both variants, meaning the current implementation/data path is too sparse or miswired; external Triple Screen screenshots are evidence that the idea can work, not evidence that our current Elder code should be live.
- 2026-05-06 12:55 UTC — closed the most dangerous AI-operator UX gap exposed by the web screenshots. The approval box no longer displays only raw JSON: [web/static/index.html](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/web/static/index.html) now shows a human title, summary, evidence/preconditions when present, validation errors, and disables execution for blocked commands. [web/routes/ai_routes.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/web/routes/ai_routes.py) now validates AI-suggested commands before the UI can execute them. The concrete bad example from the operator (`run_backtest` for crypto sleeve `bounce1` on equity tickers `AMD/UNH/GOOGL`) is now blocked as a market mismatch. Validated `run_backtest` requests are queued to `runtime/ai_operator/backtest_requests.jsonl` instead of executing arbitrary shell commands. The AI context also now includes live close-event PnL summaries and the operator snapshot's regime/allocator/Alpaca state, so the agent has better "eyes" without receiving dangerous direct mutation powers.
- 2026-05-06 17:15 UTC — post-web-screenshot server truth pass. The local browser screenshots showing `Offline / bear_chop / safe mode` were stale local mirror state, not server truth. Direct server inspection shows `bybot.service=active`, heartbeat fresh, `open_trades=0`, `safe_mode=false`, allocator `ok`, regime `bull_chop`, enabled live sleeves `att1 + flat + midterm`, and BRC1 present only as zero-risk shadow. Synced the live mirror again and relaunched local web against `runtime/live_mirror` with `WEB_DEV_MODE=1` and a non-default JWT secret so the local console reads server truth rather than old local files. Alpaca v38 paper-gate is active: monthly-managed `AMD/UNH/GOOGL` positions are covered by broker-side simple stop orders (`AMD 323.76`, `UNH 350.13`, `GOOGL 366.24`), and the earlier Telegram text `BUY+STOP AMD $162` was a confusing notional display, not a bad stop price. However, intraday paper positions `COST/META/NFLX` currently have no open broker-side stop orders, so intraday remains paper-only and is not ready for real money. The Friday weekly comparison stack is installed on server cron: `weekly_live_vs_backtest_report.py` at `07:30 UTC` and `weekly_trade_forensics_ai_report.py --ai` at `07:45 UTC`; Alpaca v38 reminder reports are also installed for `2026-05-13` and `2026-05-27`. Research status: existing `flat_live_frequency_v3` sweep is still running, with early rows positive but failing strict promotion gates mostly on net/negative-streak constraints; a follow-on unattended queue is waiting behind it for `BRC1`, `spike_rejection`, `whale_print`, `pump_fade_v5`, `inplay_breakout_retest`, `elder_canonical`, and `support_bounce` limited sweeps under `/root/by-bot/logs/day_research_20260506/`.
- 2026-05-06 17:40 UTC — hardened the overnight research plan so the server does useful work while the laptop/app can sleep. Confirmed direct server truth again: `bybot.service` active, heartbeat fresh, `open_trades=0`, current live sleeves remain `att1 + flat + midterm`, and the active research process is `flat_live_frequency_v3` (`limit80/120` in progress). Early flat-frequency rows include several PASS candidates around `net 8.37..10.31`, PF `1.56..1.83`, DD `3.30..4.71`, so ARF1/flat frequency repair is worth finishing before any live change. The existing follow-on queue was waiting for current research and will run `bear_regime_continuation`, `spike_rejection`, `whale_print`, `pump_fade_v5`, `inplay_breakout_retest`, `elder_canonical`, and `support_bounce`. Found and fixed a server queue hygiene issue: `elder_canonical_rewrite_v1.json` and `support_bounce_v1_annual_repair_v2.json` were missing on the server, so those specs were copied from the local repo; `liquidity_sweep_reversal_v2` was also local-only, so its strategy/spec were copied and a separate follow-up queue was installed to run it after the main queue. Do not promote any of these new sleeves directly from a single sweep: the next gate is standalone 180d/365d, then portfolio additivity vs current live core, then zero-risk shadow or tiny canary only if drawdown and monthly distribution survive.
- 2026-05-07 06:40 UTC — morning continuation after the user asked for a real scanner/AI operator upgrade rather than another vague status page. Direct server check still shows the trading bot alive with fresh heartbeat and `0` open trades; the active live core remains `att1 + flat + midterm`, with BRC1 still zero-risk/shadow. The overnight research chain did not finish the full strategy list yet: the old wait loop had a self-match bug and could wait on its own `pgrep`; I killed only stale queue wrappers and restarted a clean server queue under `logs/day_research_20260507/`, now running `bear_regime_continuation` first and queued for `spike_rejection`, `whale_print`, `pump_fade_v5`, `inplay_breakout_retest`, `elder_canonical`, `support_bounce`, and `liquidity_sweep`. Product-side, I added the first operator-facing `Setup Scanner` API/UI slice: [web/routes/data_routes.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/web/routes/data_routes.py) now builds setup cards from `runtime/geometry/geometry_state.json`, `runtime/router/symbol_router_state.json`, and allocator state; [web/static/index.html](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/web/static/index.html) now has a `Setups` page showing symbol, setup type, side, candidate strategy, level, ATR distance, invalidation, router profiles, and runtime risk. This does not auto-trade. It gives the bot and the AI operator a real “what am I watching?” surface, so future live silence can be diagnosed as missing setups, blocked setups, or strategy logic failure instead of a mystery. Roadmap also records the Alpaca decision: v38 stays the protected compounder; active frequency must come from a separate income lane with broker-side stop coverage.
- 2026-05-12 14:12 UTC — fixed the Telegram CPI/news-blackout spam. The news filter itself was behaving correctly by pausing breakout entries during the high-impact `US CPI` blackout, but [smart_pump_reversal_bot.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/smart_pump_reversal_bot.py) was sending one `BREAKOUT SKIP` Telegram message per symbol per check. Added `BREAKOUT_NEWS_BLACKOUT_TG_EVERY_SEC` with a default one-hour throttle and changed the live message to a single aggregated `BREAKOUT PAUSED` alert per blackout reason. Deployed the patch to `/root/by-bot`, compiled it on the server, restarted `bybot.service` only after confirming `open_trades=0`, and verified the service is active with a fresh heartbeat.
- 2026-05-12 18:18 UTC — closed the most important Alpaca v38 execution gap before considering the first real-money pilot. The server logs showed monthly v38 trying to software-close profitable `AMD` while the whole fractional position was already reserved by a broker-side stop order, producing repeated Alpaca `held_for_orders` 403 errors. I updated [equities_alpaca_paper_bridge.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/scripts/equities_alpaca_paper_bridge.py) so v38 can promote a profitable monthly position into an Alpaca-native `trailing_stop`: it cancels existing fixed sell protection, submits a broker-hosted trailing stop, and falls back to a fixed stop if submission fails. I also enabled the native trail knobs in [alpaca_v38_hybrid_top4_candidate.env](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/configs/alpaca_v38_hybrid_top4_candidate.env) and [alpaca_paper_v36_candidate.env](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/configs/alpaca_paper_v36_candidate.env). Local validation passed: Python compile, shell syntax check, and diff check. Current server truth remains: Bybit bot alive, no safe-mode hard block, allocator degraded only by `overall_health_watch` and portfolio overlap; router is OK and dynamic; support-bounce annual repair is actively running under the server research guard.
- 2026-05-12 18:31 UTC — tested the new Alpaca trail path against the actual paper account and found Alpaca's practical small-account limit: native `trailing_stop` orders are rejected for fractional shares (`AMD/UNH/GOOGL` on `$500` are fractional). The fallback fixed stops were restored correctly, so the account was not left naked. I then adjusted the bridge so fractional positions skip native trailing and use software trailing safely: cancel fixed sell protection first, then close the position, and restore a fallback stop if that close fails. Whole-share positions can still use native trailing. This means `$500` v38 can now be tested with broker fixed stops plus safe software trailing; native broker trailing becomes available once position sizes are whole-share-capable.
- 2026-05-12 18:58 UTC — closed the follow-up Alpaca v38 churn bug found by the post-fix dry run. After software trailing closed fractional `AMD`, the monthly bridge immediately planned to buy `AMD` again because it was still one of the current-cycle picks. Added a re-entry block ledger in [equities_alpaca_paper_bridge.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/scripts/equities_alpaca_paper_bridge.py) and enabled `MONTHLY_REENTRY_BLOCK_ENABLE=1` / `MONTHLY_TRAIL_REENTRY_BLOCK_DAYS=21` in the v38 paper configs. Deployed the patch to the server and backfilled `AMD` into the block. Control dry-run now shows `reentry_blocked_symbols=["AMD"]`, no planned AMD rebuy, and broker stop orders still present for `UNH` and `GOOGL`.
- 2026-05-13 15:55 UTC — fixed the Telegram `/ai` crash that appeared after the user asked whether the operator sees the new scanner/context. The bug was concrete: `_deepseek_local_regime_hint()` in [smart_pump_reversal_bot.py](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/smart_pump_reversal_bot.py) referenced `ws_connect` and `ws_disconnect` without loading them from diagnostics. Patched it locally, compiled cleanly, deployed the monolith to `/root/by-bot`, compiled on the server, and restarted `bybot.service`. Fresh server truth after restart: heartbeat fresh, `Trading: ON`, `DRY_RUN=OFF`, Bybit auth OK, `safe_mode=false`, `hard_block=False`, current applied regime `bear_chop`, live sleeves `midterm + sloped + ATT1 + ASM1 + flat + breakdown`. The allocator remains `degraded` as a risk haircut, not a hard stop. Also wrote the standing validation rule into [ROADMAP.md](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/docs/ROADMAP.md) and [CODEX_STATUS_20260513.md](/Users/nikolay.bulgakov/Documents/Work/bot-new/bybit-bot-clean-v28/docs/CODEX_STATUS_20260513.md): every promoted strategy must be compared as standalone/static and full control-plane/live-effective, and losing trades must go through forensics before strategy changes.
